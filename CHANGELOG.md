...
### Understandable team activity

- Added host-observed delegated states across Chat, Activity, Team, and Dashboard: assigned, working, waiting,
  done, and stopped, with step counts, elapsed time, waiting targets, and display-only stalled detection.
- Counted model response streaming as activity without treating model-written prose as proof of work.
- OpenAI-compatible command, write and public-web approvals report "Waiting on your approval" around the unchanged
  approver call (reference-counted for overlapping approvals), matching the Claude and Codex routes.
- A `needs-rework` decision's worker turn counts as live delegation work, and its reply reaches an idle coordinator
  through the existing async-result wake, so the PM reviews it and reports to the user. At most two automatic rework
  requests per delegation handle are sent after the user's last message; a third is recorded, not sent, and the
  closeout says so.
- Replaced internal coordinator closeout language with one plain UnodeAi line, emitted only when delegation
  decisions or verification remain outstanding; rejected and deferred results are named.
- The Dashboard opens with Latest tasks, then the team lanes, then totals, savings and charts.

### Honest integrations and safer recovery

- Preserved MCP `isError` as a typed integration failure across OpenAI-compatible, Claude, and Codex routes;
  responsive application errors do not quarantine the server and do count toward repeated-failure protection.
- Surfaced a bounded stderr tail on managed stdio connection failure, redacting SecretStorage-resolved values
  (chunk-safe) and leaving ordinary values readable. Image, audio, resource, and structured-only results use bounded
  summaries.
- Consolidated team setup into one **Create/Save/Load a Team…** entry: system defaults, save (a modal with Save to
  This project / Save to All projects), load, delete with a scope-specific confirmation, and one automatic-snapshot
  browser. Save Team and Open Saved Team remain registered for keybindings but are hidden from the palette.
- Added host-stored SHA-256 fingerprints for exact project-team saves. A recognised save offers one team-level
  confirmation listing each agent's permissions; edited, copied, or repository-provided files load read-only and open
  an unticked per-agent review, which offers the primary plus role-template skills for pre-0.9.87 saves. Restores
  change only skills, tools and playbooks, agent by agent, capped to the saved tools, with Codex kept on
  ask-for-approval; a legacy model setting no longer blocks a restore, and one failing agent no longer reverts the
  team.
- Standardized current user-facing labels on **Claude CLI** and **Codex CLI** while preserving internal route ids
  and historical release records.

### Validation status

- Candidate `2b77988` passed the clean-worktree release audit (3,298 tests; focused mutation 77/77) and the Owner's
  VS Code field test in a two-folder workspace.
- A real Anthropic-translating gateway safety-stop run remains open when no configured credential is available;
  no live-provider result is inferred from mocks.

## [0.9.86] - 2026-09-25

### Managed integration hardening

- Added real-client lifecycle coverage through the Claude loopback bridge and Codex dynamic-tool route. Missing or
  empty secrets now fail before client creation, and independent startup waiters no longer cancel a shared mount.
- Added structural `cancelled` and `outcome_unknown` results. Post-dispatch cancellation or timeout discards late
  success, quarantines the connection, forwards cancellation to the MCP client, and automatically remounts through
  the normal approved path. Calls distinguish temporary reconnection from a revoked grant.
- Replaced readable Claude bridge trust with fresh 128-bit per-session identities and exact registered-tool matching.
  Old fixed names and similar prefixes receive the normal fail-closed approval decision.
- Added Marketplace **Your integrations** status plus remove-and-revoke for custom and catalog integrations. Removal
  revokes direct and Skill-derived grants, cancels calls and readiness, and retains shared SecretStorage entries.
- Made native Codex MCP activity visible as Codex-governed, non-UnodeAi evidence; separated distinct Codex response
  parts and documented the fresh-conversation boundary when an immutable dynamic-tool surface changes.
- Restored Claude managed-MCP grant audit attribution only through the calling session's exact opaque integrations
  bridge. Another valid-looking suffix and the old fixed name attribute to nothing.
- Fixed executable-action completion after child exit, including inherited output pipes, and rejected inherited
  object properties when resolving compiled handlers. No bundled Skill exposes an action.

### Worker safety stops

- Replaced the 12-step OpenAI-compatible working limit (and Solo's separate 25-step limit) with one 100-iteration
  emergency backstop. Normal long reviews can continue past 12 steps.
- A step-limit or anti-spin stop now makes exactly one final model request with no tools, returns a decisive partial
  answer with a typed stop reason, and can never reach a coordinator as completed work. Its request-only history
  preserves tool results as explicitly labelled untrusted evidence—never instructions—so Anthropic-translating
  gateways need no tool declaration and reviewed content cannot impersonate the closeout instruction.
- A typed stop leaves the model-fallback failure streak unchanged: it is neither a provider failure nor a success.
- Added `continue_task`: the coordinator resumes the same worker conversation with prior context and a fresh
  allowance, while the host issues a new attempt-bound grant instead of reviving expired authority.

### Security

- Claude's native MCP approval gate remains in place, repository CLI automation remains behind content-bound consent,
  and Codex-native MCP remains governed by Codex. Managed integrations retain host-only secrets and live grant checks.

## [0.9.85] - 2026-09-24

### Native CLI parity and governed integrations

- Preserved Codex's native base instructions and added UnodeAi roles through App Server developer instructions.
- Added isolated, temporary native Skill roots for Codex Playbooks, with stable-binary discovery, exact path/name/
  enabled checks, restart/resume support, and cleanup without changing `~/.codex/config.toml`.
- Allowed either Claude or Codex to be the host-selected coordinator; added Codex dynamic team tools and live
  `turn/steer` support.
- Kept native user-level MCP for both CLIs. Repository MCP remains behind repository-configuration consent;
  Claude native calls keep the fail-closed tool gate, while Codex native MCP follows Codex's own approvals.
- Made Agent Builder integrations route-uniform and host-managed. Startup now waits up to 45 seconds for granted
  integrations; resolved failures require an explicit not-now, retry, or continue-without decision, while an
  unanswered approval reaches the deadline and stops startup visibly.
  Managed calls recheck the current saved grant, and Codex reports when a changed immutable tool surface requires
  a fresh conversation.
- Replaced executable files in Skills with fixed handler ids compiled into the VSIX. Every action requires
  Workspace Trust and a fresh digest-bound approval; Skill folders and remote catalogs remain instruction-only.
  The fixed runner uses bounded input/output and a hard-return timeout with a single best-effort process-tree kill.
- Clarified Codex permissions: `write` selects workspace-write, no `write` selects read-only, and the CLI has no
  separately removable execute switch. Explicit empty project-team tools are read-only; legacy host rosters retain
  native-default compatibility.
- Security: Claude's Agent Builder integrations now run in UnodeAi's host, so their secrets are no longer handed to
  the Claude CLI. New host-launched processes are the managed stdio MCP servers and the fixed action runner; both are
  listed in `SECURITY.md`, whose v0.9.85 audit records the real-binary evidence.

## [0.9.84] - 2026-09-24

**Claude Headless in v0.9.83 and earlier loaded a repository's `.claude` hooks and `.mcp.json` servers without a
UnodeAi prompt. v0.9.84 asks first.**

- **OpenAI Codex CLI is available through a consent-gated App Server route.** After local version/login checks,
  UnodeAi asks before starting App Server or making a model request; the grant is separate from ordinary OpenAI
  API consent and can be revoked. Codex uses the user's own login and configuration; UnodeAi does not read its
  credential files or control the CLI's network destinations.
- **Codex runs under its own permission profiles, chosen per agent.** In Act mode: **Ask for approval** (default —
  workspace-write sandbox with network off; routine work inside the workspace runs, and an escalation Codex
  requests shows a UnodeAi card), **Approve for me** (Codex's own reviewer decides escalations, and its reviews
  appear in chat), and **Full access (unsafe)** (no sandbox, network on, no prompts, no UnodeAi checkpoints;
  confirmed every time it is turned on). Plan mode, untrusted workspaces, read-only Folder Access and a narrowed
  tool ceiling resolve to Read only, and an agent with explicit Folder Access grants cannot use Full access. A
  repository or workspace setting cannot select a profile. Codex does not ask before every command or write, and
  its routine edits are not UnodeAi checkpoints. On Codex 0.155.1, MCP and plugin tools that Codex calls from
  inside its own `exec` code tool ran without an approval request.
- **The Codex boundary is checked against what App Server reports, and measured on the real binary.** UnodeAi
  pins the profile at the process, thread and turn, refuses a thread whose sandbox, network, writable roots or
  temp-folder exclusions differ from what it granted, and names any root it rejects. Only folders the agent may
  write are offered to Codex as workspace roots, because a resumed Codex thread makes every such root writable.
  A release probe exercises each profile as an effect on Codex CLI 0.155.1.
- **First-use Codex setup now works in every editor profile.** The model picker opens its full connection list
  before filtering and names its count/source. When no Codex account cache exists, **Load models from Codex**
  detects supported npm/user installs before PATH, shows the version and path for confirmation, verifies login,
  asks for executable-bound Codex consent, and runs a model-list-only App Server outside the workspace. Switching
  the selected binary asks again. Prerelease builds below the 0.155.0 floor, such as an editor-bundled
  `0.155.0-alpha`, are not offered or accepted. An absent generated tool ceiling also stays absent instead of
  being persisted as `[]`; legacy empty ceilings remain tolerated.
- **An agent can always be saved on the connection you chose.** Editing an agent whose connection cannot load
  UnodeAi skills, playbooks or MCP grants (such as OpenAI Codex CLI) previously kept those selections greyed out
  and then refused the save. Saving now removes them and says so; removing a grant only narrows the agent.
- **UnodeAi never makes Codex trust a folder in your own Codex configuration.** In a non-git workspace, an agent
  working in a subfolder could previously lead Codex to write a `trust_level = "trusted"` entry for that
  subfolder into `~/.codex/config.toml`. The process-local trust override now covers every folder from the
  agent's working directory to the workspace root, so nothing is written.
- **The audited dependency tree is clean.** The production Hono override is updated to 4.13.9, and the release
  test runner/js-yaml development dependencies are updated to patched versions without a Vitest major upgrade.
- **Repository automation asks before it loads.** Before Claude or Codex starts, UnodeAi detects repository
  `.claude`, `.mcp.json` and `.codex` configuration at the locations each native CLI uses, presents a redacted
  summary, and binds approval to the exact contents. Opening only a monorepo subfolder cannot bypass the
  dialog, and a changed file asks again. User-level `.claude` and `.codex` directories in the home folder remain
  uninspected. Multi-root workspaces bind the check to the folder containing the agent.
- **Gateway consent is scoped to the request that will actually run.** Grants are bound to profile, destination
  and revision. Existing users are therefore asked once again for each gateway after updating; this is an
  intentional scope upgrade, not a lost API key or connection setting.
- **Scheduled price refresh is visible and controllable.** Per-provider and global automatic-refresh toggles are
  on by default, stored as user choices, limited to previously approved metadata hosts, coalesced when triggers
  overlap, and backed off after failures. No scheduled request runs at activation.

## [0.9.83] - 2026-09-21

**Local project-discovery consent can be recovered without creating a prompt loop.** A decline now blocks the
resolved root for one top-level user-request epoch shared by the coordinator and every delegate. Immediate tool
retries, model repair and delegation do not reopen the modal; a later direct user request may ask again. Approval
remains session-only and the read, write, shell, task-scope and Workspace Trust boundaries are unchanged.

- The refusal raises one non-modal **Grant folder access** action bound to the exact refused root. It reopens the
  full confirmation rather than flipping a flag. The Security panel lists session-only granted and declined roots
  and provides the same grant/revoke controls.
- A thrown or unavailable dialog fails closed and suppresses retries for that request, but is not recorded as a
  user decline. Revocation blocks the root before the next tool call.
- The release test baseline is pinned to this candidate's exact suite count. The packaged-document link gate now
  validates the explicit shipped-document manifest directly instead of recursively invoking `vsce ls`, so ignored
  and untracked release worktrees cannot turn a documentation failure into an indefinite hang.

## [0.9.82] - 2026-09-20

**Project state now stays with its project.** Every project-sensitive value stored in editor workspace state is
bound to the canonical first workspace-folder root. Reusing an editor storage bucket for another folder cannot
carry across rosters, chats, checkpoints, runs, policies, integration evidence, MCP approvals or launch
configuration. Switching roots also clears live selection and presentation state. Existing VS Code and Cursor
state is imported only for the same proven root; no legacy Devin history is imported. Reset Workspace State
removes only the current root's namespace, and checkpoint restore independently revalidates its destination.

- **Declining model-network consent is terminal for that request.** A refusal remains a typed consent decision
  through OpenAI-compatible transport retry, request-body repair and streaming fallback. One decline produces
  one terminal result and zero requests; it is not retried as a network error.
- **A green release test means the whole suite completed.** The release runner rejects worker/pool errors,
  missing summaries, count mismatches, pending tests and suite totals below the checked-in baseline. A Windows
  native worker crash was traced to repeated loading/unloading of pdf.js's optional native canvas binding; PDF
  extraction now keeps that optional native module out of the isolated one-shot worker, and content workers exit
  naturally. A scheduled Windows job retains all 20 complete-suite stress reports.
- **Extension-host E2E remains a candidate gate.** Normal tests launch with a fresh temporary workspace, while
  the deliberate no-folder refusal has its own launch configuration.
- **Public drops identify their private source.** A checked-in root `SOURCE` record carries the exact 40-hex
  private commit for reviewers with private-repository access. The release checker rejects missing, malformed or
  mismatched records; this identifies the private source but is not independent public proof of it.

## [0.9.81] - 2026-09-19

### Security — update now

In every version represented by this repository before 0.9.81, a repository team file could place values such
as `NODE_OPTIONS` or `PATH` in an agent's provider process. It could also set the legacy `autoApprove` field,
which was able to select Claude's `bypassPermissions` mode for a write-and-execute agent. On Windows, a planted
`claude.cmd` or Git command in the working folder could also win a host-initiated shell lookup. VS Code and Devin
required the folder to be trusted first; Cursor does not supply that trust step. Update before opening and running
agents from an untrusted repository.

- `.unode/team.json` can now describe agents and narrow permissions, but cannot grant execution authority.
  `env`, `autoApprove`, `backend`, `baseUrl`, `autoRestart`, `workingDirectory`, `routeRepair`, executable
  `skills`/`playbooks`, `toolProtocol`, `editToolDialect`, and per-agent or top-level `mcpServers` are ignored
  with one visible notice and are no longer written back. Broader tool requests are ignored; folder and command
  restrictions remain only when they narrow the current host boundary.
- Because older workspace state did not record whether a roster came from the project file, the first 0.9.81
  activation removes only `env` and `autoApprove` from source-unknown saved agents and names those fields.
  Existing Skills, Playbooks, MCP grants, routes, backend choices and other user configuration remain intact.
- Existing Marketplace integrations are preserved only when the workspace already contains the exact prior
  approval for their launch configuration. The upgrade imports those integrations from the legacy team file and
  names any unapproved project entries it ignored. MCP approvals and host configuration move from
  `roam.approvedMcpServers` / `roam.hostMcpServers.v1` to `unode.approvedMcpServers` /
  `unode.hostMcpServers.v1`; downgrading to 0.9.80 therefore requires approving the integrations again.
- Programs UnodeAi itself starts are resolved to absolute host paths without consulting the workspace or relative
  `PATH` entries; workspace/worktree executables are refused. Git runs without a shell, approved stdio MCP
  programs are fixed to the resolved host path on both backend routes, and the Claude tool-gate wrapper is
  replaced by the host before every launch. Commands whose literal text you approved keep their project-relative
  behavior.
- OpenAI-compatible MCP placeholders use SecretStorage. The Claude CLI's direct MCP route may still inherit a
  same-named variable from the editor process, but a project team file can no longer supply or override it. This
  remains a documented limitation of the current release.

## [0.9.80] - 2026-09-19

**MCP listings are governed Integrations.** The active bundled catalog now requires an authoritative source,
maintenance state, pinned install identity or endpoint source, and a bounded verification date. Seven entries
from the archived reference-server repository were removed from active one-click configuration, and the legacy
combined catalog's MCP array is explicitly quarantined. A permanent gate plants and kills archived-source,
missing-provenance, missing-maintenance, installable-deprecated, unknown-schema, identity-mismatch, and
display/enforcement-classification and mutable-identity failures in source, bundled-VSIX, and public-drop paths.
An installed extension labels an overdue entry for re-verification but keeps the catalog visible.

- Marketplace cards and MCP mount enforcement consume the same host-derived approval classification. Catalog
  content can no longer state a weaker answer for a subprocess, remote endpoint, or credential-bearing server.
- Listed, configured, approved, mounted, exercised, and succeeded are separate host facts. Cards say **Adding
  configuration…**, persist exercised/succeeded evidence against the exact workspace and launch fingerprint,
  and offer a read-only **Check setup** that grants nothing. **Check setup** and every other notice that directly
  answers a user action open as a modal dialog with the editor's single **OK** by default. Settings → More can
  make those same results quiet corner notifications; this user-only choice never changes approvals, consent,
  or dialogs with action buttons. Notices raised by background events remain non-blocking toasts.
- Role Templates and *Create a team* now project the same 52-entry catalog. **Your Agents** includes the current
  roster—including hand-built agents—and reports recorded template/custom origin or *origin not recorded*.
  Integration suggestions are display-only and never configure or grant a server.

**The v0.9.79 field findings are corrected.** A Cursor 3.20.10 probe proved a repository-level
`unode.commandApproval` value becomes effective. All 40 authority-bearing settings—including Smart Mode,
model-tier routing, pricing and the skill-library link—therefore resolve only from User Settings and manifest
defaults in every editor. VS Code greys project copies of these application-scoped keys and supplies its own scope
explanation before an extension can inspect them; UnodeAi's ignored-values notice is a backstop in editors that
do expose project values. *Allow for project* command approvals
moved out of `.vscode/settings.json` into inspectable and revocable UnodeAi-owned workspace state without changing
the user's global allowlist. Workspace writers no longer fall back to the editor process directory; a first-folder
change stops active agents, Integrations, and UnodeAi terminals, closes Team Rules, and refuses later turns and saves
until reload. UnodeAi terminals carry a durable creation marker, so activation and invalidation also find terminals
left by an earlier extension host; cleanup kills the process tree before closing the tab and never touches a user's
unmarked terminal.

Consent, egress and destructive dialogs now place an explicit non-consent action first so Enter chooses the safe
outcome; the structural gate also requires effects to follow an explicit affirmative result, so Escape/Cancel
cannot fall through to consent. The metadata-host picker starts with no hosts selected. Every coordinator turn—including a crashed
Claude turn—publishes a separate host-receipt delegation footer. Queued work stays pending and publishes a
corrected accepted/refused count when admission resolves.
On Windows, a PowerShell-only cmdlet sent to the `cmd.exe` runner is refused before any approval prompt with a
shell-compatibility reason and a native equivalent; `Start-Sleep -Seconds N` is mapped to the noninteractive-safe
`ping -n N+1 127.0.0.1 >nul`, and an Allow click can no longer be rewritten as missing consent.
An admitted delegation whose worker or extension host disappears before a terminal reply is now durably
`interrupted`, with separate last-observed and detection timestamps. Activity stops its timer and excludes it
from active work. A host-instance epoch plus renewable heartbeat prevents a second live editor window from
interrupting or overwriting the first window's assignment; only the same window's dead epoch or an expired
foreign lease is reconciled. Host shards consolidate into a bounded v8 baseline; only merged stale shards are
collected, idle ownership ticks do not read them, and a window resuming after a long stall skips one foreign
lease judgment. Unchanged baselines are not rewritten on each active tick, and migration shards with no owner
heartbeat are collected after their exact bytes reach the baseline. The output channel records the resolved host
identity and reconciliation branch; when Reload
Window changes the session id, interruption can take up to the 30-second lease instead of being immediate.
Live worker loss ends attempt-scoped grants, while restart reconciliation closes
the restored identity after its process-private grant store is already gone. The handle cannot auto-wake,
cancellation is idempotent, and the coordinator can explicitly abandon it or admit and link an atomic
replacement without hiding the interruption. A later exact-owner heartbeat repairs a stale `host-restarted`
observation only before any coordinator disposition; abandoned, superseded and replaced attempts stay terminal.
A run containing interrupted work can close only as partial.
Portable Run Evidence
moves to `portable-run-evidence/4` so `interrupted` and its separate mechanical count do not silently widen `/3`.
Whitespace after `<` in a known tool call remains invalid: it is suppressed as ordinary reply text, shown as a
failed call, never executed, and fed back for one bounded retry. Memory review names the writer, duplicate Cancel
buttons are structurally forbidden, and the transcript warning is now a display-integrity sensor: a stable item
omitted from a later full-state push is a real display bug, including a streaming row, while expected trims of
the independently bounded message, reasoning, tool, delegation, context-card and marker streams are classified
separately even after those streams are interleaved into one transcript. An interrupted task's first terminal
`superseded` or `abandoned` disposition now wins: later contradictory dispositions are refused without changing the
ledger, and both status inspection and idempotent cancellation name the recorded disposition.
Turn footers now lead with the local finish time, or the local date and time for an older turn, before the existing
elapsed time and approval-wait exclusion.

Workbench approval cards now make the composer content-aware. While a command, tool, or write decision is
pending, a column that cannot fit both the decision and its measured composer reservation returns the composer
to normal flow; 24px of clearance is required before it floats again. This closes the 561–760px editor-height
band where a long approval's note and action buttons could sit under the fixed composer without any scrollable
overflow. The short-viewport and content-triggered fallback styles are source-pinned to remain identical.
The native-subagent warning now uses the compact **Disable subagents** action so all three controls remain
readable in VS Code's fixed notification row at narrow window widths, while the message states before the
decision that disabling applies to this agent only.

Opening an untrusted folder now shows one activation-scoped UnodeAi dialog explaining that agents remain
read-only: chat, planning, reads and searches work, while shell commands, file changes, MCP servers and the verify
command are disabled. The dialog points to **Workspaces: Manage Workspace Trust**; trusted and folderless windows
do not show it, and the detailed capability wording comes from the extension manifest.

**Exact-content claims now need an exact-content receipt.** `publish_content_receipt` records `shown`, `partial`,
or `not-delivered` against the bytes the host actually published, so agent prose alone cannot claim a requested
file was shown.

The manual and wiki now route message-log, run-evidence, portable-evidence and worker-progress exports through
the visible **UnodeAi: Export…** picker instead of telling users to invoke implementation commands hidden from
the Command Palette.

**Editor naming follows the recorded product.** Current README, package and wiki surfaces say VS Code, Cursor
and Devin. Historical changelog entries keep the name used at their date.

**Known issues deferred to v0.9.81.** Each fails safe; workarounds are in the manual's Troubleshooting section.
- Revoking Workspace Trust while Chat is open restarts only the extension host; a send made from the surviving
  chat can stay at **Sending…** until **Developer: Reload Window** (the draft is kept and nothing runs).
- Choosing **Do not allow** on the network-consent dialog is retried like a network error, so the dialog can
  return up to four more times before the turn ends as declined. Nothing is sent on any attempt.
- Delegation summaries restored after a reload are grouped at the next render point and show raw agent ids.
  Presentation only; no transcript content is removed.
- Polling a running background command can trip the repeated-call breaker and leave a worker's interim
  narration as its reply.
- A run whose only interrupted task is superseded from a later run stays visibly open; its evidence is not
  presented as complete.
- In Devin, the reusable agent-window workspace carries a team's roster and chat history into another project.
  No write reaches the wrong folder.

**Field record.** The released bytes were field-tested in VS Code 1.137.0, Cursor 3.21.13 and Devin 3.8.20
(`docs/FIELD_TEST_v0980.md`). VS Code ran every step; by Owner decision Cursor and Devin ran the steps whose
result can depend on the editor — install, dialogs and Enter defaults, repository settings, reload recovery,
the no-folder window and the first-folder switch. Cursor 3.21.13 withholds application-scoped repository
values from extensions without greying them; Cursor has no untrusted state, so its trust rows are not
testable. In Devin, the v0.9.79 write into its program folder does not recur.

**Security note.** New host process: to close UnodeAi's own terminals, the extension runs `taskkill /T /F` on
Windows (or signals the process tree elsewhere) against terminals carrying its creation marker only. The new
`unode.notifications.resultStyle` setting is an application-scoped authority key read from User Settings only.
No new network destination, credential path or MCP behaviour. See SECURITY.md's 2026-09-19 audit entry.

## [0.9.79] - 2026-09-08

**Cross-provider dispatch no longer asks per dispatch by default — an Owner decision, recorded as a relaxed
consent default.** v0.9.75 added a modal whenever a coordinator brief would travel to a teammate on a different
model provider, because a brief can paraphrase your documents. In practice it appeared on every such delegation.
The new **Allow cross-provider dispatch without approval** toggle (Settings → More;
`unode.allowCrossProviderDispatchWithoutApproval`) defaults to on; turning it off restores the modal. The setting
is application-scoped and listed in `restrictedConfigurations`, so a repository's settings file cannot turn
approval back off after you turned it on. It removes a dialog, not a check: an unresolvable destination still
refuses the dispatch, same-destination dispatch never asked, and each send made without a dialog writes its
destination host to the output channel. The modal, when shown, also lost its duplicate **Cancel** button — the
code passed its own `Cancel` to a dialog that already has one.

**The outcome of something you just did now appears where you are looking.** A result or refusal that follows a
dialog or picker you have just answered used to appear as a corner notification. The Owner's a13 field run found
*Shared-memory attestation is disabled until you trust this workspace* sitting in the bottom-right corner where
nobody would see it. Thirty-two such messages are now centred modal pop-ups: reviewing, attesting and revoking shared
memory; applying execution hooks; restoring a checkpoint or archived chat; importing a chat or activity log;
exporting run evidence; saving or opening a team; recording a human verdict; sending a demo task; adding an MCP
server; choosing an agent icon; adding or updating an agent; setting the verify command; and storing or removing a
key. Deliberately unchanged: progress notices (*Starting all agents…*, *Workflow is running*), activation-time
migration notices (a modal at startup would block the editor), and four messages that are themselves questions
with buttons.

**The price-coefficient prompt now reads as a question you can answer, and the manual stopped contradicting
it.** Storing an API key asks what fraction of the published price that key pays. The prompt said *"Leave this
blank or press Esc for 1 if you do not know — the gateway does not report this, so UnodeAi cannot check it"*:
two actions welded to one dangling outcome, followed by a disclaimer where the instruction belonged, and no
mention of the one fact that makes the number worth thinking about — every cost estimate is computed from it.
It now states the choice, gives the range with examples, says plainly that Esc and blank both accept 1, and
says why it matters. The two refusal messages were rewritten for the same reason; one had named `0` twice
while describing the range as "0 or a positive number".

**A behaviour change from v0.9.58 had left three documents wrong for twenty-one releases.** `USAGE.md` and
both wiki pages stated that an empty coefficient box is refused. It has not been refused since `f9a6f72`:
the box opens empty so that an Enter still travelling from the preceding key prompt cannot accept a price
before the question is read, and because it opens empty, empty means list price. The day before, `2999a45`
had fixed the genuine *blank means free* defect by refusing blank and had documented that accurately — the
next day's change corrected the behaviour and left the sentence behind. `check:docs` compares version stamps,
not claims, so nothing failed. The three documents are corrected, and the test that owns this rule now also
asserts that no shipped document describes the refusal.

**Shared memory can now inform the team without becoming authority.** `pitfall`, `contract`, and `decision`
remain agent-selected semantic kinds, but none grants trust or admission priority. Rows reloaded from the mutable
primary-workspace `.unode/memory/notes.md` file are explicitly `untrusted` unless an exact-row digest has been
confirmed through the new **UnodeAi: Review Shared Memory** host command. That local action can be cancelled,
attested, and revoked; editing one byte invalidates the match.

**Attestation is deliberately narrower than approval or verification.** Its versioned, bounded workspace-state
record stores a normalized-workspace digest, exact-row SHA-256, timestamp, and fixed `local-user` action marker —
never note prose or a claimed person identity. It changes prompt admission and display only. It cannot widen
tools, folders, commands, writes, network, MCP, delegation, execution hooks, completion, or evidence authority;
untrusted workspaces refuse new attestations and treat every row as untrusted. Nothing automatically promotes a
memory into Team Rules, a skill, workflow, hook, or task contract.

**Prompt structure and source scope now fail closed.** Shared-memory load and append physically resolve the file
and its existing ancestors inside the first workspace folder, refusing symlink/junction escapes. Rendered text
and metadata are structurally encoded and bounded by both rows and bytes, so delimiter text, forged trust fields,
and malformed legacy rows remain quoted data. Exact duplicate raw rows consume one prompt slot; human-attested
rows are admitted first and remaining slots use recency across all kinds, with final output in file order.

**The context receipt now describes the exact memory snapshot supplied to that turn.** Before this release,
project context and the manifest called `sharedMemory.block()` independently; a note/reload between those calls
could make the receipt describe a different snapshot. One atomic selection now supplies both. Its manifest entry
retains only bytes and bounded typed counts of selected `untrusted` and `humanAttested` rows, not note bodies, and
context receipts remain excluded from portable evidence.

**A restored dispatch is no longer presented as work that is still running.** The durable Run Ledger can retain
an `active` row after activation even though the old worker promise and result correlation cannot survive with it.
`inspect_task_status` previously projected that combination as `within-deadline` / `pending` and told the
coordinator to wait for a wake that could never happen. It now reports the task as interrupted, preserves the
durable lifecycle as history, and directs the coordinator to dispatch a replacement. A genuinely current-host
dispatch remains pending even when its worker is stopped in the concurrency queue.

**Candidate field record is still required before publication.** This source build has not yet been manually
exercised through write, cancel, attest, revoke, edit-invalidate in VS Code, Cursor, and Windsurf. Source tests
cover propagation and authority invariants; they are not a substitute for running the exact bundled VSIX in each
named host.

## [0.9.78] - 2026-09-08

**UnodeAi is now presented as an organized AI workforce rather than a multi-model team.** The extension
name, the store description, this repository's overview page and the Chinese positioning document all lead
with the same claim: companies already solved how groups of workers coordinate — defined roles, a manager,
handoffs, a record of who did what — and this product applies that structure to AI agents. Model choice
remains available and is no longer the headline.

**Every outward claim was graded before it was rewritten.** `docs/MARKETING_CLAIMS_LEDGER.md` records each
public statement with an evidence grade and an action. Five claims were removed from the Chinese positioning
document rather than softened: verified-only merges and auto-merge described as capabilities when
`autoMerge` defaults to false, an opinion about verification gating stated as fact, an unverifiable model
count and pricing claim, and an R&D-team framing the product does not support. The editors UnodeAi runs in
are now named — VS Code, Cursor and Windsurf — in place of an open-ended claim about editors in general.

**User actions no longer depend on rewriting the editor's User Settings.** Cursor refuses a configuration
update while its User Settings JSON has an error or warning; the price-coefficient read repair used to run
before both `UnodeAi: Open Settings` and `UnodeAi: Refresh Model Prices`. The panel now opens first, and a
price refresh completes and reports its result before the repair starts. Both repairs settle independently;
a failed repair shows a warning and a direct route to User Settings JSON without aborting the requested work.

**ECC is now a pinned external source for developer-side governance assessment.** A source adapter inventories
the fixed ECC 2.2.1 snapshot at commit `5064474d4d762dc9640234a41617cccb79185cec`, resolves provenance and
licence evidence conservatively, projects candidates into a temporary workspace, and runs the existing
source-neutral ingestion gate without executing candidate content. Its deterministic report keeps ingestion
status separate from static compatibility signals and labels every candidate citation as `ingestion`; neither
is evidence of behavioural effectiveness. The public source drop contains the adapter, synthetic fixtures,
tests and metadata-only results, with a canary that runs the fixture from inside the built drop.

**Developer and source tooling only. No ECC Skill is installed, bundled, enabled, or available in the
extension in v0.9.78.** The VSIX continues to exclude `tools/**`; the report publishes paths, hashes, licence
outcomes, status/reason codes and aggregate classifications, never upstream Skill bodies, descriptions,
excerpts, scripts, attachments or rewrites.

**Runtime claims that say “this release” now have an expiry mechanism.** The known-false TeamTools statement
that no human acceptance layer existed now says what the product actually records: an optional person's run
verdict remains separate from coordinator acceptance. Five true capability-limit messages and the registry-
controlled availability messages are version-affirmed in inert JSON; a standalone syntax-aware check fails
new, removed or stale entries without treating source comments as user-visible claims.

## [0.9.77] - 2026-09-07

**Agents can now discover adjacent local projects without making you register each one in Agent Builder.**
In a trusted workspace, the default local read scope includes the workspace's parent folder. Before any agent
reads or searches that outside-workspace root, UnodeAi displays its resolved absolute path and asks once for
the current extension session; the dialog explicitly says that a search can walk the whole tree and that
returned filenames and contents may be sent to the configured model provider. Declining, dismissing, or being
unable to show that dialog fails closed. `unode.localReadScope` offers `workspace`, `parent`, and `volume`;
the volume choice adds a warning about other users' profiles. Every admitted local-scope access records an
opaque root ordinal, action, and count on its tool card and in internal and portable run evidence — never a
path, query, filename, or file contents. This is host-owned configuration: an untrusted workspace cannot
enable it, per-agent Folder Access remains a stricter ceiling, and writes plus generic shell commands remain
confined to the working folder.

**A Git tag is now a host-verified fact, not a document inference.** The new read-only `inspect_git_tag`
tool opens an already-readable local repository, resolves a tag to its commit, and reports its SHA, date and
subject. It runs a fixed Git argv without a shell, creates no locks, and applies `safe.directory` only to that
single child process; it never writes the user's global Git configuration.

**The dashboard was looked at in all four bundled themes, and it was not right.** v0.9.76 moved the status
colours onto theme tokens and said plainly that no theme had been checked. Checking them found that the
*done* dot measured 2.00:1 against the default light theme's background, where 3.00 is the floor, and its
`Verified / mergeable` chip 1.84:1 against a floor of 4.50 — because the token behind it carries one value
for dark, light and high-contrast-dark themes. The fixed green it replaced was already failing the same way.
*Done* and `Verified / mergeable` are now blue, which separates them from the green *working* dot by
lightness as well as hue in every theme; the two greens had been close enough to read as one colour in both
dark themes. Verification chips are now outlined rather than filled, because a fill of the text's own colour
makes the text less readable as the pill becomes more visible — `Failed / held` had been below the text floor
in the default dark theme for the same reason, and `Unverified` and `Not in worktree` had no visible shape at
all in three of the four themes. All twelve chip states were measured after the change and all twelve pass.

**A status dot can no longer vanish.** Four indicators read a theme token with no fallback, and an undefined
token makes the whole declaration invalid, so the dot would disappear rather than lose its colour. They now
fall back to the editor foreground.

**A streamed paragraph containing bold, code or a link keeps your selection too.** v0.9.76's fix covered only
paragraphs that were entirely plain text, which is not what most replies look like; any inline markup sent
the paragraph back down the rebuild path on every paint. The in-place path now covers a paragraph whose
rendered spans are a prefix of the new ones, preserving each already-rendered child. A paragraph whose tail is
genuinely re-parsed — a closing `*` turning trailing text into emphasis — is still rebuilt, because rendering
text the parser no longer agrees with would be worse than the rebuild it replaces. **What is proven here is
the mechanism, not the experience:** the tests hold every already-rendered child to the same object across
paints, which is what a live selection depends on, but no test in this repository can hold a real browser
selection. **Confirmed by hand after publication, 2026-09-07:** on the published build, a reply arrives
steadily line by line, an existing selection is not overwritten as text grows, and text selected while the
reply is still streaming copies out intact. One person, one machine, one theme — a confirmation, not a
matrix.

**Two panels now share one document skeleton.** This changes nothing you can see: the Dashboard and
Marketplace webviews no longer each carry their own doctype, content-security policy, nonce plumbing and base
token layer. Both panels' rendered bodies are pinned to snapshots captured before the change, and the shared
shell is covered by tests that a panel emits exactly one policy and that a script runs only under a nonce that
policy names.
