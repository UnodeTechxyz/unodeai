...
parent available so an agent can follow an already-evidenced sibling such as `../RoamCrew`; and `volume` makes
the current filesystem volume available. It is ignored in an untrusted workspace, and it never widens writes,
...
The first time any agent tries to access the local-scope root, UnodeAi opens a modal that names the resolved
...
current extension session; **Deny**, closing the dialog, or an unavailable dialog refuses the access and does
not ask again in that session. Each admitted access has a tool-card/run-evidence receipt with only an opaque
