...
During a command, write, or public-web decision, delegated work reads **Waiting on your approval** instead of
**Stalled**, including for OpenAI-compatible workers. The waiting state closes when the approval surface resolves.
...
The current schema is **`portable-run-evidence/4`**. It distinguishes complete and partial run closeout,
carries each delegation's independent completion state, counts terminal interrupted work separately, and names
the sensor-bounded input field `readReceipt: observed | not-observed`. Consumers of `/3` must treat `/4` as a
schema upgrade rather than assuming the added state and accounting field exist under the old version.
...
The **Latest tasks** panel sits at the top of the dashboard, newest task first, followed by the team lanes; the totals, savings banner and cost charts come below as reference. It shows your most recent tasks, each broken down by the agents that worked on it (a token bar per agent, plus the task's total tokens and cost) — so you can see exactly where the tokens went across a PM-led, multi-agent run. Use the **Show last: 3 · 5 · 10 · 20** control in the panel header to choose how many tasks to display.
...
| Command | Purpose |
|---|---|
| `UnodeAi: Export…` | Pick **Message log**, **Run evidence pack**, **Portable run evidence**, or another export from the visible list |
...
### Known issues retained in 0.9.82

Each of these fails safe: nothing runs, is sent, or is written that you did not approve.
