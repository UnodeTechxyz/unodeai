...
/**
 * Hashes of prior role-template prompts. They let an agent with an explicit persisted template key
 * recognize its own prior default without retaining historic prompt bodies. A hash or role alone is
 * never used to invent provenance for an older agent that has no recorded template key.
 */
...
/** Resolve identity only from a persisted template key. A role or matching prompt is not provenance. */
...
export function promptTemplateStatus(config: AgentConfig): PromptTemplateStatus {
  const templateKey = currentRoleTemplateKey(config);
...
/**
 * Safely annotate persisted configs. Only an exact match for an explicitly recorded template key
 * becomes template-managed; older agents with no key remain untouched and origin-unknown.
 */
 ...
 *
 * Custom is an ownership claim, and it must survive a round trip. Clear any prior explicit template identity
 * so an agent deliberately made Custom cannot reopen as a shipped role merely because its instructions still
 * happen to equal that template's text.
 */
