...
const DENYLIST_DECLARATION = 'src/backend/CodexSpawnArgs.ts';
...
describe('Codex spawn escape-hatch source guard', () => {
  ...
  it('keeps every sandbox-bypass literal at the reviewed Codex argv boundary', () => {
  ...
  it('flags a banned literal outside the Codex argv denylist declaration', () => {
    const offenders = codexBannedFlagOffenders([
      { f: 'src/backend/CodexSpawnArgs.ts', text: 'danger-full-access' },
...
describe('workspace-supplied settings cannot choose what we execute or where we connect', () => {
  ...
  it('does not expose the host-owned Codex permission profile as a VS Code setting', () => {
    expect(pkg.contributes.configuration.properties).not.toHaveProperty('unode.codexPermissionProfile');
  });
  ...
  it.each([
    ...
    ['unode.allowCrossProviderDispatchWithoutApproval', 'can disable the cross-provider brief approval prompt'],
...
describe('no phone-home: the extension performs no network request on a fresh install', () => {
  ...
  it('every picker path passes a provider scope to refreshPrices — standing consent is not an action-wide licence', () => {
    ...
    // must therefore refresh with { scope: <its provider> }; only the daily timer and the
    ...
    // Allowed unscoped call sites: the daily timer and explicit refresh command (deliberately all-source refreshes).
    expect(unscoped.length,
      'a new refreshPrices() call without a scope was added — picker/provider-specific paths must pass '
      + '{ scope: providerId }; only the timer and the explicit Refresh command run unscoped'
    ...
    expect(dialogs.text, 'the Add-Agent dialog picker must scope its price refresh to the selected provider')
      .toMatch(/refreshPrices\(\{ scope: providerKey, trigger: 'Add Agent model picker' \}\)/);
    ...
    expect(ext.text).toMatch(/refreshPrices\(\{ scope: connectionId, trigger \}\)/);  // agentBuilderListModels
  });
  ...
  it('activation does not refresh prices; the later background timer cannot open a modal', () => {
    ...
    const timer = /scheduleDailyAutomaticRefresh\(\s*setInterval,\s*\(\) => void refreshPrices\(\{ automatic: true, trigger: 'daily timer' \}\),\s*\)/.exec(ext.text)?.[0] ?? '';
