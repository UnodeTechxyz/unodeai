...
class WorkerPdfExtractor implements PdfExtractor {
  async extract(path: string, range: { start: number; end: number }): Promise<{ totalPages: number; pages: PdfPageExtraction[] }> {
    ...
    const workerPath = [sibling, bundled, join(resolve(__dirname, '..', '..'), 'out', 'content', 'PdfWorker.js')].find(existsSync);
    ...
    return new Promise((resolve, reject) => {
      let settled = false;
      const settle = async <T>(complete: () => T, terminate = false): Promise<void> => {
        if (settled) return;
        settled = true;
        clearTimeout(timer);
        // Successful and refused parses close their one-shot worker port and exit naturally. Waiting for
        // that exit makes teardown deterministic and ensures its isolated module state is gone before the
        // next parse starts. Only a timed-out worker is forcibly terminated.
        if (terminate) {
          await worker.terminate().catch(() => undefined);
        } else if (worker.threadId !== -1) {
          await new Promise<void>((exited) => worker.once('exit', () => exited()));
        }
        complete();
      };
      const timer = setTimeout(() => {
        void settle(() => reject(new Error('PDF parsing timed out.')), true);
      ...
      worker.once('exit', (code) => {
        if (!settled) void settle(() => reject(new Error(`PDF worker exited before replying (code ${code}).`)));
      });
...
class WorkerOfficeExtractor implements OfficeExtractor {
  async extract(path: string): Promise<{ format: 'docx' | 'pptx'; blocks: string[] }> {
    ...
    const workerPath = [sibling, bundled, join(resolve(__dirname, '..', '..'), 'out', 'content', 'OfficeWorker.js')].find(existsSync);
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        void settle(() => reject(new Error('Office document parsing timed out.')), true);
      }, CONTENT_ASSET_PARSE_TIMEOUT_MS);
      ...
      worker.once('error', (error) => { void settle(() => reject(error)); });
      worker.once('exit', (code) => {
        if (!settled) void settle(() => reject(new Error(`Office worker exited before replying (code ${code}).`)));
      });
