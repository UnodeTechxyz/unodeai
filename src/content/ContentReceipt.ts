...
export type LocalScopeContentReceiptAction = 'file-read' | 'directory-list' | 'tree-scan' | 'git-tag-inspection';
...
/**
 * A read outside the workspace that was admitted by unode.localReadScope. `rootId` is an opaque,
 * host-generated ordinal rather than a path; this receipt never carries a filename, query, or bytes.
 */
export interface LocalScopeContentReceiptObservation {
  contentClass: 'local-scope';
  action: LocalScopeContentReceiptAction;
  rootId: string;
  /** One file/directory/repository, or the number of files visited by a tree scan. */
  count: number;
}

export type ContentReceiptObservation =
  | PdfContentReceiptObservation
  | ImageContentReceiptObservation
  | ConversationContentReceiptObservation
  | LocalScopeContentReceiptObservation;
...
export function sanitizeContentReceipt(value: ContentReceiptObservation): ContentReceiptObservation | undefined {
  ...
  if (value.contentClass === 'local-scope') {
    if (!['file-read', 'directory-list', 'tree-scan', 'git-tag-inspection'].includes(value.action) ||
        !/^local-scope-[1-9]\d*$/.test(value.rootId) ||
        !Number.isInteger(value.count) || value.count < 0 || value.count > 1_000_000) {
      return undefined;
    }
    return {
      contentClass: 'local-scope',
