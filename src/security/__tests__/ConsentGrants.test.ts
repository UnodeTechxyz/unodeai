...
describe('ConsentGrantRegistry', () => {
  ...
  it('binds model egress to profile id, exact destination, and profile revision', () => {
    const grants = new ConsentGrantRegistry();
    expect(grants.grantModel(
      'custom:pay', 'https://gateway.example.test:443', 7, 'gateway.example.test', 'Payment agent',
      new Date('2026-09-21T12:00:00.000Z'),
    )).toBe(true);
    expect(grants.hasModel('custom:pay', 'https://gateway.example.test:443', 7)).toBe(true);
    expect(grants.hasModel('custom:pay', 'https://gateway.example.test:8443', 7)).toBe(false);
    expect(grants.hasModel('custom:pay', 'https://gateway.example.test:443', 8)).toBe(false);
    expect(grants.hasModelForHost('gateway.example.test')).toBe(true);
    expect(grants.list('model')).toEqual([expect.objectContaining({
      host: 'gateway.example.test',
      profileId: 'custom:pay',
      destination: 'https://gateway.example.test:443',
      profileRevision: 7,
    })]);
  });

  it('restores legacy host grants for display but never widens them into a scoped model grant', () => {
    const grants = new ConsentGrantRegistry();
    expect(grants.restore('model', ['gateway.example.test'])).toEqual({ migratedLegacy: true });
    expect(grants.hasModel('custom:pay', 'https://gateway.example.test:443', 1)).toBe(false);
    expect(grants.hasModelForHost('gateway.example.test')).toBe(true);
  });
