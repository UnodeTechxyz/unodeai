...
export interface SessionEventData {
  ...
  'session.tool': {
    ...
    detail?: string;
    diff?: string;
    failureKind?: 'blocked' | 'not_found' | 'error' | 'integration_error' | 'cancelled' | 'outcome_unknown';
    epoch: number;
...
export class SessionManager {
  ...
  /** Host-authored coordinator footer waiting to be posted beside, not inside, the model reply. */
  private pendingDelegationReceipts = new Map<string, string>();
  ...
  interrupt(sessionId: string, reason = USER_CANCELLED_TURN, forceTimeoutMs = INTERRUPT_FORCE_STOP_MS): void {
    const info = this.sessions.get(sessionId);
  ...
  private deliverTurn(sessionId: string, msg: Message): void {
    ...
    const projectContext = contextBundle?.projectContext ?? this.deps.getProjectContext?.() ?? '';
    let contextManifest = contextBundle?.contextManifest
      ?? this.deps.getTurnContextManifest?.(info.config, msg, projectContext);
  ...
  private onBackendEvent(info: SessionInfo, backend: AgentBackend, evt: BackendEvent): void {
    ...
    switch (evt.kind) {
      ...
      case 'delegation_receipt':
        info.lastActiveAt = new Date().toISOString();
        this.pendingDelegationReceipts.set(info.id, evt.text);
        this.fire('session.output', info.id, 'message', { stream: 'stdout', content: evt.text });
        break;
      ...
      case 'native_mcp_activity': {
        ...
        this.fire('session.tool', info.id, 'message', evt.status === 'inProgress'
          ? {
              phase: 'use', name, input: { status: evt.status, notice }, epoch,
              correlationId: this.currentTurnCorrelationId(info.id),
            }
          : {
              phase: 'result', name, ok: evt.status === 'completed',
              summary: `${evt.status}. ${notice}`, detail: notice,
              ...(evt.status === 'failed' ? { failureKind: 'error' as const } : {}),
      ...
      case 'host_wait':
        this.publishDelegatedProgress(
          info,
          evt.phase === 'started' ? 'Waiting on your approval.' : 'Approval wait ended.',
          epoch,
          false,
          evt.phase === 'started' ? 'host-wait' : 'host-wait-ended',
        );
        break;
      ...
      case 'turn_complete': {
        ...
        if (delegationReceipt) {
          // A separate host notice keeps receipt-published assistant content byte-exact while making the
          // per-turn dispatch truth visible in the same agent chat.
          this.bus.send(
            info.id,
            'user',
            'ask.answer',
            { instruction: delegationReceipt, metadata: { hostDelegationReceipt: true, turnEpoch: epoch } },
            'normal',
          );
        }
