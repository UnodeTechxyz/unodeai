...
export interface ContextManifestSource {
  ...
  /** Selected-row counts for this exact shared-memory text snapshot. Never includes note prose. */
  memoryTrustCounts?: SharedMemoryTrustCounts;
}
...
export interface TurnContextManifestEntry {
  ...
  /** Present only for a shared-memory entry and bounded by the prompt row budget. */
  memoryTrustCounts?: SharedMemoryTrustCounts;
}
...
export function createTurnContextManifest(sources: readonly (ContextManifestSource | undefined)[]): TurnContextManifest {
  const entries: TurnContextManifestEntry[] = sources.filter((source): source is ContextManifestSource => !!source).map((source): TurnContextManifestEntry => {
    ...
    const memoryTrustCounts = source.kind === 'shared-memory'
      ? validMemoryTrustCounts(source.memoryTrustCounts)
      : undefined;
    return {
      kind: source.kind,
      label: source.label,
      ...
      ...(memoryTrustCounts ? { memoryTrustCounts } : {}),
    };
  });
  return {
    schemaVersion: 1,
...
function validMemoryTrustCounts(value: SharedMemoryTrustCounts | undefined): SharedMemoryTrustCounts | undefined {
  if (!value
    || !Number.isSafeInteger(value.untrusted)
    || !Number.isSafeInteger(value.humanAttested)
    || value.untrusted < 0
    || value.humanAttested < 0
    || value.untrusted + value.humanAttested > 30) {
    return undefined;
  }
  return { untrusted: value.untrusted, humanAttested: value.humanAttested };
}
