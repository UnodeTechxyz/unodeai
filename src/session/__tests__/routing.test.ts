...
import { memoryFilePath, memoryRowDigest, SharedMemory } from '../SharedMemory';
import { MemoryAttestationStore } from '../MemoryAttestationStore';
...
describe('SessionManager <-> MessageBus routing', () => {
  ...
  it('posts a delegation receipt as a separate host-owned chat item without changing the reply', async () => {
    mgr.create(makeConfig('pm', 'pm'));
    await mgr.start('pm');
    backends.get('pm')!.emit({ kind: 'ready' });
    bus.send('user', 'pm', 'task.assign', { instruction: 'Coordinate this work.' });

    backends.get('pm')!.emit({
      kind: 'delegation_receipt',
      text: 'Delegations this turn: 0 accepted · 2 refused.',
    });
    backends.get('pm')!.emit({
      kind: 'turn_complete',
      result: { text: 'Both agents were dispatched.', isError: false },
    });

    expect(bus.query({ type: 'task.complete' }).at(-1)?.payload.instruction).toBe('Both agents were dispatched.');
    expect(bus.query({ type: 'ask.answer' }).at(-1)).toMatchObject({
      from: 'pm',
      to: 'user',
      payload: {
        instruction: 'Delegations this turn: 0 accepted · 2 refused.',
        metadata: { hostDelegationReceipt: true, turnEpoch: 1 },
      },
    });
  });
  ...
  it('shows Codex-native MCP activity but never counts it as UnodeAi-observed evidence', async () => {
    const progress: Array<{ toolCalls: number; outcome: string }> = [];
    const tools: any[] = [];
    mgr.on('session.taskProgress', (event) => progress.push({
      toolCalls: event.data.progress.toolCalls,
      outcome: event.data.progress.outcome,
    ...
    await mgr.start('dev');
    backends.get('dev')!.emit({ kind: 'ready' });
    bus.send('pm', 'dev', 'task.assign', { instruction: 'Use native docs.' }, 'normal', 'native-mcp-progress');
    ...
    expect(tools[0].name).toContain('Codex-native MCP');
    expect(tools[1].summary).toContain('not approved or mediated by UnodeAi');
    expect(progress).toEqual([{ toolCalls: 0, outcome: 'no-framework-evidence' }]);
  });
  ...
  it('reports one safe Activity action while retaining a failed tool result in the session transcript (C3/C4)', async () => {
    ...
    backends.get('dev')!.emit({
      kind: 'tool_use',
      name: 'mcp__unode_files_00112233445566778899aabbccddeeff__read_file',
    ...
    expect(toolEvents).toEqual([
      expect.objectContaining({ phase: 'use', name: 'mcp__unode_files_00112233445566778899aabbccddeeff__read_file' }),
      expect.objectContaining({ phase: 'result', name: 'mcp__unode_files_00112233445566778899aabbccddeeff__read_file', ok: false, summary: 'document parser failed' }),
    ]);
  ...
  it('shows only reviewed structural targets and never persists command arguments or arbitrary MCP input (C3/C4j)', async () => {
    ...
    const activity = bus.query({ type: 'task.status' });
  ...
  it('routes a safety stop as typed partial work, never task.complete or worker failure', async () => {
    mgr.create(makeConfig('dev', 'senior-dev'));
    await mgr.start('dev');
    backends.get('dev')!.emit({ kind: 'ready' });

    bus.send('pm', 'dev', 'task.assign', { instruction: 'Review the full specification.' }, 'normal', 'stopped-handle');
    backends.get('dev')!.emit({
      kind: 'turn_complete',
      result: {
        text: 'Stopped: step safety limit after 100 steps.\n\nFAIL — two checks remain.',
        isError: false,
        stop: { state: 'stopped', reason: 'step-safety-limit', steps: 100, lastActivity: 'tool search_files completed' },
      },
    });

    expect(bus.query({ type: 'task.complete', correlationId: 'stopped-handle' })).toEqual([]);
    expect(bus.query({ type: 'system.error', correlationId: 'stopped-handle' })).toEqual([]);
    expect(bus.query({ type: 'task.partial', correlationId: 'stopped-handle' }).at(-1)?.payload.metadata).toMatchObject({
      completionState: 'partial',
      stopped: true,
      stopReason: 'step-safety-limit',
      stop: { state: 'stopped', steps: 100 },
      midPlan: true,
    });
  });
  ...
  it('uses one atomic context bundle so mutable memory cannot diverge from its receipt mid-turn', async () => {
    const localBus = new MessageBus();
    const localBackends = new Map<string, FakeBackend>();
    let bundleCalls = 0;
    let memoryVersion = 1;
    const localMgr = new SessionManager(5, localBus, {
      ...
      resolveEnv: async () => ({}),
      getProjectContext: () => { throw new Error('atomic bundle must win'); },
      getTurnContextManifest: () => { throw new Error('atomic bundle must win'); },
      getTurnContextBundle: () => {
        bundleCalls += 1;
        const memoryBlock = `<shared_memory><note>snapshot-${memoryVersion}</note></shared_memory>`;
        const contextManifest = createTurnContextManifest([{
          kind: 'shared-memory', label: 'Shared team memory', location: '.unode/memory/notes.md',
          text: memoryBlock, reason: 'same snapshot',
          memoryTrustCounts: { untrusted: 1, humanAttested: 0 },
        }]);
        memoryVersion += 1; // models a memory_note write or watcher reload after selection
        return { projectContext: memoryBlock, contextManifest };
      },
    });
    localMgr.create(makeConfig('dev', 'senior-dev'));
    await localMgr.start('dev');
    ...
    localBus.send('user', 'dev', 'ask.question', { instruction: 'Use the supplied memory.' });

    const attachments = localBackends.get('dev')!.attachments[0];
    expect(bundleCalls).toBe(1);
    expect(attachments?.projectContext).toContain('snapshot-1');
    expect(attachments?.projectContext).not.toContain('snapshot-2');
    expect(attachments?.contextManifest?.entries[0]).toMatchObject({
      bytes: Buffer.byteLength(attachments?.projectContext ?? '', 'utf8'),
      memoryTrustCounts: { untrusted: 1, humanAttested: 0 },
    });
    localMgr.dispose();
    localBus.dispose();
  ...
  it('propagates agent A memory to agent B as untrusted, then exact-row attested, revoked, and edit-invalidated', async () => {
    let content = '';
    const memory = new SharedMemory(
      ...
      async () => undefined,
      async (root, relative) => path.resolve(root, relative)
    );
    ...
    await memory.append('agent-a', 'Shared boundary A', 'standard', 'contract');
    await memory.load();
    const exactRow = content.trimEnd();
    const digest = memoryRowDigest(exactRow);
    ...
    const localMgr = new SessionManager(5, localBus, {
      ...
      resolveEnv: async () => ({}),
      getTurnContextBundle: () => {
        const snapshot = memory.promptSnapshot(attestations.attestedDigests());
        return {
          projectContext: snapshot.block,
          contextManifest: createTurnContextManifest([{
            kind: 'shared-memory', label: 'Shared team memory', location: '.unode/memory/notes.md',
            text: snapshot.block, reason: 'same selected snapshot', memoryTrustCounts: snapshot.trustCounts,
          }]),
        };
      },
    });
    localMgr.create(makeConfig('agent-b', 'senior-dev'));
    const authorityBefore = {
      allowedTools: [...localMgr.get('agent-b')!.config.allowedTools],
      autoApprove: localMgr.get('agent-b')!.config.autoApprove,
      folderAccess: localMgr.get('agent-b')!.config.folderAccess,
    };
    await localMgr.start('agent-b');
    const backend = localBackends.get('agent-b')!;
    backend.emit({ kind: 'ready' });

    const takeTurn = (instruction: string) => {
      localBus.send('user', 'agent-b', 'ask.question', { instruction });
      const attachments = backend.attachments.at(-1)!;
      backend.emit({ kind: 'turn_complete', result: { text: 'observed', isError: false } });
      return attachments;
    };

    const initial = takeTurn('Read team context.');
    expect(initial.projectContext).toContain('trust="untrusted"');
    expect(initial.contextManifest?.entries[0].memoryTrustCounts).toEqual({ untrusted: 1, humanAttested: 0 });

    await attestations.attest(digest);
    const attested = takeTurn('Read it again.');
    expect(attested.projectContext).toContain('trust="human-attested"');
    expect(attested.contextManifest?.entries[0].memoryTrustCounts).toEqual({ untrusted: 0, humanAttested: 1 });

    await attestations.revoke(digest);
    expect(takeTurn('After revocation.').projectContext).toContain('trust="untrusted"');
    await attestations.attest(digest);
    content = content.replace('Shared boundary A', 'Shared boundary B');
    await memory.load();
    expect(takeTurn('After edit.').projectContext).toContain('trust="untrusted"');
    expect({
      allowedTools: localMgr.get('agent-b')!.config.allowedTools,
      autoApprove: localMgr.get('agent-b')!.config.autoApprove,
      folderAccess: localMgr.get('agent-b')!.config.folderAccess,
...
describe('SessionManager workspace invalidation', () => {
  it('refuses both a new start and a later turn after the first-folder binding becomes unavailable', async () => {
    let available = false;
    const bus = new MessageBus();
    const backends = new Map<string, FakeBackend>();
    const manager = new SessionManager(5, bus, {
      createBackend: (config) => {
        const backend = new FakeBackend(config);
        backends.set(config.id, backend);
        return backend;
      ...
      workspaceUnavailableReason: () => 'The first workspace folder changed; reload this window.',
    });
    manager.create(makeConfig('dev', 'senior-dev'));

    await expect(manager.start('dev')).rejects.toThrow('first workspace folder changed');
    ...
    backends.get('dev')!.emit({ kind: 'ready' });
    available = false;
    bus.send('user', 'dev', 'task.assign', { instruction: 'write in the old folder' });

    expect(backends.get('dev')!.turns).toEqual([]);
    expect(bus.query({ type: 'system.error' }).at(-1)?.payload.instruction).toContain('first workspace folder changed');
  });
});
...
describe('SessionManager model fallback (P1#6)', () => {
  ...
  it('leaves the fallback failure counter unchanged for a typed stopped turn', async () => {
    const { bus, backends, mgr } = setup();
    const cfg = makeConfig('dev', 'senior-dev');
    cfg.model = 'primary-x';
    cfg.fallbackModel = 'backup-y';
    ...
    const turn = (result: { isError: boolean; stop?: { state: 'stopped'; reason: 'anti-spin'; steps: number; lastActivity: string } }) => {
      bus.send('user', 'dev', 'task.assign', { instruction: 'go' });
      backends.get('dev')!.emit({ kind: 'turn_complete', result: { text: '', ...result } });
    };
    turn({ isError: true }); // failure streak = 1
    turn({ isError: false, stop: { state: 'stopped', reason: 'anti-spin', steps: 4, lastActivity: 'checking' } });
    expect(mgr.get('dev')!.config.model).toBe('primary-x'); // stop did not increment to 2
    turn({ isError: true });
    expect(mgr.get('dev')!.config.model).toBe('backup-y'); // stop also did not reset the existing streak
  });
