...
describe('SessionManager egress-consent start lifecycle (B6)', () => {
  ...
  it('terminates the queued chat turn with the refusal reason and starts the next message as a fresh turn', async () => {
    const bus = new MessageBus();
    const backends: ConsentBackend[] = [];
    const manager = new SessionManager(5, bus, {
      createBackend: (agent) => {
        const backend = new ConsentBackend(agent.id);
    ...
    manager.create(config('consent'));
    await manager.start('consent');
    const first = bus.send('user', 'consent', 'ask.question', { instruction: 'first request' });

    backends[0].deny();
    await vi.waitFor(() => expect(manager.get('consent')?.status).toBe('error'));
    const refusal = bus.query({ type: 'system.error' }).find((message) => message.correlationId === first.id);
    expect(refusal?.payload).toMatchObject({
      instruction: expect.stringContaining('declined'),
      metadata: { isError: true, startRefused: true },
    });

    bus.send('user', 'consent', 'ask.question', { instruction: 'second request' });
    await vi.waitFor(() => expect(backends).toHaveLength(2));
    expect(manager.get('consent')?.status).toBe('consent_required');
  });

  it('Stop finishes immediately while consent is pending', async () => {
    const { manager } = setup();
    manager.create(config('consent'));
    await manager.start('consent');

    await manager.stop('consent');

    expect(manager.get('consent')?.status).toBe('stopped');
    expect(manager.isRunning('consent')).toBe(false);
  });
  ...
  it('waits for managed-integration preparation before constructing the backend', async () => {
    let release!: () => void;
    const preparing = new Promise<void>((resolve) => { release = resolve; });
    const createBackend = vi.fn((agent: AgentConfig) => new ReadyBackend(agent.id));
    const manager = new SessionManager(5, new MessageBus(), {
      createBackend,
      resolveEnv: async () => ({}),
      prepareBackendStart: () => preparing,
    });
    manager.create(config('ready'));

    const start = manager.start('ready');
    await Promise.resolve();
    expect(manager.get('ready')?.status).toBe('starting');
