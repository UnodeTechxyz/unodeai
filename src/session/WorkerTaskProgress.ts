...
export interface WorkerTaskProgressRecord {
  ...
  terminalState: 'completed' | 'stopped' | 'error-or-unresolved';
}
...
export class WorkerTaskProgressTracker {
  ...
  finish(sessionId: string, result: TurnResult): WorkerTaskProgressRecord | undefined {
    ...
    const terminalState = result.stop?.state === 'stopped'
      ? 'stopped'
      : result.isError || result.unresolvedReason ? 'error-or-unresolved' : 'completed';
