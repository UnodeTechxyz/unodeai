...
describe('custom secret confirmation', () => {
  it('does not claim that an arbitrary secret refreshed provider prices or models', async () => {
    ...
    // v0.9.79: the outcome of storing a key is a centred modal, not a corner notification.
    expect(showInformationMessage).toHaveBeenCalledWith('Stored GITHUB_TOKEN in SecretStorage.', { modal: true });
  });
});
