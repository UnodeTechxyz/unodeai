...
import { ProjectCommandApprovalWriter, promptCommandApproval } from './backend/CommandApprovalPrompter';
...
import { describeTeamEntry, type TeamLibraryEntry, type TeamLibraryRef } from './state/TeamLibrary';
...
export interface DialogDeps {
  ...
  projectCommandApprovals: ProjectCommandApprovalWriter;
  output: vscode.LogOutputChannel;
  /** `scope` narrows the refresh to one provider's own price source — picker paths must pass it. */
  refreshPrices: (opts?: { scope?: string; trigger?: 'Add Agent model picker' }) => Promise<void> | void;
  /**
  ...
  /** Saved-team rows shown in the unified Create or Load picker. */
  listSavedTeams?: () => Promise<readonly TeamLibraryEntry[]>;
  /** Reuse the host's guarded saved-team open path for one selected document. */
  openSavedTeam?: (ref: TeamLibraryRef) => Promise<AgentConfig[]>;
  /** Reuse the host's existing save flow from the unified picker. */
  saveCurrentTeam?: () => Promise<void>;
  /** Open the automatic-snapshot browser without exposing every snapshot in the main picker. */
  restorePreviousTeam?: () => Promise<AgentConfig[]>;
  /** Validate the configured creation route only after the user chooses a system preset. */
  assertCanCreateTeam?: () => void;
}
 ...
 */
export function adaptGeneratedConfigToConnection(d: DialogDeps, config: AgentConfig): string[] {
  ...
  const compatibleTools = beforeTools === undefined
    ? undefined
    : supportedAllowedTools(profile.capabilities, beforeTools);
  const removedTools = beforeTools?.filter((tool) => !compatibleTools?.includes(tool)) ?? [];
  // Undefined means "no per-agent ceiling". Persisting [] here turned an unrestricted generated route into
  // an explicit no-tools roster entry, so preserve absence while still narrowing a ceiling the user supplied.
  config.allowedTools = compatibleTools;
  if (profile.backendKind === 'codex') {
    config.toolCeiling = !compatibleTools || compatibleTools.length === 0 ? 'native-default' : 'bounded';
  } else {
  ...
  if (!profile.capabilities.skills && (config.skills?.length ?? 0) > 0) {
    config.skills = [];
    removed.push('capability Skill grants');
  }
  if (!profile.capabilities.playbooks && (config.playbooks?.length ?? 0) > 0) {
    config.playbooks = [];
    removed.push('Playbook grants');
  }
...
function applyPickedContextWindow(config: AgentConfig, picked: PickedModel, providerName: string): void {
  ...
  if (config.contextWindowTokens !== undefined) {
    if (measurement && measurement.tokens !== config.contextWindowTokens) {
      void showResultNotice('warning', `Kept your explicit ${config.contextWindowTokens.toLocaleString()}-token context window; `
  ...
  if (measurement?.model === config.model) {
    ...
    void showResultNotice('information', `Using the ${measurement.tokens.toLocaleString()}-token context window advertised by ${providerName} via ${measurement.field}.`);
    return;
  }
  void showResultNotice('information', `${providerName} did not advertise a context window for ${config.model}; using the assumed 1,048,576-token default.`);
}
...
export async function pickModel(
  ...
  void d.refreshPrices({ scope: providerKey, trigger: 'Add Agent model picker' });
...
export async function instantiateTeam(
  ...
  if (roleKeys.includes('pm') && !canCoordinateTeam(providerKey, d.connectionResolver ?? BUILTIN_CONNECTION_REGISTRY)) {
    await showResultNotice('warning', 'The selected connection cannot run a Project Manager or delegate tasks. Choose a coordinator-capable connection to create this team.');
    return [];
  }
  ...
  if (!d.suppressInteractivePostCreatePrompts) {
    ...
    if (secretName && !(await d.secrets.has(secretName))) {
      ...
      showResultNotice('information', `Team created: ${label}. Send the PM a task to begin.`);
    }
    // F2: after creating a team, proactively suggest enabling commands.
    const accepted = await promptCommandApproval(d.commandPolicy.approvalMode, d.projectCommandApprovals);
    if (accepted) {
      const cfg = vscode.workspace.getConfiguration('unode');
      d.commandPolicy.reload(
        readUnodeSetting(cfg, 'commandApproval', 'ask'),
        readUnodeSetting(cfg, 'allowedCommands', [])
      );
    }
  }
  return created;
...
export function teamPresetItems(): vscode.QuickPickItem[] {
  ...
  const sortedPresets = (kind: NonNullable<TeamPreset['kind']>) => presets
    .filter((preset) => (preset.kind ?? 'knowledge') === kind)
    .sort((left, right) => byDisplayName(left.label, right.label))
    .map(fromPreset);
  return [
...
async function maybeOfferVerifyCommand(pick: TeamPresetItem): Promise<void> {
  ...
  const cfg = vscode.workspace.getConfiguration('unode');
  const current = readUnodeSetting(cfg, 'verifyCommand', '').trim();
  if (current === command) {
    // Already configured — confirm it so the user knows the gate is wired for this crew (no silent no-op).
    void showResultNotice('information', `${pick.teamLabel}: verification gate is set to "${command}". ✓`);
    return;
  }
  ...
  if (!current) {
    ...
    if (choice === 'Use Verify Command') {
      await cfg.update('verifyCommand', command, vscode.ConfigurationTarget.Global);
      void showResultNotice('information', `unode.verifyCommand set to "${command}" for ${pick.teamLabel}.`);
    }
    return;
  }
  const choice = await vscode.window.showWarningMessage(
...
export type TeamActionItem = vscode.QuickPickItem & (
  { action: 'preset'; preset: TeamPresetItem }
  | { action: 'saved'; ref: TeamLibraryRef }
  | { action: 'save' }
  | { action: 'restore' }
);

/** Only manually saved teams can be deleted from the unified picker; defaults and recovery snapshots cannot. */
export function teamActionHasDeleteControl(item: TeamActionItem): boolean {
  return item.action === 'saved';
}

export function teamActionItems(
  currentCount: number,
  savedTeams: readonly TeamLibraryEntry[] = [],
): (TeamActionItem | vscode.QuickPickItem)[] {
  const presets = teamPresetItems()
    .filter(isTeamPresetItem)
    .sort((left, right) => byDisplayName(left.teamLabel, right.teamLabel));
  const manual = savedTeams
    .filter((entry) => !entry.automatic)
    .sort((left, right) => byDisplayName(left.label, right.label) || left.scope.localeCompare(right.scope));
  const items: (TeamActionItem | vscode.QuickPickItem)[] = [
    { label: 'System defaults', kind: vscode.QuickPickItemKind.Separator },
    ...presets.map((preset): TeamActionItem => ({
      ...preset,
      description: currentCount > 0 ? `Replace ${currentCount} current agent(s)` : 'Create this team',
      detail: currentCount > 0 ? `Replaces your active roster with ${preset.teamLabel}.` : `Creates ${preset.teamLabel}.`,
      action: 'preset',
      preset,
    })),
  ];
  // This section is present even before the first save, so the primary team picker is the one place a
  // person needs to remember for saving, loading, and deleting a team.
  {
    items.push({ label: 'Saved teams', kind: vscode.QuickPickItemKind.Separator });
    items.push({
      label: '$(save) Save current team...',
      description: 'Store the active roster under a name you choose',
      detail: currentCount > 0 ? `Save the current ${currentCount}-agent team.` : 'Create a team first, then save it here.',
      action: 'save',
    });
    items.push(...manual.map((entry): TeamActionItem => ({
      label: `$(organization) ${entry.label}`,
      description: entry.scope === 'workspace' ? 'This project (in the repository)' : 'All projects',
      detail: describeTeamEntry(entry),
      action: 'saved',
      ref: { scope: entry.scope, slug: entry.slug },
    })));
    if (savedTeams.some((entry) => entry.automatic)) {
      items.push({
        label: '$(history) Restore a previous team...',
        description: 'Browse automatic snapshots',
        detail: 'Teams saved automatically before the active roster was replaced.',
        action: 'restore',
      });
    }
  }
...
export async function createTeamFromPreset(d: DialogDeps, selectedAction?: TeamActionItem): Promise<AgentConfig[]> {
  const currentCount = d.sessionManager.getAll().length;
  const isTeamAction = (item: TeamActionItem | vscode.QuickPickItem): item is TeamActionItem => 'action' in item;
  const action = selectedAction ?? await (async (): Promise<TeamActionItem | undefined> => {
    const savedTeams = await d.listSavedTeams?.() ?? [];
    const actions = teamActionItems(currentCount, savedTeams);
    const selected = await vscode.window.showQuickPick<TeamActionItem | vscode.QuickPickItem>(actions, {
      title: 'Create/Save/Load a Team',
      placeHolder: 'Choose a system default or one of your saved teams',
      matchOnDescription: true,
      matchOnDetail: true,
    });
    return selected && isTeamAction(selected) ? selected : undefined;
  })();
  if (!action) { return []; }

  if (action.action === 'saved') {
    return d.openSavedTeam?.(action.ref) ?? [];
  }
  if (action.action === 'save') {
    await d.saveCurrentTeam?.();
    return [];
  }
  if (action.action === 'restore') {
    return d.restorePreviousTeam?.() ?? [];
  }

  // Validate the configured creation route after selection but before confirmation or roster mutation.
  d.assertCanCreateTeam?.();

  if (currentCount > 0) {
    const choice = await vscode.window.showWarningMessage(
      `This replaces your current ${currentCount} agent(s) only if you choose Replace. How should "${action.preset.teamLabel}" be applied?`,
      { modal: true },
      'Keep current team',
      'Add to current team',
      'Replace current team',
    );
    if (choice !== 'Add to current team' && choice !== 'Replace current team') { return []; }
    if (choice === 'Replace current team') {
...
export async function createSoloAgent(d: DialogDeps): Promise<AgentConfig | undefined> {
  ...
  const rejected = assertCreatableConfig(d, config);
  if (rejected) {
    await showResultNotice('warning', rejected);
    return undefined;
  }
  d.sessionManager.create(config);
  d.output.info(`Solo agent created: ${config.name} (model ${config.model}).`);
  if (removed.length > 0) {
    void showResultNotice('information', `${config.name} uses a read-only connection. ${removed.join(', ')} will not be available.`);
  }
...
export type AddAgentRoleItem = vscode.QuickPickItem & { roleKey: keyof typeof ROLE_TEMPLATES };

export function addAgentRoleItems(): AddAgentRoleItem[] {
  const roleKeys = (Object.keys(ROLE_TEMPLATES) as (keyof typeof ROLE_TEMPLATES)[])
    .sort((left, right) => byDisplayName(ROLE_TEMPLATES[left].name, ROLE_TEMPLATES[right].name));
...
export async function showAddAgentDialog(d: DialogDeps): Promise<AgentConfig | undefined> {
  ...
  if (secretName && !(await d.secrets.has(secretName))) {
    ...
    showResultNotice('information', `Agent "${config.name}" added to your team`);
  }
  return config;
}
...
export async function showEditAgentDialog(d: DialogDeps, agentId: string): Promise<void> {
  ...
  if (!info) {
    showResultNotice('warning', 'Agent not found.');
    return;
  }
  ...
  showResultNotice('information', `Updated ${cfg.name}.`);
}
...
export async function showRunWorkflowDialog(d: DialogDeps): Promise<void> {
  ...
  showResultNotice('information', `Workflow "${pick.label}" is running`);
}
...
export async function showSetApiKeyDialog(d: DialogDeps): Promise<void> {
  ...
  if (await d.secrets.has(secretName)) {
    ...
    if (action === CLEAR) {
      ...
      showResultNotice('information', `${secretName} removed from this machine. Agents on that connection will ask for it again.`);
      return;
    }
  }
  ...
  showResultNotice('information', connectionId
...
export function parsePriceCoefficient(input: string): PriceCoefficientInput {
  ...
  if (!Number.isFinite(n) || n < 0) {
    return { ok: false, reason: 'Enter a number between 0 and 1, e.g. 1 for full price, 0.33 for a third, or 0 if the key is free.' };
  }
  if (n > 1) {
    return { ok: false, reason: 'This key cannot pay more than the full published price, so the number cannot be above 1.' };
  }
...
export async function promptForKeyPriceMultiplier(
  ...
  const raw = await vscode.window.showInputBox({
    ...
    prompt: 'Does this key pay the full published price, or a discounted rate? '
      + 'Enter the fraction you pay: 1 = full price, 0.33 = a third, 0 = free. '
      + 'Press Esc or leave blank to accept 1. '
      + 'Cost estimates use this number, and the gateway does not report it — so only you can say.',
    ignoreFocusOut: true,
    validateInput: (input) => {
  ...
  const cfg = vscode.workspace.getConfiguration('unode');
  const current = readUnodeSetting(cfg, 'priceMultiplier', {} as unknown);
