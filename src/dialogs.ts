...
async function maybeOfferVerifyCommand(pick: TeamPresetItem): Promise<void> {
  ...
  if (!current) {
    ...
    if (choice === 'Use Verify Command') {
      ...
      void vscode.window.showInformationMessage(`unode.verifyCommand set to "${command}" for ${pick.teamLabel}.`, { modal: true });
    }
    return;
  }
  const choice = await vscode.window.showWarningMessage(
...
export async function createSoloAgent(d: DialogDeps): Promise<AgentConfig | undefined> {
  ...
  const rejected = assertCreatableConfig(d, config);
  if (rejected) {
    await vscode.window.showWarningMessage(rejected, { modal: true });
    return undefined;
  }
  d.sessionManager.create(config);
  ...
  if (removed.length > 0) {
    void vscode.window.showInformationMessage(
      `${config.name} uses a read-only connection. ${removed.join(', ')} will not be available.`, { modal: true }
    );
  }
...
export async function showAddAgentDialog(d: DialogDeps): Promise<AgentConfig | undefined> {
  ...
  if (secretName && !(await d.secrets.has(secretName))) {
    ...
    vscode.window.showInformationMessage(`Agent "${config.name}" added to your team`, { modal: true });
  }
  return config;
}
...
export async function showEditAgentDialog(d: DialogDeps, agentId: string): Promise<void> {
  ...
  vscode.window.showInformationMessage(`Updated ${cfg.name}.`, { modal: true });
}
...
export async function showSetApiKeyDialog(d: DialogDeps): Promise<void> {
  ...
  if (await d.secrets.has(secretName)) {
    ...
    if (action === CLEAR) {
      ...
      vscode.window.showInformationMessage(`${secretName} removed from this machine. Agents on that connection will ask for it again.`, { modal: true });
      return;
    }
  }
  ...
  vscode.window.showInformationMessage(connectionId
    ...
    : `Stored ${secretName} in SecretStorage.`, { modal: true });
}
