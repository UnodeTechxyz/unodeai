...
export interface TeamToolset {
  ...
  /** Host-authored per-turn dispatch receipt; present even when the count is zero. */
...
export class TeamMcpBridge implements McpClient {
  ...
  async callTool(name: string, args: Record<string, unknown>, _signal?: AbortSignal): Promise<string> {
  ...
  turnDelegationFooter(): string | undefined {
    return this.team.turnDelegationFooter?.();
  }
