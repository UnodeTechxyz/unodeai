...
import approvalPolicy from './approvalPolicy.json';

const APPROVAL_TRANSPORTS = new Set<string>(approvalPolicy.approvalTransports);
...
export function shouldRequireApproval(cfg: MCPServerConfig): boolean {
  const sensitive =
    APPROVAL_TRANSPORTS.has(cfg.transport) ||
    (approvalPolicy.environmentRequiresApproval && Object.keys(cfg.env ?? {}).length > 0);
  if (sensitive) {
