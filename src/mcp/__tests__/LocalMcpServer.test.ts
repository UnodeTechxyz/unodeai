...
describe('LocalMcpServer', () => {
  ...
  it('forwards a Claude MCP cancellation notification to the active bridge handler', async () => {
    const server = createLocalMcpServer();
    let started!: () => void;
    ...
    const call = rpc(server, {
      jsonrpc: '2.0', id: 'slow-call', method: 'tools/call',
      params: { name: 'slow', arguments: {} },
    });
    await didStart;
    const cancelled = await rpc(server, {
      jsonrpc: '2.0', method: 'notifications/cancelled', params: { requestId: 'slow-call', reason: 'interrupted' },
    });
    ...
    await expect(call).resolves.toMatchObject({
      status: 200,
      body: { error: { message: 'cancelled upstream' } },
    });
  });

  it('aborts active bridge handlers when the local server stops', async () => {
    const server = createLocalMcpServer();
    let started!: () => void;
    const didStart = new Promise<void>((resolve) => { started = resolve; });
    let upstreamAborted = false;
    ...
    servers.push(server);
    const call = rpc(server, {
      jsonrpc: '2.0', id: 'slow-call', method: 'tools/call', params: { name: 'slow', arguments: {} },
    }).catch(() => undefined);
    await didStart;
