...
/**
 * Remove grants a connection cannot load before an Agent Builder save is validated. The builder keeps an
 * existing agent's unsupported selections greyed out (so switching connections back and forth loses nothing);
 * rejecting them on save made such an agent impossible to save and impossible to fix. Dropping a grant only
 * ever narrows the agent, and the caller reports what was removed.
 */
export function dropUnsupportedGrants<T extends AgentGrantSelection>(
  selection: T,
  capabilities: Pick<RuntimeCapabilities, 'skills' | 'playbooks' | 'mcp'>,
): { selection: T; droppedSkills: boolean; droppedPlaybooks: boolean; droppedMcp: boolean } {
  const droppedSkills = !capabilities.skills && selection.skillIds.length > 0;
  const droppedPlaybooks = !capabilities.playbooks && selection.playbooks.length > 0;
  const droppedMcp = !capabilities.mcp && selection.mcpServers.length > 0;
  return {
    selection: {
      ...selection,
      ...(droppedSkills ? { skillIds: [] } : {}),
      ...(droppedPlaybooks ? { playbooks: [] } : {}),
      ...(droppedMcp ? { mcpServers: [] } : {}),
    },
    droppedSkills,
    droppedPlaybooks,
    droppedMcp,
...
export function capabilityViolations(
  ...
  const mcpViolation = (config.mcpServers?.length ?? 0) > 0 && !capabilities.mcp
    ...
    : [];
  const skillViolation = (config.skills?.length ?? 0) > 0 && !capabilities.skills
    ? [{ capability: 'skills', message: `${profile.presentation.displayName} does not support UnodeAi capability Skill grants on this route.` }]
    : [];
  const playbookViolation = (config.playbooks?.length ?? 0) > 0 && !capabilities.playbooks
    ? [{ capability: 'playbooks', message: `${profile.presentation.displayName} does not support UnodeAi Playbooks on this route.` }]
    : [];
