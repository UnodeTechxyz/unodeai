...
const OPENAI_COMPAT_CAPABILITIES: RuntimeCapabilities = Object.freeze({
  ...
  delegation: true, coordinator: true, mcp: true, playbooks: true, skills: true, folderAccess: true, toolProtocol: true, smartMode: true, worktrees: true, verification: true,
...
const CLAUDE_CAPABILITIES: RuntimeCapabilities = Object.freeze({
  plan: true, act: true, read: true, write: true, command: true, commandApproval: 'unode-mediated',
  delegation: true, coordinator: true, mcp: true, playbooks: true, skills: true, folderAccess: true, toolProtocol: false, smartMode: false, worktrees: true, verification: true,
  stop: true, steer: true, resume: true,
  modelParams: new Set(['reasoning_effort']),
});

/** Codex App Server route: effects are mediated one request at a time by UnodeAi's approval UI. */
const CODEX_APP_SERVER_CAPABILITIES: RuntimeCapabilities = Object.freeze({
  plan: true, act: true, read: true, write: true, command: true, commandApproval: 'unode-mediated',
  delegation: true, coordinator: true, mcp: true, playbooks: true, skills: true, folderAccess: true, toolProtocol: false, smartMode: false, worktrees: true, verification: true,
  stop: true, steer: true, resume: true,
  modelParams: new Set(['reasoning_effort']),
...
const CODEX_MODELS: readonly ConnectionModel[] = Object.freeze([
  { id: 'codex-cli-default', name: 'Codex CLI account default', vision: true },
]);
...
export const CONNECTION_PROFILES: readonly ConnectionProfile[] = Object.freeze([
  ...
  profile({ id: 'claude-cli', revision: 1, kind: 'claude-headless', availability: 'available', backendKind: 'claude', billingKind: 'cli-account', authKind: 'claude-cli', catalogKind: 'claude-cli', catalogModels: CLAUDE_MODELS, capabilities: CLAUDE_CAPABILITIES, presentation: cliPresentation({ displayName: 'Claude CLI', runtimeLabel: 'Claude CLI', billingLabel: 'Your Claude account (as configured in the CLI)', installCommand: 'npm install -g @anthropic-ai/claude-code', loginCommand: 'claude login', terminalName: 'UnodeAi — Claude CLI', privacySummary: 'Prompts and any included workspace content are sent through your logged-in Claude CLI.' }) }),
  profile({ id: 'codex-cli', revision: 2, kind: 'codex-headless', availability: 'available', backendKind: 'codex', billingKind: 'cli-account', authKind: 'codex-cli', catalogKind: 'codex-cli', catalogModels: CODEX_MODELS, capabilities: CODEX_APP_SERVER_CAPABILITIES, presentation: cliPresentation({ displayName: 'Codex CLI', runtimeLabel: 'Codex CLI', billingLabel: 'Your OpenAI account (as configured in the CLI)', installCommand: 'npm install -g @openai/codex', loginCommand: 'codex login', terminalName: 'UnodeAi — Codex CLI', requiredSetting: 'unode.codexCliPath', privacySummary: 'Runs your logged-in Codex CLI. Codex connects to OpenAI itself; UnodeAi does not control its network behaviour. Routine work follows the selected native Codex permission profile; only escalations Codex routes to the user receive a UnodeAi approval card.' }) }),
]);
...
export const CONNECTION_SETUP_GROUPS: readonly ConnectionSetupGroup[] = Object.freeze([
  ...
  { kind: 'claude-headless', displayName: 'Claude CLI', description: 'Use your logged-in Claude CLI subscription.' },
  { kind: 'codex-headless', displayName: 'Codex CLI', description: 'Use your logged-in Codex CLI subscription with a native Codex permission profile.' },
]);
