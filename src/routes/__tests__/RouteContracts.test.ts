...
describe('E0 route contracts', () => {
  ...
  it('keeps Codex as a new-agent default now that its consent-gated route is available', () => {
    expect(resolveAvailableDefaultProviderId('codex')).toEqual({ providerId: 'codex' });
  });
  ...
  it('derives every host-side capability rejection from the same profile row used for presentation', () => {
    for (const profile of CONNECTION_PROFILES) {
      ...
      expect(names.includes('playbooks')).toBe(!profile.capabilities.playbooks);
  ...
  it('admits the mediated Codex App Server route without an availability violation', () => {
    const codex = connectionProfile('codex-cli')!;
    expect(codex.availability).toBe('available');
    expect(manifestDefaultConnectionIds()).toContain('codex-cli');
    expect(capabilityViolations({
      provider: { providerId: 'codex' }, backend: 'codex', role: 'reviewer', allowedTools: ['read'],
    } as any)).not.toContainEqual(expect.objectContaining({ capability: 'availability' }));
  });

  it('admits Codex Playbooks and maps write independently of its always-sandboxed command tools', () => {
    const base = {
      provider: { providerId: 'codex' }, backend: 'codex', role: 'reviewer',
      playbooks: ['api-contract-review'], toolCeiling: 'bounded',
    } as any;
    expect(capabilityViolations({ ...base, allowedTools: ['read', 'search'] }))
      .not.toContainEqual(expect.objectContaining({ capability: 'playbooks' }));
    expect(capabilityViolations({ ...base, allowedTools: ['read', 'write'] })).toEqual([]);
    expect(capabilityViolations({ ...base, allowedTools: ['read', 'execute'] })).toEqual([]);
    expect(capabilityViolations({ ...base, allowedTools: ['read', 'write', 'execute'] })).toEqual([]);
  });
...
describe('Agent Builder save adaptation', () => {
  it('removes grants a connection cannot load instead of making the agent unsaveable', () => {
    // Field, v0.9.84: editing a Codex agent kept its playbooks greyed out, the save was rejected with
    // "does not support UnodeAi skill or playbook grants", and the user could neither save nor remove them.
    const codex = connectionProfile('codex-cli')!;
    const selection = { skillIds: ['file-io'], playbooks: ['tdd'], mcpServers: ['docs'], name: 'Codex Tester' };
    const adapted = dropUnsupportedGrants(selection, codex.capabilities);

    expect(adapted.droppedSkills).toBe(!codex.capabilities.skills);
    expect(adapted.droppedPlaybooks).toBe(!codex.capabilities.playbooks);
    expect(adapted.droppedMcp).toBe(!codex.capabilities.mcp);
    if (!codex.capabilities.skills) expect(adapted.selection.skillIds).toEqual([]);
    if (!codex.capabilities.playbooks) expect(adapted.selection.playbooks).toEqual([]);
    else expect(adapted.selection.playbooks).toEqual(['tdd']);
    if (!codex.capabilities.mcp) expect(adapted.selection.mcpServers).toEqual([]);
    expect(adapted.selection.name).toBe('Codex Tester');
    // What remains must pass the same check the save runs, so the save can succeed.
    const violations = capabilityViolations({
      provider: { providerId: 'codex' }, backend: 'codex', role: 'reviewer',
      skills: adapted.selection.skillIds.map((id) => ({ id })) as any,
      playbooks: adapted.selection.playbooks, mcpServers: adapted.selection.mcpServers,
    } as any);
    expect(violations.filter((v) => v.capability === 'skills' || v.capability === 'mcp')).toEqual([]);
  });

  it('keeps grants on a connection that loads them', () => {
    const unode = connectionProfile('unode')!;
    const selection = { skillIds: ['file-io'], playbooks: ['tdd'], mcpServers: ['docs'] };
    const adapted = dropUnsupportedGrants(selection, unode.capabilities);
    expect(adapted.droppedSkills).toBe(false);
    expect(adapted.droppedPlaybooks).toBe(false);
    expect(adapted.selection.playbooks).toEqual(['tdd']);
  });
});
