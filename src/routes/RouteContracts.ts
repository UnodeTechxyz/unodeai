...
type CapabilityBase = {
  ...
  coordinator: boolean;
  mcp: boolean;
  /** The route can load extension-owned instruction-only Playbooks. */
  playbooks: boolean;
  /** The route can represent at least some UnodeAi capability Skill packages. */
  skills: boolean;
...
export function assertAgentRoute(value: unknown): asserts value is AgentRoute {
  ...
  if (route.kind === 'claude-headless' && route.connectionId !== 'claude-cli') {
    throw new Error('Claude CLI must use the claude-cli connection.');
  }
  if (route.kind === 'codex-headless' && route.connectionId !== 'codex-cli') {
    throw new Error('Codex CLI must use the codex-cli connection.');
  }
