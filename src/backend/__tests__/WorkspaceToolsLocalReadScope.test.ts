...
describe('WorkspaceTools local read scope', () => {
  it('fails closed, suppresses an immediate retry, and re-asks on a new user request', async () => {
    ...
    tools.setLocalReadScopeAccess([parent], consent);
    ...
    const recovered = await tools.run('read_file', { path: sibling });
    ...
    expect(first.output).toContain('Grant folder access');
    expect(recovered).toMatchObject({ status: 'success' });
    expect(recovered.output).toContain('private sibling content');
    expect(prompts).toEqual([parent, parent]);
  });
