...
describe('TeamTools (PM coordinator)', () => {
  ...
  beforeEach(() => {
    ...
    bus = new MessageBus();
    team = new TeamTools('pm', view, bus, { timeoutMs: 1000, cwd: process.cwd(), taskInputResolver: taskInputResolver() });
  });

  it('publishes a turn-local delegation receipt for zero, refused, and accepted dispatches', async () => {
    team.beginTurnContentReceipts();
    expect(team.turnDelegationFooter()).toBe('Delegations this turn: 0 accepted · 0 refused.');

    await team.run('dispatch_task', {
      agent: 'missing-agent',
      instruction: 'Try the bounded handoff.',
      contract: taskContract(),
    });
    expect(team.turnDelegationFooter()).toBe('Delegations this turn: 0 accepted · 1 refused.');

    team.beginTurnContentReceipts();
    await team.run('dispatch_task', {
      agent: 'dev',
      instruction: 'Take the bounded handoff.',
      contract: taskContract(),
    });
    expect(team.turnDelegationFooter()).toBe('Delegations this turn: 1 accepted · 0 refused.');
  });

  it('reports a busy worker as pending and publishes a host correction after late admission', async () => {
    const updates = vi.fn();
    const waiting = new TeamTools('pm', view, bus, {
      timeoutMs: 1_000,
      cwd: process.cwd(),
      taskInputResolver: taskInputResolver(),
      waitForTaskAdmission: true,
      onTurnDelegationReceiptChanged: updates,
    });
    let handle = '';
    bus.onType('task.assign', (message) => { handle = message.correlationId ?? ''; });
    waiting.beginTurnContentReceipts();

    await waiting.run('assign_task_async', { agent: 'dev', instruction: 'Wait until the worker is available.' });
    expect(waiting.turnDelegationFooter()).toBe('Delegations this turn: 0 accepted · 0 refused · 1 pending.');

    bus.send('dev', 'pm', 'task.admitted', { instruction: 'started' }, 'normal', handle);
    expect(updates).toHaveBeenLastCalledWith(expect.objectContaining({
      accepted: 1,
      refused: 0,
      pending: 0,
      text: 'Delegations this turn: 1 accepted · 0 refused.',
    }));
  });
  ...
  it('does not offer a worker any dispatch or dispatch-status surface even when constructed with TeamTools', async () => {
    ...
    for (const name of ['dispatch_task', 'continue_task', 'assign_task_async', 'collect_ready_tasks', 'inspect_task_status']) {
  ...
  it('refuses an explicit shell task for a teammate whose connection has no shell capability', async () => {
    ...
    const detail = (capabilityTeam as any).delegationCapabilityMismatch('reviewer', {
      requiredCapabilities: { capabilities: ['shell'] },
    });
    expect(detail).toContain('resulting permissions include Run commands');
    expect(detail).not.toContain('resulting permissions include Read files');
  ...
  it('captures an explicit coordinator rejection, forwards its reason, and keeps acceptance counters honest', async () => {
    ...
    expect(metrics).toContain("A person's run verdict, when recorded, remains separate");
    expect(metrics).not.toContain('This release has no human acceptance layer');
  });
  ...
  it('keeps a host-sent rework live until its teammate replies', async () => {
    const replies: Array<{ handle: string; agentId: string }> = [];
    const ready: Array<{ handle: string; ref: string; text: string }> = [];
    const reworkTeam = new TeamTools('pm', view, bus, {
      timeoutMs: 1_000,
      evidenceEnabled: true,
      onDelegationReworkReply: (event) => replies.push(event),
      onAsyncResultReady: (result) => { ready.push(result); return true; },
    });
    bus.onType('task.assign', (message: Message) => {
    ...
    const first = await reworkTeam.run('assign_task', { agent: 'dev', instruction: 'Draft the release note.' });
    const handle = /Handle: ([^\s.]+)/.exec(first)?.[1]!;
    await reworkTeam.run('record_task_disposition', {
      handle, disposition: 'needs-rework', reason: 'State the upgrade path explicitly.',
    });

    expect(reworkTeam.coordinatorCloseoutState()).toMatchObject({ hasLiveDelegationWork: true });
    ...
    expect(reworkTeam.coordinatorCloseoutState()).toMatchObject({ hasLiveDelegationWork: false });
    expect(ready).toEqual([expect.objectContaining({ handle, ref: 'dev', text: expect.stringContaining('Reworked result.') })]);
    expect(reworkTeam.isAsyncResultReady(handle)).toBe(true);
    expect(replies).toEqual([]);
  });

  it('uses the rework-reply notice only when the async-result wake cannot be queued', async () => {
    const fallback: Array<{ handle: string; agentId: string }> = [];
    const reworkTeam = new TeamTools('pm', view, bus, {
      timeoutMs: 1_000,
      evidenceEnabled: true,
      onAsyncResultReady: () => false,
      onDelegationReworkReply: (event) => fallback.push(event),
    });
    bus.onType('task.assign', (message: Message) => {
      bus.send('dev', 'pm', 'task.complete', { instruction: 'Initial result.' }, 'normal', message.correlationId);
    });
    ...
    expect(fallback).toEqual([expect.objectContaining({ handle, agentId: 'dev' })]);
  });
  ...
  it('continues a safety-stopped task on the same worker conversation with fresh attempt authority', async () => {
    const assignments: Message[] = [];
    bus.onType('task.assign', (message) => assignments.push(message));
    const resumable = new TeamTools('pm', view, bus, {
      timeoutMs: 1000,
      cwd: process.cwd(),
      taskInputResolver: taskInputResolver(),
      evidenceEnabled: true,
    ...
    const dispatched = await resumable.run('dispatch_task', {
      agent: 'dev',
      instruction: 'Read the long specification and return PASS or FAIL.',
      contract: taskContract(),
    });
    const stoppedHandle = /Handle: ([^\s.]+)/.exec(dispatched)![1];
    const first = assignments[0];
    bus.send('dev', 'pm', 'task.partial', {
      instruction: 'Stopped: step safety limit after 100 steps.\n\nFAIL — unchecked: final URL scan.',
      metadata: {
        completionState: 'partial',
        unfinishedActivity: 'Finish the URL scan.',
        stopped: true,
        stopReason: 'step-safety-limit',
        delegationEvidence: { hadToolActions: true, changedFiles: [] },
      },
    }, 'normal', stoppedHandle);
    await new Promise<void>((resolve) => setTimeout(resolve, 0));

    const continued = await resumable.run('continue_task', {
      handle: stoppedHandle,
      focus: 'Finish only the final URL scan and return PASS or FAIL.',
    });

    expect(continued).toMatch(/Continued stopped task .*same dev conversation/i);
    expect(assignments).toHaveLength(2);
    expect(assignments[1].to).toBe(first.to);
    expect(assignments[1].correlationId).not.toBe(first.correlationId);
    expect(assignments[1].payload.taskAttempt?.attemptId).not.toBe(first.payload.taskAttempt?.attemptId);
    expect(assignments[1].payload.instruction).toMatch(/HOST CONTINUATION[\s\S]*existing conversation[\s\S]*final URL scan/i);
    expect(await resumable.run('continue_task', { handle: stoppedHandle })).toMatch(/not a resumable stopped task/i);
    resumable.cancelPending('test cleanup');
  });
  ...
  it('exposes the coordinator tools', () => {
    ...
    expect(names).toEqual([
      'list_agents', 'dispatch_task', 'continue_task', 'collect_ready_tasks', 'inspect_task_status', 'close_assignment',
  ...
  it('does not claim that an active durable row survived without a live host assignment', async () => {
    const stoppedRoster: TeamRosterEntry[] = roster.map((entry) => entry.id === 'dev'
      ? { ...entry, status: 'stopped' as const }
      : entry as TeamRosterEntry);
    const stoppedView: TeamView = {
      ...view,
      list: () => stoppedRoster,
    };
    const restored = new TeamTools('pm', stoppedView, bus, {
      inspectTaskStatus: () => [{
        handle: 'orphaned-handle',
        runId: 'restored-run',
        agentId: 'dev',
        requestedAgent: 'Content Strategist',
        lifecycle: 'active',
      }],
    });
    const messagesBefore = bus.query().length;

    const first = await restored.run('inspect_task_status', { handles: ['orphaned-handle'] });
    const second = await restored.run('inspect_task_status', { handles: ['orphaned-handle'] });

    expect(first).toBe(second);
    expect(first).toContain('state: interrupted (durable lifecycle: active; no live assignment in this host)');
    expect(first).toMatch(/worker state: stopped[\s\S]*wait state: live-state-unavailable[\s\S]*result state: none/);
    expect(first).toContain('next action: do not wait for this handle; dispatch a replacement task');
    expect(first).not.toContain('the host will wake the coordinator');
    expect(bus.query()).toHaveLength(messagesBefore);
  });

  it('lets a coordinator abandon an interrupted durable task and treats cancellation as idempotent', async () => {
    const ledger = new RunLedger();
    ledger.recordDelegationDispatched({
      coordinatorId: 'pm', handle: 'interrupted-handle', requestedAgent: 'dev', agentId: 'dev',
      instruction: 'Build it.', originCorrelationId: 'root-interrupted',
    });
    ledger.reconcileRestoredActiveDelegations('2026-09-11T12:00:00.000Z');
    const restored = new TeamTools('pm', view, bus, {
      inspectTaskStatus: (handles) => ledger.inspectTaskStatus('pm', handles),
      onDelegationDisposition: (event) => ledger.recordDisposition(event),
    });

    expect(await restored.run('cancel_task', { handle: 'interrupted-handle' }))
      .toMatch(/already terminal.*interrupted/i);
    expect(await restored.run('record_task_disposition', {
      handle: 'interrupted-handle',
      disposition: 'abandoned',
      reason: 'The worker continuity was lost and no replacement is needed.',
    })).toMatch(/recorded.*abandoned/i);
    expect(ledger.inspectTaskStatus('pm', ['interrupted-handle'])[0]).toMatchObject({
      lifecycle: 'interrupted', disposition: { value: 'abandoned' },
    });
  });

  it('keeps the first interrupted-task terminal disposition and reports it through inspect and cancel', async () => {
    const ledger = new RunLedger();
    for (const handle of ['first-superseded', 'first-abandoned']) {
      ledger.recordDelegationDispatched({
        coordinatorId: 'pm', handle, requestedAgent: 'dev', agentId: 'dev',
        instruction: 'Build it.', originCorrelationId: `root-${handle}`,
      });
    }
    ledger.reconcileRestoredActiveDelegations('2026-09-18T17:00:00.000Z');
    ledger.recordDelegationDispatched({
      coordinatorId: 'pm', handle: 'replacement-handle', requestedAgent: 'dev', agentId: 'dev',
      instruction: 'Replace the interrupted work.', originCorrelationId: 'root-replacement',
    });
    const guarded = new TeamTools('pm', view, bus, {
      timeoutMs: 1000,
      taskInputResolver: taskInputResolver(),
      inspectTaskStatus: (handles) => ledger.inspectTaskStatus('pm', handles),
      onDelegationDisposition: (event) => ledger.recordDisposition(event),
    ...
    expect(await guarded.run('record_task_disposition', {
      handle: 'first-superseded',
      disposition: 'superseded',
      reason: 'A replacement was dispatched.',
      replacement_handle: 'replacement-handle',
    })).toContain('Replacement replacement-handle was host-dispatched');
    const afterSuperseded = ledger.snapshot();
    expect(await guarded.run('record_task_disposition', {
      handle: 'first-superseded',
      disposition: 'abandoned',
      reason: 'Try to contradict the replacement.',
    })).toMatch(/already superseded by replacement-handle[\s\S]*ledger was not changed/i);
    expect(ledger.snapshot()).toEqual(afterSuperseded);
    expect(await guarded.run('inspect_task_status', { handles: ['first-superseded'] }))
      .toMatch(/state: interrupted[\s\S]*disposition: superseded by replacement-handle[\s\S]*next action: none/i);
    expect(await guarded.run('cancel_task', { handle: 'first-superseded' }))
      .toMatch(/already terminal: interrupted[\s\S]*superseded by replacement-handle/i);

    expect(await guarded.run('record_task_disposition', {
      handle: 'first-abandoned',
      disposition: 'abandoned',
      reason: 'No replacement is needed.',
    })).toMatch(/recorded.*abandoned/i);
    const afterAbandoned = ledger.snapshot();
    expect(await guarded.run('record_task_disposition', {
      handle: 'first-abandoned',
      disposition: 'superseded',
      reason: 'Try to contradict abandonment.',
      replacement_handle: 'replacement-handle',
    })).toMatch(/already abandoned[\s\S]*ledger was not changed/i);
    expect(ledger.snapshot()).toEqual(afterAbandoned);
    expect(await guarded.run('inspect_task_status', { handles: ['first-abandoned'] }))
      .toMatch(/state: interrupted[\s\S]*disposition: abandoned[\s\S]*next action: none/i);
    expect(await guarded.run('cancel_task', { handle: 'first-abandoned' }))
      .toMatch(/already terminal: interrupted[\s\S]*abandoned/i);

    expect(await guarded.run('dispatch_task', {
      agent: 'dev', instruction: 'Do not dispatch this second replacement.',
      replaces_handle: 'first-superseded', replacement_reason: 'A replacement already exists.',
      contract: taskContract(),
    })).toMatch(/already superseded by replacement-handle/i);
    expect(await guarded.run('dispatch_task', {
      agent: 'dev', instruction: 'Do not revive abandoned work.',
      replaces_handle: 'first-abandoned', replacement_reason: 'Abandonment was final.',
      contract: taskContract(),
    })).toMatch(/already abandoned/i);
  });

  it('admits and durably links an atomic replacement for interrupted work', async () => {
    const ledger = new RunLedger();
    const runId = ledger.recordDelegationDispatched({
      coordinatorId: 'pm', handle: 'original-interrupted', requestedAgent: 'dev', agentId: 'dev',
      instruction: 'Build it.', originCorrelationId: 'root-atomic-replacement',
    });
    ledger.reconcileRestoredActiveDelegations('2026-09-11T12:05:00.000Z');
    const replacing = new TeamTools('pm', view, bus, {
      timeoutMs: 1000,
      taskInputResolver: taskInputResolver(),
      inspectTaskStatus: (handles) => ledger.inspectTaskStatus('pm', handles),
      onDelegationDispatched: (event) => ledger.recordDelegationDispatched({
        ...event, originCorrelationId: 'root-atomic-replacement',
      }),
    });

    const result = await replacing.run('dispatch_task', {
      agent: 'dev',
      instruction: 'Resume from the durable inputs.',
      replaces_handle: 'original-interrupted',
      replacement_reason: 'The previous worker continuity was lost.',
      contract: taskContract(),
    });
    const replacementHandle = result.match(/Handle: ([0-9a-f-]+)/i)![1];
    const run = ledger.get(runId)!;
    expect(run.delegations.find((item) => item.handle === replacementHandle)?.state).toBe('active');
    expect(run.delegations.find((item) => item.handle === 'original-interrupted')).toMatchObject({
      state: 'interrupted',
      dispositions: [{ disposition: 'superseded', replacementHandle }],
    });
    replacing.cancelPending('test cleanup');
  });

  it('releases attempt grants and never queues an auto-wake after a worker interruption', async () => {
    const resolver = taskInputResolver();
    ...
    const live = new TeamTools('pm', view, bus, {
      timeoutMs: 1000,
      taskInputResolver: resolver,
      onDelegationInterrupted: interrupted,
      onAsyncResultReady: woke,
    });
    ...
    const handle = dispatched.match(/Handle: ([0-9a-f-]+)/i)![1];
    const attemptId = assignment!.payload.taskAttempt!.attemptId;
    expect(resolver.isAttemptLive(attemptId, 'dev')).toBe(true);
    ...
    bus.send('dev', 'pm', 'system.error', {
      ...
      metadata: {
        ...
        lastObservedAt: '2026-09-11T12:10:00.000Z',
        detectedAt: '2026-09-11T12:11:00.000Z',
      },
    }, 'normal', handle);
    await new Promise<void>((resolve) => setTimeout(resolve, 0));

    expect(resolver.isAttemptLive(attemptId, 'dev')).toBe(false);
    expect(interrupted).toHaveBeenCalledWith(expect.objectContaining({
      handle, attemptId, reason: 'worker-lost',
    }));
    expect(woke).not.toHaveBeenCalled();
  });

  it('keeps a stopped current-host dispatch pending when its live correlation exists', async () => {
    const stoppedRoster: TeamRosterEntry[] = roster.map((entry) => entry.id === 'dev'
      ? { ...entry, status: 'stopped' as const }
      : entry as TeamRosterEntry);
    const stoppedView: TeamView = {
    ...
    const live = new TeamTools('pm', stoppedView, bus, {
      timeoutMs: 1000,
      taskInputResolver: taskInputResolver(),
      inspectTaskStatus: (handles) => (handles ?? []).map((handle) => ({
        handle, agentId: 'dev', requestedAgent: 'Content Strategist', lifecycle: 'active' as const,
      })),
    });

    const dispatched = await live.run('dispatch_task', {
      agent: 'dev', instruction: 'Build the company list.', contract: taskContract(),
    });
    const handle = dispatched.match(/Handle: ([0-9a-f-]+)/i)![1];
    const status = await live.run('inspect_task_status', { handles: [handle] });

    expect(status).toContain('state: active');
    expect(status).toMatch(/worker state: stopped[\s\S]*wait state: within-deadline[\s\S]*result state: pending/);
    expect(status).toContain('next action: end this turn');
    expect(status).not.toContain('live-state-unavailable');
    live.cancelPending('test cleanup');
  });
