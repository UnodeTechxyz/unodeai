...
describe('claudeToolGate fail-closed hook process', () => {
  ...
  it('keeps a human decision alive when the authenticated gate ACKs and heartbeats', async () => {
    const server = await serve((_req, res) => {
      ...
      res.write('{"type":"ack"}\n');
      // Keep this on a seconds-scale liveness window like production. The former 25/100 ms pair
      // measured Windows scheduler latency under the complete-suite stress job: one otherwise healthy
      // child missed that artificial window. The decision still spans multiple liveness intervals and
      // many authenticated heartbeats; the next test independently pins fast fail-closed behavior.
    ...
    const result = await runHook(server.port, {
      UNODE_CLAUDE_TOOL_GATE_TIMEOUT_MS: '4000',
      UNODE_CLAUDE_TOOL_GATE_LIVENESS_MS: '1000',
    });
