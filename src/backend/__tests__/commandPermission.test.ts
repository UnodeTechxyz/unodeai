...
describe('decideCommandPermission (claude --permission-prompt-tool gate)', () => {
  ...
  it('defaults read-only scopes to deny except read-only Claude and Unode MCP tools', async () => {
    ...
    const tool = 'mcp__unode_files_0123456789abcdef0123456789abcdef__read_file';
    const mcpRead = await decideCommandPermission(tool, { path: 'README.md' }, {
      readOnly: true,
      allowedLocalMcpTools: new Set([tool]),
    });
    expect(mcpRead).toEqual({ behavior: 'allow', updatedInput: { path: 'README.md' } });

    const forged = await decideCommandPermission('mcp__unode_files__read_file', { path: 'README.md' }, { readOnly: true });
    expect(forged.behavior).toBe('deny');
  ...
  it('defaults untrusted workspaces to deny except read-only Claude and Unode MCP tools', async () => {
    ...
    const tool = 'mcp__unode_files_0123456789abcdef0123456789abcdef__read_file';
    const allowed = await decideCommandPermission(tool, { path: 'README.md' }, {
      isTrusted: false,
      allowedLocalMcpTools: new Set([tool]),
    });
    expect(allowed.behavior).toBe('allow');
