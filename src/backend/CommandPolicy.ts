...
/** A project-local opt-in can only lift the user default from disabled to ask; never to allow/all. */
export function effectiveCommandApprovalMode(
  userMode: CommandApprovalMode,
  projectExecutionEnabled: boolean,
): CommandApprovalMode {
  return userMode === 'none' && projectExecutionEnabled ? 'ask' : userMode;
}
...
export function windowsCmdletCompatibilityWarning(command: string): string | undefined {
  ...
  const sleepSeconds = /^start-sleep\s+(?:-seconds\s+)?(\d+)\b/i.exec(command.trim())?.[1];
  // ping waits between replies rather than before the first one, so N seconds needs N + 1 requests.
  const sleepPingCount = sleepSeconds === undefined ? '<seconds+1>' : (BigInt(sleepSeconds) + 1n).toString();
  const equivalent = cmdlet === 'start-sleep'
    ? `ping -n ${sleepPingCount} 127.0.0.1 >nul`
    : cmdlet === 'get-childitem'
      ...
      : cmdlet === 'get-content'
        ...
        : cmdlet === 'copy-item'
          ? 'copy or xcopy'
          : 'a native cmd.exe command';
  return `"${cmdlet}" is a PowerShell cmdlet, but agent commands run in cmd.exe — it will never succeed. Use the cmd.exe equivalent (${equivalent}), or switch the agent shell.`;
}
