...
export type BackendEvent =
  ...
  /** Host-authored per-turn coordinator dispatch accounting, rendered separately from model prose. */
  | { kind: 'delegation_receipt'; text: string }
  /** One host-observed attempt to contact the model provider. Observational only; it does not alter retries. */
  | { kind: 'model_request' }
  | { kind: 'tool_use'; name: string; input: unknown }
  | { kind: 'tool_result'; name: string; ok: boolean; summary: string; detail?: string; diff?: string; failureKind?: 'blocked' | 'not_found' | 'error' | 'integration_error' | 'cancelled' | 'outcome_unknown' }
  /** Codex-owned native MCP visibility only. Never contributes UnodeAi work or verification evidence. */
  | { kind: 'native_mcp_activity'; server: string; tool: string; status: 'inProgress' | 'completed' | 'failed' }
  ...
  | { kind: 'host_wait'; phase: 'started' | 'completed'; waitingOn: 'your approval' }
