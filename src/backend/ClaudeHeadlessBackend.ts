...
export class ClaudeHeadlessBackend implements AgentBackend {
  ...
  sendUserTurn(instruction: string, attachments?: TurnAttachments): void {
    ...
    if (attachments?.userAttachments?.some((attachment) => attachment.kind === 'pdf')) {
      ...
      this.emitTurnComplete({ text: message, isError: true });
      return;
    }
  ...
  private async prepareMcpConfig(cwd: string): Promise<ClaudeMcpConfig | undefined> {
    ...
    if (this.deps.teamMcpBridge) {
      if (this.deps.localMcpServerFactory) {
        ...
        mcpServers[this.bridgeIds.team] = buildTeamBridgeConfig(this.localMcpServer);
      } else {
    ...
    if (integrationSpecs.length > 0) {
      const server = this.deps.localMcpServerFactory?.() ?? createLocalMcpServer();
        ...
        server.addLocalTool({
          ...
          handler: async (args, context) => {
            if (this.deps.commandPermission?.isTrusted?.() === false) {
              return { text: 'Managed MCP tools are blocked because the workspace is not trusted.', isError: true };
            }
            ...
            return {
              text: boundManagedMcpResult(outcome.output),
    ...
    if (actionSpec) {
      ...
      server.addLocalTool({
        ...
        handler: async (args) => {
          if (this.activeTurnMode === 'plan') {
            return { text: `${RUN_SKILL_ACTION_TOOL} is unavailable in Plan mode.`, isError: true };
          }
          const outcome = await this.deps.executableSkills!.run(args);
          return { text: outcome.output, isError: outcome.status !== 'success' };
        },
      });
      await server.start();
  ...
  private buildFilesBridgeTools(tools: WorkspaceTools): LocalMcpTool[] {
    return [
      ...
      },
      {
        ...
        handler: async (args) => tools.runText('inspect_git_tag', args),
      },
      {
      ...
      handler: async (args) => {
        ...
        const decision = await this.awaitHumanDecision(decideCommandPermission(toolName, input, {
          ...
          requestApproval: gate?.requestApproval
            ? (request) => this.withHostApprovalWait(() => gate.requestApproval!(request))
            : undefined,
  ...
  private async decideNetworkRead(toolName: string): Promise<ClaudeToolApprovalDecision> {
    ...
    const decision = await this.awaitHumanDecision(
      this.withHostApprovalWait(() => webAccess.requestApproval({ agentName: this.config.name, toolName })),
      () => ({
        allow: false,
  ...
  private async approveNativeWrite(toolName: string, input: Record<string, unknown>): Promise<ClaudeToolApprovalDecision> {
    ...
    const answer = await this.awaitHumanDecision(
      this.withHostApprovalWait(() => this.deps.requestWriteApproval!(preview)),
      () => 'deny' as const,
    );
    return answer === 'deny'
  ...
  private async requestToolApproval(
    ...
    const answer = await this.awaitHumanDecision(
      this.withHostApprovalWait(() => this.deps.requestToolApproval!({ toolName, detail, input, timeoutMs: this.humanApprovalTimeoutMs() })),
      () => ({
        allow: false,
    ...
    try {
      return await Promise.race([
        ...
        new Promise<T>((_resolve, reject) => {
          timer = setTimeout(() => reject(new Error('The approval window expired.')), this.humanApprovalTimeoutMs());
        }),
      ]);
    } finally {
  ...
  private emitTurnComplete(result: TurnResult): void {
    this.turnOpen = false;
