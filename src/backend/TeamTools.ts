...
export class TeamTools {
  ...
  private inspectTaskStatus(handles?: string[]): string {
    ...
    return rows.map((row) => {
      ...
      } else if (liveStateUnavailable) {
        lines.push('next action: do not wait for this handle; dispatch a replacement task because this host has no resumable assignment for it');
