...
import { DelegationCompletionState, DelegationTaskScope, type Message, type SessionStatus } from '../types';
...
/**
 * A rework reply wakes an idle coordinator, which may request rework again. Without a person in the loop that
 * cycle has no natural end, so only this many rework requests per handle are sent after the user last wrote.
 */
export const MAX_AUTOMATIC_REWORK_ROUNDS = 2;
...
/** The original dispatch has settled, but its host-sent rework turn still owns live delegation work. */
interface ActiveRework {
  agentId: string;
  task: PendingAsyncTask;
  outstanding: number;
  dispose: () => void;
  seenTerminalMessageIds: Set<string>;
}
...
export type DelegationWaitState =
  | 'not-started' | 'live-state-unavailable' | 'within-deadline' | 'settled-on-time' | 'cancelled-before-timeout'
...
export interface CoordinatorTaskStatus {
  ...
  lifecycle: 'active' | 'settled' | 'cancelled' | 'interrupted' | 'timed-out' | 'policy-refused' | 'unknown';
  ...
  terminalAt?: string;
  interruption?: {
    reason: 'host-restarted' | 'worker-lost';
...
const COORDINATOR_DISPOSITIONS = new Set<CoordinatorDisposition>([
  ...
  'accepted-with-caveat', 'accepted-after-rework', 'accepted-despite-framework-no-evidence', 'superseded', 'abandoned',
]);
...
const REJECTION_DISPOSITIONS = new Set<CoordinatorDisposition>(['rejected', 'needs-rework', 'superseded', 'abandoned']);
const REASON_REQUIRED_DISPOSITIONS = new Set<CoordinatorDisposition>([
  'rejected', 'needs-human', 'needs-rework', 'deferred', 'accepted-with-caveat', 'superseded', 'abandoned',
]);
...
/** A worker completed the host-sent rework turn for an already-settled delegation. */
export interface DelegationReworkReplyEvent {
  handle: string;
  agentId: string;
  repliedAt: string;
}
...
export interface CoordinatorCloseoutState {
  ...
  /** Latest decision for each settled delegation, with a host-resolved agent name for a user-facing nudge. */
  latestDispositions: ReadonlyArray<{ agentName: string; disposition: CoordinatorDisposition }>;
  ...
  /** Rework requests recorded but not sent because the automatic cap was reached since the user last wrote. */
  reworkHeldAtCap: ReadonlyArray<{ agentName: string; rounds: number }>;
...
export interface DelegationDispatchEvent {
  ...
  dispatchedAt: string;
  /** When present, the host records this dispatch and the interrupted original's supersession together. */
  replacesHandle?: string;
...
/** A terminal receipt for admitted work whose live worker/host continuity was lost. */
export interface DelegationInterruptionEvent {
  coordinatorId: string;
  handle: string;
  agentId: string;
  /** Concrete attempt whose temporary grants must be released, when still present in this host. */
  attemptId?: string;
  reason: 'host-restarted' | 'worker-lost';
  lastObservedAt: string;
  detectedAt: string;
...
export function isAcceptanceDisposition(disposition: CoordinatorDisposition | undefined): boolean {
...
type TerminalTaskDisposition = {
  value: Extract<CoordinatorDisposition, 'superseded' | 'abandoned'>;
  replacementHandle?: string;
};

function terminalTaskDisposition(
  disposition: { value: CoordinatorDisposition; replacementHandle?: string } | undefined,
): TerminalTaskDisposition | undefined {
  if (disposition?.value !== 'superseded' && disposition?.value !== 'abandoned') return undefined;
  return {
    value: disposition.value,
    ...(disposition.replacementHandle ? { replacementHandle: disposition.replacementHandle } : {}),
  };
}

function describeTaskDisposition(disposition: { value: CoordinatorDisposition; replacementHandle?: string }): string {
  return disposition.value === 'superseded'
    ? `superseded by ${disposition.replacementHandle ?? 'another task'}`
    : disposition.value;
}
...
export interface TeamToolsOptions {
  ...
  /** Receives a rework-reply notice only if the existing async-result wake cannot be queued. */
  onDelegationReworkReply?: (event: DelegationReworkReplyEvent) => void;
  ...
  /** Receives a terminal interruption when an admitted worker disappears without a result. */
  onDelegationInterrupted?: (event: DelegationInterruptionEvent) => void;
  /**
   ...
   */
  onAsyncResultReady?: (result: AsyncDelegationResult) => boolean | void;
  ...
  /** A late admission/refusal changed a footer that the host already published. */
  onTurnDelegationReceiptChanged?: (event: TurnDelegationReceipt) => void;
}

export interface TurnDelegationReceipt {
  turn: number;
  accepted: number;
  refused: number;
  pending: number;
...
interface StoppedDelegationRecipe {
  agentId: string;
  requestedRef: string;
  instruction: string;
  contract: EffectiveTaskContract;
...
export class TeamTools {
  ...
  private onDelegationReworkReply?: TeamToolsOptions['onDelegationReworkReply'];
  ...
  private onDelegationInterrupted?: TeamToolsOptions['onDelegationInterrupted'];
  ...
  private onTurnDelegationReceiptChanged?: TeamToolsOptions['onTurnDelegationReceiptChanged'];
  ...
  /** A safety-stopped result may be resumed only on its original worker conversation. */
  private stoppedDelegations = new Map<string, StoppedDelegationRecipe>();
  private continuingStoppedDelegations = new Set<string>();
  ...
  /** Rework results share their original handle with the normal async-result delivery path. */
  private reworkResultHandles = new Set<string>();
  ...
  /** A needs-rework disposition sends a real worker turn, independent of the settled original dispatch. */
  private activeReworks = new Map<string, ActiveRework>();
  /** Rework requests sent per handle since the user last wrote to this coordinator. */
  private reworkRequestsSinceUser = new Map<string, number>();
  /** Rework requests recorded but not sent because the automatic cap was reached (handle → worker id). */
  private reworkHeldAtCap = new Map<string, string>();
  /** Listens for the user's next message only while a rework count exists, so no listener outlives it. */
  private offUserMessage?: () => void;
  ...
  /** First terminal coordinator decision wins, including before a durable callback can be queried again. */
  private terminalDispositionByHandle = new Map<string, TerminalTaskDisposition>();
  /** Prevents an interrupted async promise from becoming a ready result and waking the coordinator. */
  private interruptedHandles = new Set<string>();
  ...
  /** Turn receipts survive long enough for a busy worker's late admission to correct a published footer. */
  private turnDelegationReceipts = new Map<number, { accepted: number; refused: number; pending: number; published: boolean }>();
  private turnSequence = 0;
  private dispatchTurnByHandle = new Map<string, number>();
  ...
  constructor(
    ...
    // Product hosts always supply a workspace root. Standalone hosts may omit it only for
    // workspace-independent tools; run_checks refuses below instead of inheriting process.cwd().
    this.cwd = opts.cwd?.trim() ?? '';
    ...
    this.onDelegationReworkReply = opts.onDelegationReworkReply;
    ...
    this.onDelegationInterrupted = opts.onDelegationInterrupted;
    ...
    this.onTurnDelegationReceiptChanged = opts.onTurnDelegationReceiptChanged;
  ...
  beginTurnContentReceipts(): void {
    this.turnSequence++;
    this.turnDelegationReceipts.set(this.turnSequence, { accepted: 0, refused: 0, pending: 0, published: false });
    for (const turn of this.turnDelegationReceipts.keys()) {
      if (turn < this.turnSequence - 20) this.turnDelegationReceipts.delete(turn);
    }
  ...
  /** Host-authored truth footer for every coordinator turn; never inferred from assistant prose. */
  turnDelegationFooter(): string {
    const state = this.delegationReceiptFor(this.turnSequence);
    state.published = true;
    return this.formatTurnDelegationReceipt(this.turnSequence, state).text;
  }

  private noteDispatchAttempt(): void {
    this.dispatchAttempts++;
  }

  private noteAcceptedDispatch(turn: number): void {
    const state = this.delegationReceiptFor(turn);
    state.pending = Math.max(0, state.pending - 1);
    state.accepted++;
    this.publishUpdatedDelegationReceipt(turn, state);
  }

  private notePendingDispatch(turn: number): void {
    this.delegationReceiptFor(turn).pending++;
  }

  private noteRefusedDispatch(turn: number): void {
    const state = this.delegationReceiptFor(turn);
    state.pending = Math.max(0, state.pending - 1);
    state.refused++;
    this.publishUpdatedDelegationReceipt(turn, state);
  ...
  private delegationReceiptFor(turn: number): { accepted: number; refused: number; pending: number; published: boolean } {
    let state = this.turnDelegationReceipts.get(turn);
    if (!state) {
      state = { accepted: 0, refused: 0, pending: 0, published: false };
      this.turnDelegationReceipts.set(turn, state);
    }
    return state;
  }

  private formatTurnDelegationReceipt(
    turn: number,
    state: { accepted: number; refused: number; pending: number },
  ): TurnDelegationReceipt {
    return {
      turn,
      accepted: state.accepted,
      refused: state.refused,
      pending: state.pending,
      text: `Delegations this turn: ${state.accepted} accepted · ${state.refused} refused`
        + `${state.pending > 0 ? ` · ${state.pending} pending` : ''}.`,
    };
  }

  private publishUpdatedDelegationReceipt(
    turn: number,
    state: { accepted: number; refused: number; pending: number; published: boolean },
  ): void {
    if (state.published) this.onTurnDelegationReceiptChanged?.(this.formatTurnDelegationReceipt(turn, state));
  }
  ...
  specs(): ToolSpec[] {
    const specs = [
      ...
      spec(
        'dispatch_task',
        'Submit a versioned task contract. The host validates inputs, capabilities, scope, claims and verification sensors before selecting an executor. It dispatches asynchronously, authorises coordinator fallback only when the declared strategy permits it, or returns the distinct no-executor task state. To replace interrupted work atomically, provide replaces_handle and replacement_reason; admission then records the new durable handle and supersedes the old one in the same host step.',
        {
          ...
          replaces_handle: { type: 'string', description: 'Optional durable interrupted handle this new task replaces. The host refuses active, settled, cancelled, foreign, or unknown handles.' },
          replacement_reason: { type: 'string', description: 'Required with replaces_handle: the concrete reason the interrupted work is being superseded.' },
        },
        ['agent', 'instruction', 'contract']
      ),
      spec(
        'continue_task',
        'Continue a task that UnodeAi reported as stopped by a safety or budget boundary. This resumes the same worker conversation with its prior context and a fresh allowance, while issuing a new attempt-bound authority grant. Do not reject, blame, or dispatch the task from scratch. Optionally narrow the remaining work.',
        {
          handle: { type: 'string', description: 'The stopped delegation handle returned by the host.' },
          focus: { type: 'string', description: 'Optional concrete narrowing for the remaining work. It cannot broaden the original contract, capabilities, inputs, or write scope.' },
        },
        ['handle'],
      ),
      spec(
      ...
      spec(
        'record_task_disposition',
        'Record the coordinator decision you actually made about one settled delegated result, or mark interrupted work superseded/abandoned. This does not judge the reply. Use the vocabulary precisely: needs-rework sends the concrete reason back to the delegate; needs-human escalates outside the agent loop; deferred, superseded, abandoned, and accepted-with-caveat require their reason. Prefer dispatch_task with replaces_handle for an atomic replacement. This records coordinator acceptance only, never enterprise/customer acceptance.',
        {
          handle: { type: 'string', description: 'The settled or interrupted durable delegation handle.' },
          ...
          reason: { type: 'string', description: 'Required for rejected, needs-rework, needs-human, deferred, superseded, abandoned, and accepted-with-caveat. State the actionable reason; rework reasons are forwarded to the delegate.' },
  ...
  async run(name: string, args: Record<string, any>): Promise<string> {
    ...
    switch (name) {
      ...
      case 'dispatch_task': {
        ...
        return this.assignContract(
          String(args.agent ?? ''),
          String(args.instruction ?? ''),
          compiled.contract,
          typeof args.replaces_handle === 'string' ? args.replaces_handle : undefined,
          typeof args.replacement_reason === 'string' ? args.replacement_reason : undefined,
        );
      }
      case 'continue_task':
        return this.continueStoppedTask(
          String(args.handle ?? ''),
  ...
  private cancelTask(args: Record<string, unknown>): string {
    ...
    if (!dispatch) {
      ...
      if (known) {
        return `Assignment ${handle} has already settled; there is nothing running to stop. Its result is still collectable.`;
      }
      const durable = this.durableTaskStatus(handle);
      if (durable && durable.lifecycle !== 'unknown' && durable.lifecycle !== 'active') {
        const reason = durable.lifecycle === 'interrupted' && durable.interruption
          ? ` (${durable.interruption.reason}; detected ${durable.interruption.detectedAt})`
          : '';
        const disposition = durable.disposition ? `; ${describeTaskDisposition(durable.disposition)}` : '';
        return `Assignment ${handle} is already terminal: ${durable.lifecycle}${reason}${disposition}. Nothing is running to stop.`;
      }
      return this.toolFailure(`Error: no assignment with handle ${handle} is running for you. Use collect_ready_tasks to see what is in flight.`);
    }
  ...
  private recordTaskDisposition(
    ...
    const durable = this.durableTaskStatus(key);
    const interrupted = durable?.lifecycle === 'interrupted' ? durable : undefined;
    if (!settled && !interrupted) {
      return this.toolFailure('Error: no framework-observed settled delegation or interrupted task has that durable handle. Use inspect_task_status; do not guess a disposition for in-flight work.');
    }
    ...
    const settledTerminal = settled?.dispositions
      .map((event) => terminalTaskDisposition({ value: event.disposition, replacementHandle: event.replacementHandle }))
      .find((event): event is TerminalTaskDisposition => event !== undefined);
    const existingTerminal = this.terminalDispositionByHandle.get(key)
      ?? terminalTaskDisposition(durable?.disposition)
      ?? settledTerminal;
    if (existingTerminal) {
      return this.toolFailure(`Error: task ${key} is already ${describeTaskDisposition(existingTerminal)}. `
        + `Its terminal disposition cannot be changed to ${disposition}; the ledger was not changed.`);
    }
    ...
    if (interrupted && disposition !== 'superseded' && disposition !== 'abandoned') {
      return this.toolFailure('Error: interrupted work accepts only superseded or abandoned. It has no result to accept, reject, defer, or send for rework.');
    }
    if (!interrupted && disposition === 'abandoned') {
      return this.toolFailure('Error: abandoned is only valid for interrupted work. Use the settled-result disposition that matches the decision actually made.');
    }
    const replacementHandle = rawReplacementHandle?.trim();
    const replacementStatus = replacementHandle ? this.durableTaskStatus(replacementHandle) : undefined;
    const replacementReceipt = replacementHandle ? this.dispatchReceipts.get(replacementHandle) : undefined;
    const replacementAgentId = replacementStatus && replacementStatus.lifecycle !== 'unknown' && replacementStatus.lifecycle !== 'policy-refused'
      ? replacementStatus.agentId
      : !this.inspectTaskStatusQuery ? replacementReceipt?.agentId : undefined;
    if (disposition === 'superseded') {
      ...
      if (!replacementAgentId || replacementHandle === key) {
      ...
      return this.toolFailure('Error: replacement_handle is valid only with superseded.');
    }

    const agentId = settled?.agentId ?? interrupted!.agentId!;
    const event: DelegationDispositionEvent = {
      ...
      recordedAt: new Date().toISOString(),
      outcome: settled?.outcome ?? interrupted?.evidenceOutcome ?? 'no-evidence',
    };
    // A later disposition replaces an outstanding rework request. In particular, an
    // accepted-after-rework or superseded result must not leave a stale late-reply notice behind.
    if (disposition !== 'needs-rework') {
      this.endPendingRework(key);
      this.reworkHeldAtCap.delete(key);
    }
    settled?.dispositions.push(event);
    this.onDelegationDisposition?.(event);
    const terminal = terminalTaskDisposition({ value: disposition, replacementHandle });
    if (terminal) this.terminalDispositionByHandle.set(key, terminal);

    if (disposition === 'needs-rework') {
      const round = this.countReworkRequest(key);
      if (round > MAX_AUTOMATIC_REWORK_ROUNDS) {
        // Recorded, but deliberately not sent: the host-authored closeout tells the user, who decides.
        this.reworkHeldAtCap.set(key, agentId);
        return `Recorded needs-rework for ${key}, but it was NOT sent to ${agentId}: this is rework request ${round} `
          + `since the user last wrote, and automatic rework stops after ${MAX_AUTOMATIC_REWORK_ROUNDS}. Tell the user `
          + 'what is still missing and let them decide: accept the result, or ask you for another round.';
      }
      // The directed message below starts real work. Its original dispatch listener has already settled,
      // so keep a narrow listener for the rework response instead of calling this an unsent follow-up.
      this.beginPendingRework(key, agentId);
      this.bus.send(
        this.selfId,
        agentId,
        'agent.message',
        { instruction: `Coordinator requested rework for delegation ${key}. Reason: ${reason}\n\nAddress this specific gap and return the corrected result with concrete evidence.` },
        'high',
        key,
      );
      return `Recorded needs-rework for ${key}; the actionable reason was sent to ${agentId}.`;
    }
    if (disposition === 'rejected') {
      ...
      this.bus.send(
        ...
        agentId,
        'agent.message',
        { instruction: `Coordinator rejected delegation ${key}. Reason: ${reason}\n\nDo not redo work until you receive a new assignment; retain this reason for that rework.` },
        'high',
        key,
      );
      return `Recorded coordinator rejection for ${key}; the reason was sent to ${agentId}. The displayed framework verdict is visibly amended, not rewritten.`;
    }
    if (disposition === 'needs-human') {
      return `Recorded that ${key} requires human intervention: ${reason}. This is not coordinator acceptance and is not enterprise acceptance.`;
    }
    if (disposition === 'deferred') {
      return `Recorded deferred for ${key}: ${reason}. The result remains in the append-only decision history.`;
    }
    if (disposition === 'superseded') {
      return `Recorded ${key} as superseded: ${reason}. Replacement ${replacementHandle} was host-dispatched to ${replacementAgentId}; ${interrupted ? 'the interrupted task remains terminal' : 'the framework verdict is retained for calibration'}.`;
    }
    if (disposition === 'abandoned') {
      return `Recorded interrupted assignment ${key} as abandoned: ${reason}. It remains terminal and will not be woken or resumed.`;
    }
    if (disposition === 'accepted-with-caveat') {
      return `Recorded coordinator acceptance with caveat for ${key}: ${reason}. This is not enterprise/customer acceptance.`;
    }
    if (disposition === 'accepted-after-rework') {
      return `Recorded coordinator acceptance after rework for ${key}. This is not enterprise/customer acceptance.`;
    }
    if (disposition === 'accepted-despite-framework-no-evidence') {
      return `Recorded acceptance despite framework no-evidence for ${key}. The evidence verdict remains unchanged for calibration.`;
    }
    return `Recorded coordinator acceptance for ${key}. This is coordinator-accepted only; it is not enterprise/customer acceptance.`;
  }

  private durableTaskStatus(handle: string): CoordinatorTaskStatus | undefined {
    if (!handle || !this.inspectTaskStatusQuery) return undefined;
    return this.inspectTaskStatusQuery([handle]).find((row) => row.handle === handle && row.lifecycle !== 'unknown');
  }
  ...
  private delegationMetrics(): string {
    ...
    return [
      ...
      'Limit: coordinator-accepted is an agent coordinator decision, NOT enterprise/customer acceptance. A person\'s run verdict, when recorded, remains separate and is never inferred from coordinator acceptance.',
    ].join('\n');
  }
  ...
  coordinatorCloseoutState(): CoordinatorCloseoutState {
    ...
    const namesById = new Map(this.view.list().map((entry) => [entry.id, entry.name]));
    const latestDispositions = settled.flatMap((entry) => {
      const disposition = latest(entry)?.disposition;
      return disposition ? [{ agentName: namesById.get(entry.agentId) ?? entry.agentId, disposition }] : [];
    });
    const reworkHeldAtCap = [...this.reworkHeldAtCap.entries()].map(([handle, agentId]) => ({
      agentName: namesById.get(agentId) ?? agentId,
      rounds: this.reworkRequestsSinceUser.get(handle) ?? MAX_AUTOMATIC_REWORK_ROUNDS + 1,
    }));
  ...
  private hasLiveDelegationWork(): boolean {
    return this.activeDispatches.size > 0 || this.liveAsyncTaskCount() > 0 || this.activeReworks.size > 0;
  }

  /**
   * Observe the terminal response to the rework turn the coordinator already sent.
   *
   * A rework is a new worker turn, but it belongs to the original delegation handle. Store its result in
   * the same pending/ready path as assign_task_async so SessionManager can issue its existing bounded PM
   * wake instead of leaving an idle coordinator unaware of the reply.
   */
  private beginPendingRework(handle: string, agentId: string): void {
    const existing = this.activeReworks.get(handle);
    if (existing?.agentId === agentId) {
      existing.outstanding += 1;
      return;
    }
    this.endPendingRework(handle);

    let resolveResult!: (text: string) => void;
    const task: PendingAsyncTask = {
      ref: agentId,
      promise: new Promise<string>((resolve) => { resolveResult = resolve; }),
    };
    // A settled rework must be delivered through the original handle. `needs-rework` is recorded only
    // after the initial result has reached the PM, so this cannot replace an undelivered initial result.
    this.pending.set(handle, task);
    this.reworkResultHandles.add(handle);
    this.updateLiveDispatch(handle, { agentId, resultState: 'pending' });
    void task.promise.then(
      (text) => this.noteAsyncResultUnlessInterrupted(handle, task, text),
      (reason) => this.noteAsyncResultUnlessInterrupted(handle, task, `Error: ${String(reason)}`),
    );
    ...
    const matches = (message: Message) =>
      message.correlationId === handle && message.from === agentId && message.to === this.selfId;
    const complete = (message: Message, isReply: boolean) => {
      const active = this.activeReworks.get(handle);
      if (!active || active.agentId !== agentId || active.seenTerminalMessageIds.has(message.id)) return;
      active.seenTerminalMessageIds.add(message.id);
      active.outstanding = Math.max(0, active.outstanding - 1);
      if (active.outstanding === 0) {
        active.dispose();
        this.activeReworks.delete(handle);
      }
      const reply = typeof message.payload.instruction === 'string' ? message.payload.instruction : '';
      resolveResult(isReply
        ? `[rework reply] ${agentId} replied to the PM's rework request.\n\n${reply}\n\n${this.withDispositionPrompt('', handle)}`
        : `Error: ${agentId}'s rework turn ended without a reply. ${reply}`.trim());
    };
    offComplete = this.bus.onType('task.complete', (message) => {
      if (matches(message)) complete(message, true);
    });
    offPartial = this.bus.onType('task.partial', (message) => {
      if (matches(message)) complete(message, true);
    });
    offError = this.bus.onType('system.error', (message) => {
      if (matches(message)) complete(message, false);
    });
    this.activeReworks.set(handle, {
      agentId,
      task,
      outstanding: 1,
      seenTerminalMessageIds: new Set<string>(),
      dispose: () => {
        offComplete();
        offPartial();
        offError();
  ...
  /**
   * Count one rework request for `handle` and return its round since the user last wrote. Only a person's
   * message re-arms automatic rework: the host's async-result wake arrives from `unode`, not from `user`.
   */
  private countReworkRequest(handle: string): number {
    const round = (this.reworkRequestsSinceUser.get(handle) ?? 0) + 1;
    this.reworkRequestsSinceUser.set(handle, round);
    if (!this.offUserMessage) {
      this.offUserMessage = this.bus.onType('ask.question', (message) => {
        if (message.from !== 'user' || message.to !== this.selfId) return;
        this.reworkRequestsSinceUser.clear();
        this.reworkHeldAtCap.clear();
        this.offUserMessage?.();
        this.offUserMessage = undefined;
      });
    }
    return round;
  }

  private endPendingRework(handle: string): void {
    const active = this.activeReworks.get(handle);
    if (!active) return;
    active.dispose();
    this.activeReworks.delete(handle);
    if (this.pending.get(handle) === active.task) {
      this.pending.delete(handle);
      this.readyAsync.delete(handle);
      this.reworkResultHandles.delete(handle);
    }
  }
  ...
  private recordRefusedDispatch(
    ...
    const dispatchTurn = details.handle ? this.dispatchTurnByHandle.get(details.handle) : this.turnSequence;
    this.noteRefusedDispatch(dispatchTurn ?? this.turnSequence);
  ...
  private async assignContract(
    ref: string,
    instruction: string,
    contract: EffectiveTaskContract,
    rawReplacesHandle?: string,
    rawReplacementReason?: string,
  ): Promise<string> {
    this.noteDispatchAttempt();
    const replacement = this.validateInterruptedReplacement(rawReplacesHandle, rawReplacementReason);
    if (!replacement.ok) {
      return this.toolFailure(replacement.error);
    }
    if (replacement.value && contract.executionStrategy !== 'delegate-required') {
      return this.toolFailure('Error: replacing interrupted work requires execution_strategy "delegate-required" so coordinator fallback cannot leave the replacement unrecorded.');
    }
    ...
    const started = await this.startContractAttempt(target.id, ref, instruction, contract, replacement.value);
    if (!started.ok) {
    ...
    void compliant.then(
      (text) => this.noteAsyncResultUnlessInterrupted(started.handle, task, text),
      (reason) => this.noteAsyncResultUnlessInterrupted(started.handle, task, `Error: ${String(reason)}`),
    );
    return `Dispatched contract ${contract.contractId} to ${target.id}. Handle: ${started.handle}. `
      + (replacement.value
        ? `Interrupted handle ${replacement.value.handle} will be superseded atomically when this worker is admitted. `
        : '')
      + 'End this turn; UnodeAi opens a later PM turn when the result settles.';
  }

  /**
   * Resume a safety-stopped task without throwing away the worker's conversation.  The original attempt
   * is terminal, so normal admission issues a fresh correlation-bound card; conversation continuity is
   * provided by routing the new turn to the exact same backend session.
   */
  private async continueStoppedTask(rawHandle: string, rawFocus?: string): Promise<string> {
    const handle = rawHandle.trim();
    const recipe = this.stoppedDelegations.get(handle);
    if (!recipe) {
      return this.toolFailure(`Error: ${handle || 'that handle'} is not a resumable stopped task owned by this coordinator.`);
    }
    if (this.continuingStoppedDelegations.has(handle)) {
      return this.toolFailure(`Error: stopped task ${handle} is already being continued.`);
    }
    const focus = rawFocus?.trim();
    if (focus && focus.length > 2_000) {
      return this.toolFailure('Error: continue_task focus exceeds the 2000-character limit.');
    }
    const coordinator = this.view.list().find((entry) => entry.id === this.selfId);
    const worker = this.view.list().find((entry) => entry.id === recipe.agentId);
    if (!coordinator || coordinator.status === 'stopped' || coordinator.status === 'error' || !worker || isUnavailableStatus(worker.status)) {
      return this.toolFailure(`Error: the original coordinator or worker for stopped task ${handle} is no longer reachable.`);
    }

    this.continuingStoppedDelegations.add(handle);
    const continuationInstruction =
      `[HOST CONTINUATION] Resume the same task from your existing conversation after a ${recipe.reason} stop. `
      + 'Keep the files, evidence, and conclusions you already gathered; do not restart or re-read everything. '
      + 'Finish the remaining work and return the decisive result the original task requested.'
      + (focus ? `\n\nNarrowed remaining focus: ${focus}` : '')
      + `\n\nOriginal task: ${recipe.instruction}`;
    try {
      ...
      if (!started.ok) {
        return this.toolFailure(`Error: stopped task ${handle} could not be continued: ${started.error}`);
      }
      this.stoppedDelegations.delete(handle);
      const compliant = started.promise.then((result) => this.enforceContractCompliance(
        recipe.requestedRef,
        recipe.agentId,
        continuationInstruction,
        result,
      ...
      return `Continued stopped task ${handle} on the same ${recipe.agentId} conversation. New handle: ${started.handle}. `
        + 'The worker has a fresh safety allowance and newly admitted attempt authority. End this turn; UnodeAi will wake you when it settles.';
    } finally {
      this.continuingStoppedDelegations.delete(handle);
    }
  }

  private validateInterruptedReplacement(
    rawHandle?: string,
    rawReason?: string,
  ): { ok: true; value?: { handle: string; reason: string } } | { ok: false; error: string } {
    const handle = rawHandle?.trim();
    const reason = rawReason?.trim();
    if (!handle && !reason) return { ok: true };
    if (!handle || !reason) {
      return { ok: false, error: 'Error: replaces_handle and replacement_reason must be supplied together.' };
    }
    if (reason.length > 2_000) {
      return { ok: false, error: 'Error: replacement_reason exceeds the 2000-character limit.' };
    }
    const durable = this.inspectTaskStatusQuery?.([handle]).find((row) => row.handle === handle);
    if (!durable || durable.lifecycle !== 'interrupted') {
      return { ok: false, error: `Error: replaces_handle must identify your durable interrupted task; ${handle} is ${durable?.lifecycle ?? 'unknown'}.` };
    }
    if (durable.disposition?.value === 'superseded') {
      return { ok: false, error: `Error: interrupted task ${handle} is already superseded by ${durable.disposition.replacementHandle ?? 'another task'}.` };
    }
    if (durable.disposition?.value === 'abandoned') {
      return { ok: false, error: `Error: interrupted task ${handle} is already abandoned.` };
    }
    return { ok: true, value: { handle, reason } };
  }
  ...
  private async startContractAttempt(
    ...
    instruction: string,
    contract: EffectiveTaskContract,
    replacement?: { handle: string; reason: string },
    ...
    const dispatched = this.dispatch(
      ...
      contract.effects.readFiles, contract, attempt.card, requestedRef, replacement,
    );
    if (!dispatched.ok) {
  ...
  private delegationCapabilityMismatch(agentId: string, contract: EffectiveTaskContract): string | undefined {
    ...
    const grants = missing.map((capability) => capabilityLabels[capability] ?? `Grant ${capability}`);
    return `${target.name} lacks declared capability ${missing.join(', ')} (tool families: ${families}). `
      + `Open ${target.name} in Agent Builder and select a Skill whose resulting permissions include ${grants.join(' and ')}.`;
  }
  ...
  private async runChecks(): Promise<string> {
    ...
    if (!this.cwd) {
      return this.toolFailure('Open a workspace folder before running checks; no process was started.');
    }
    ...
    if (!gate.ok) {
      ...
      return this.toolRefusal(
        ...
        gate.kind === 'outside-unattended'
          ? 'scope'
          : gate.kind === 'shell-compatibility' ? 'shell-compatibility' : 'capability',
      );
    }
  ...
  private assignAsync(
    ...
    void compliant.then(
      (text) => this.noteAsyncResultUnlessInterrupted(d.handle, task, text),
      (reason) => this.noteAsyncResultUnlessInterrupted(d.handle, task, `Error: ${String(reason)}`)
    );
  ...
  private inspectTaskStatus(handles?: string[]): string {
    ...
    return rows.map((row) => {
      const liveStateUnavailable = row.waitState === 'live-state-unavailable';
      const interrupted = row.lifecycle === 'interrupted';
      const lines = [
        row.lifecycle === 'unknown'
          ? `handle ${row.handle} · state: unknown`
          : `handle ${row.handle} · ${row.requestedAgent || row.agentId || 'agent'}`,
        (interrupted
          ? `state: interrupted (${row.interruption?.reason ?? 'worker continuity lost'}; no active worker)`
          : liveStateUnavailable
          ? `state: interrupted (durable lifecycle: ${row.lifecycle}; no live assignment in this host)`
      ...
      if (row.interruption) {
        lines.push(`last observed activity: ${row.interruption.lastObservedAt}`);
        lines.push(`interruption detected: ${row.interruption.detectedAt}`);
      }
      ...
      lines.push(row.disposition
        ? `disposition: ${describeTaskDisposition(row.disposition)}`
        : 'disposition: not recorded');
      if (row.resultState === 'ready') {
        lines.push(`next action: call collect_ready_tasks with handle "${row.handle}"`);
      } else if (interrupted || liveStateUnavailable) {
        const terminalDisposition = terminalTaskDisposition(row.disposition);
        lines.push(terminalDisposition
          ? `next action: none; terminal disposition already recorded: ${describeTaskDisposition(terminalDisposition)}`
          : 'next action: do not wait for this handle; dispatch a replacement task with replaces_handle for an atomic replacement, or record it abandoned');
  ...
  private projectLiveTaskStatus(row: CoordinatorTaskStatus): CoordinatorTaskStatus {
    ...
    // The Run Ledger can correctly retain an active (no terminal receipt) lifecycle across activation,
    // but the worker promise and its correlation live only in the TeamTools instance that dispatched it.
    // Never turn that durable historical fact into a present-tense promise that this host will wake later.
    // A dispatch made by this instance has `live`, even while its worker is stopped/queued, so it remains
    // distinguishable from an interrupted assignment after restore.
    const liveStateUnavailable = row.lifecycle === 'active' && !live;
    ...
    const resultState: DelegationResultState = live?.resultState
      ?? (row.delivery?.state === 'delivered' ? 'delivered'
        : row.delivery?.state === 'pending' ? 'ready'
          : row.lifecycle === 'active' && !liveStateUnavailable ? 'pending' : 'none');
    const waitState: DelegationWaitState = live?.waitState
      ?? (liveStateUnavailable ? 'live-state-unavailable'
        : row.lifecycle === 'active' ? 'within-deadline'
        : row.lifecycle === 'interrupted' ? 'interrupted'
  ...
  private noteAsyncResultUnlessInterrupted(handle: string, task: PendingAsyncTask, text: string): void {
    if (this.interruptedHandles.delete(handle)) {
      this.pending.delete(handle);
      this.awaiting.delete(handle);
      this.readyAsync.delete(handle);
      this.reworkResultHandles.delete(handle);
    ...
    this.noteAsyncResultReady(handle, task, text);
  }
  ...
  private flushAsyncResultReady(): void {
    ...
    for (const result of this.readyAsync.values()) {
      if (this.pending.get(result.handle) === result.task) {
        const queued = this.onAsyncResultReady({ handle: result.handle, ref: result.ref, text: result.text });
        // Normal rework delivery uses the same durable async-result wake. Only a host that cannot queue
        // that wake (for example because Stop removed the PM backend) gets a static notice instead.
        if (queued === false && this.reworkResultHandles.delete(result.handle)) {
          this.onDelegationReworkReply?.({
  ...
  private dispatch(
    ...
    const dispatchTurn = this.turnSequence;
    const target = this.resolveTarget(ref);
    if (!target) {
    ...
    this.dispatchTurnByHandle.set(pendingId, dispatchTurn);
    ...
    let dispatchReceiptPublished = false;
    let deferredInterruption: DelegationInterruptionEvent | undefined;
    ...
    const finishInterrupted = (
      ...
      if (lateTimer) { clearTimeout(lateTimer); }
      this.activeDispatches.delete(pendingId);
      this.interruptedHandles.add(pendingId);
      this.updateLiveDispatch(pendingId, { waitState: 'interrupted', resultState: 'none' });
      if (taskAttempt) this.taskInputResolver?.endAttempt(taskAttempt.attemptId, 'interrupted');
      this.markFree(target.id);
      this.pending.delete(pendingId);
      this.awaiting.delete(pendingId);
      this.readyAsync.delete(pendingId);
      this.releaseDispatchClaim(pendingId);
      this.onAsyncResultConsumed?.(pendingId);
      const interruptedAt = detectedAt ?? new Date().toISOString();
      const event: DelegationInterruptionEvent = {
        coordinatorId: this.selfId,
        handle: pendingId,
        agentId: target.id,
        ...(taskAttempt ? { attemptId: taskAttempt.attemptId } : {}),
        reason,
        lastObservedAt: lastObservedAt ?? interruptedAt,
        detectedAt: interruptedAt,
      };
      if (dispatchReceiptPublished) this.onDelegationInterrupted?.(event);
      else deferredInterruption = event;
      resolvePromise(`[INTERRUPTED] ${target.id} lost its live worker before a terminal result was observed.`);
    };
    ...
    const timeOutBlockingWait = () => {
      ...
      void latePromise.then((text) => this.noteAsyncResultUnlessInterrupted(pendingId, task, text));
    ...
    const settleCompletion = (
      ...
      const stopped = (metadata as { stopped?: unknown; stopReason?: unknown } | undefined)?.stopped === true;
      const continuationPrompt = stopped
        ? `\n\n[coordinator continuation] This task stopped at a host safety boundary; it was not completed and the worker did not fail. `
          + `Call continue_task with handle ${pendingId} to resume this same worker conversation, optionally narrowing the remaining focus. `
          + 'Do not reject, blame, or dispatch it from scratch. Report the stop reason to the user if continuation is not appropriate.'
        : '';
      if (!this.evidenceEnabled || isTaskFailure(reply)) {
        finish(`${shownReply(reply)}${continuationPrompt}`, 'no-evidence', undefined, completionState);
        return;
      }
      const evidence = delegationEvidenceFromMetadata(metadata);
      ...
      const framed = formatDelegationEvidence(shownReply(reply), record);
      finish(stopped ? `${framed}${continuationPrompt}` : this.withDispositionPrompt(framed, pendingId), record.outcome, record);
    };
    ...
    offPartial = this.bus.onType('task.partial', (m) => {
      if (m.correlationId !== pendingId) return;
      const stopped = m.payload.metadata as {
        stopped?: unknown;
        stopReason?: unknown;
      } | undefined;
      if (stopped?.stopped === true
        && (stopped.stopReason === 'step-safety-limit' || stopped.stopReason === 'anti-spin' || stopped.stopReason === 'budget')) {
        this.stoppedDelegations.set(pendingId, {
          agentId: target.id,
          requestedRef,
          instruction,
          contract,
    ...
    offError = this.bus.onType('system.error', (m) => {
      if (m.correlationId === pendingId) {
        const policyMetadata = m.payload.metadata as {
        ...
        if (policyMetadata?.interrupted === true) {
          finishInterrupted(
            policyMetadata.interruptionReason === 'host-restarted' ? 'host-restarted' : 'worker-lost',
            typeof policyMetadata.lastObservedAt === 'string' ? policyMetadata.lastObservedAt : undefined,
            typeof policyMetadata.detectedAt === 'string' ? policyMetadata.detectedAt : m.timestamp,
          );
          return;
        }
    ...
    const dispatchEvent: DelegationDispatchEvent = {
      ...
      routing,
      dispatchedAt,
      ...(replacement ? { replacesHandle: replacement.handle, replacementReason: replacement.reason } : {}),
    };
    const publishDispatchReceipt = () => {
      if (dispatchReceiptPublished) return;
      dispatchReceiptPublished = true;
      this.onDelegationDispatched?.(dispatchEvent);
      if (deferredInterruption) {
        this.onDelegationInterrupted?.(deferredInterruption);
        deferredInterruption = undefined;
      }
    };
    let admitted = !this.waitForTaskAdmission;
    if (this.waitForTaskAdmission) {
      this.notePendingDispatch(dispatchTurn);
      offAdmitted = this.bus.onType('task.admitted', (message) => {
        if (message.correlationId !== pendingId || admitted || settled || cancelled) return;
        admitted = true;
        offAdmitted();
        this.noteAcceptedDispatch(dispatchTurn);
        publishDispatchReceipt();
      });
    }
...
const defaultRunner: CommandRunner = (command, cwd) =>
  new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      killProcessTree(proc, (program) => resolveHostExecutable(program, { workspaceRoots: [cwd] })); // Windows: kill the whole tree, not just cmd.exe (audit N2)
