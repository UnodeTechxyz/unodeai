...
import { CommandPolicy, windowsCmdletCompatibilityWarning } from './CommandPolicy';
...
export type ShellCommandGateBlockKind = 'policy' | 'approval' | 'outside-unattended' | 'shell-compatibility';
...
export async function gateShellCommand(opts: ShellCommandGateOptions): Promise<ShellCommandGateResult> {
  ...
  // cmd.exe cannot execute a bare PowerShell cmdlet. This is an executor fact, not a consent decision:
  // refuse before opening an approval surface so an Allow click can never be reported as missing consent.
  // A catastrophic-command denial remains dominant so compatibility guidance can never soften that block.
  const compatibilityMessage = commandCheckOptions?.activeShell === 'cmd'
    ? windowsCmdletCompatibilityWarning(opts.command)
    : undefined;
  if (compatibilityMessage && verdict.reason !== 'matches a blocked destructive pattern') {
    return {
      ok: false,
      kind: 'shell-compatibility',
      reason: compatibilityMessage,
      message: compatibilityMessage,
    };
  }

  // A policy DENIAL is final. Outside-root detection may add a prompt/warning only to a command policy
  // would otherwise permit; it must never make a denied command runnable.
