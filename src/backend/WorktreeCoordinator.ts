      ...
      if (committed && this.deps.verify && wt.agentId) {
        ...
        if (verification.blocked) {
          ...
          this.deps.notifyAgent?.(
            ...
            `to run, so the gate can't confirm it. Ask the user to approve it from a normal command prompt ` +
            `or use "UnodeAi: Manage Project Command Approvals". Your work is safe on your own branch until verification can run.`
          );
