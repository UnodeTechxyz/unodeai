...
import { localReadRootsForScope, SessionLocalReadScopeConsent, type LocalReadScope, type LocalReadScopeConsentGate } from './backend/localReadScope';
...
let memoryAttestationStore: MemoryAttestationStore;
...
async function applyExecutionHooks(context: vscode.ExtensionContext): Promise<void> {
  ...
  void vscode.window.showInformationMessage('Execution hooks applied. Running agents enforce them at their next hook point.', { modal: true });
}
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // persistent egress-grant registry: local-scope confirmation is once per extension session, per root.
  const localReadScopeConsent: LocalReadScopeConsentGate = new SessionLocalReadScopeConsent(requestLocalReadScopeConsent);
  ...
  memoryAttestationStore = new MemoryAttestationStore(context.workspaceState, workspaceRoot());
    ...
    messageBus,
    {
      ...
      getTurnContextBundle: (config, msg) => {
        const memorySnapshot = sharedMemory.promptSnapshot(
          vscode.workspace.isTrusted ? memoryAttestationStore.attestedDigests() : new Set()
        );
        const projectContext = [
          projectConventions.get(),
          rulesFile.getRepositorySummaryContext(),
          projectKnowledge.promptBlock(),
...
async function approveCoordinatorBriefEgress(
  ...
  const decision = decideCoordinatorBriefConsent({
    ...
    allowCrossProviderWithoutApproval: vscode.workspace.getConfiguration('unode')
      .get<boolean>(ALLOW_CROSS_PROVIDER_DISPATCH_SETTING, ALLOW_CROSS_PROVIDER_DISPATCH_DEFAULT) === true,
  });
  ...
  // No explicit Cancel button: a modal already has one, and passing another rendered two.
  const choice = await vscode.window.showWarningMessage(
...
function configuredUserAdditionalRoots(): string[] {
  ...
  const cfg = vscode.workspace.getConfiguration('unode');
  const inspected = cfg.inspect<string[]>('additionalRoots');
  return [
...
function configuredLocalReadScopeRoots(): string[] {
  if (!vscode.workspace.isTrusted) { return []; }
  const cfg = vscode.workspace.getConfiguration('unode');
  return localReadRootsForScope(cfg.get<LocalReadScope>('localReadScope', 'parent'), workspaceRoot(), true);
}
...
async function restoreCheckpoint(c: Checkpoint, edits = 1): Promise<void> {
  ...
  try {
    ...
    vscode.window.showInformationMessage(`Restored ${c.path}.`, { modal: true });
  } catch (err) {
...
async function reviewSharedMemory(): Promise<void> {
  if (!vscode.workspace.workspaceFolders?.length) {
    void vscode.window.showInformationMessage('Open a workspace folder before reviewing shared memory.', { modal: true });
    return;
  }
  await sharedMemory.load();
  const trustedWorkspace = vscode.workspace.isTrusted;
  const attested = trustedWorkspace ? memoryAttestationStore.attestedDigests() : new Set<string>();
  const rows = sharedMemory.reviewRows(attested);
  if (rows.length === 0) {
    void vscode.window.showInformationMessage('Shared memory has no rows to review in the primary workspace folder.', { modal: true });
    return;
  }
  if (trustedWorkspace) {
    await memoryAttestationStore.prune(new Set(rows.map((row) => row.digest)));
  }
  const selected = await vscode.window.showQuickPick(
    [...rows].reverse().map((row) => ({
      label: `${row.trust === 'human-attested' ? '$(verified-filled)' : '$(warning)'} ${row.trust}`,
      description: `${row.kind} · ${row.agentId} · ${row.tier}`,
  ...
  if (selected.row.trust === 'human-attested') {
    ...
    await memoryAttestationStore.revoke(selected.row.digest);
    void vscode.window.showInformationMessage('Shared-memory row is now untrusted.', { modal: true });
    return;
  }

  if (!vscode.workspace.isTrusted) {
    void vscode.window.showWarningMessage('Shared-memory attestation is disabled until you trust this workspace. All rows remain untrusted.', { modal: true });
    return;
  }
  if (!selected.row.attestable) {
    void vscode.window.showWarningMessage('This row cannot be attested because its exact content exceeds the bounded review display or requires whitespace normalization. Shorten or normalize the row, then review it again.', { modal: true });
    return;
  }
  ...
  if (!vscode.workspace.isTrusted) {
    void vscode.window.showWarningMessage('Workspace Trust changed before attestation was recorded. The row remains untrusted.', { modal: true });
    return;
  }
  await memoryAttestationStore.attest(selected.row.digest);
  void vscode.window.showInformationMessage('Shared-memory row is now human-attested guidance.', { modal: true });
}
...
async function reviewRunAcceptance(runId?: string): Promise<void> {
  ...
  void vscode.window.showInformationMessage(`Recorded human verdict: ${choice.verdict}.`, { modal: true });
}
...
function registerCommands(context: vscode.ExtensionContext) {
  ...
  reg('unode.importChat', () => guard(async () => {
    ...
    if (chatViewProvider.importToSelected(parsed.messages)) {
      vscode.window.showInformationMessage(`Imported ${parsed.messages.length} chat message(s) into ${who}.`, { modal: true });
    }
  }));
  ...
  reg('unode.exportRun', () => guard(async () => {
    ...
    if (saved) {
      void vscode.window.showInformationMessage(runEvidenceExportConfirmation(run), { modal: true });
    }
  }));
  ...
  reg('unode.exportPortableRun', () => guard(async () => {
    ...
    if (saved) {
      void vscode.window.showInformationMessage(
        ...
        + 'timestamps, and validated SHA-256 digests, which its "retained" section declares.', { modal: true }
      );
    }
  }));
  ...
  reg('unode.importMessages', () => guard(async () => {
    ...
    vscode.window.showInformationMessage(
      `Imported ${parsed.messages.length} message(s) into the activity feed for viewing (not restored to history — cleared on reload).`, { modal: true }
    );
  }));
  ...
  reg('unode.viewArchivedChats', () => guard(async () => {
    ...
    vscode.window.showInformationMessage(`Restored the archived chat with ${pick.label}.`, { modal: true });
  }));
  ...
  reg('unode.saveTeam', () => guard(async () => {
    ...
    void vscode.window.showInformationMessage(
      scope === 'workspace'
        ...
        : `Saved "${label}" to your personal team library. Bring it back with "UnodeAi: Open Saved Team…".`, { modal: true }
    );
  }));
  ...
  reg('unode.openSavedTeam', () => guard(async () => {
    ...
    void vscode.window.showInformationMessage(
      ...
      + 'The team you were on was saved first.', { modal: true }
    );
  }));
  ...
  reg('unode.openSettings', (tab?: unknown) => {
    SettingsPanel.createOrShow(context.extensionUri, {
      ...
      getAllowCrossProviderDispatch: () => vscode.workspace.getConfiguration('unode')
        .get<boolean>(ALLOW_CROSS_PROVIDER_DISPATCH_SETTING, ALLOW_CROSS_PROVIDER_DISPATCH_DEFAULT) === true,
...
async function guidedAddMcpServer(): Promise<void> {
  ...
  if (res.ok) { vscode.window.showInformationMessage(`UnodeAi: ${res.message}`, { modal: true }); }
  else { vscode.window.showWarningMessage(`UnodeAi: ${res.message}`, { modal: true }); }
}
...
async function runDemoTask(taskId?: string): Promise<void> {
  ...
  vscode.window.showInformationMessage(`Sent "${task.title}" to ${pm.config.name}.`, { modal: true });
}
...
async function pickAgentBuilderIcon(): Promise<string | undefined> {
  ...
  if (bytes.byteLength > MAX_AGENT_ICON_BYTES) {
    void vscode.window.showWarningMessage('Use a small icon under 64 KB', { modal: true });
    return undefined;
  }

  const mime = mimeForAgentIcon(uri.fsPath);
  if (!mime) {
    void vscode.window.showWarningMessage('Choose a PNG, JPEG, WebP, or SVG icon.', { modal: true });
    return undefined;
  }
