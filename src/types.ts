...
export interface AgentConfig {
  ...
  color?: string;
  autoApprove: boolean;
  /** Host-owned Codex Act permission preset. Missing or malformed values always resolve downward to Ask. */
  codexPermissionProfile?: CodexActPermissionProfile;
  /** Optional per-agent tool ceiling. Absent means inherit/unrestricted; [] is retained for legacy rosters. */
  allowedTools?: string[];
  /**
   * Explicit meaning of `allowedTools`. `native-default` keeps a native CLI's own tool set; `bounded`
   * treats the array as a real ceiling, including an intentionally empty/read-only ceiling. Legacy Codex
   * records with an empty array migrate to `native-default` when next saved.
   */
...
/** User-selectable Codex permission presets. Plan mode is always resolved separately to read-only. */
export type CodexActPermissionProfile = 'ask-for-approval' | 'approve-for-me' | 'full-access';
