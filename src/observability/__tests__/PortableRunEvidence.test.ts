...
describe('Portable Run Evidence carries no prose', () => {
  ...
  it('keeps the structured accounting the schema asks for', () => {
    ...
    expect(pack.version).toBe('portable-run-evidence/4');
    expect(pack.accounting).toEqual({
      dispatched: 1, settled: 1, cancelled: 0, interrupted: 0, refusedBeforeDispatch: 1, noExecutor: 0, dispositionsByKind: { accepted: 1 },
    });
  ...
  it('exports interrupted as a terminal state with separate mechanical accounting under schema v4', () => {
    const run = runWithEverything();
    run.delegations[0].state = 'interrupted';
    run.delegations[0].evidence = undefined;
    run.delegations[0].interruption = {
      reason: 'host-restarted',
      lastObservedAt: '2026-08-19T10:18:00.000Z',
      detectedAt: '2026-08-19T10:20:00.000Z',
    };

    const pack = buildPortableRunEvidence(run);
    expect(pack.version).toBe('portable-run-evidence/4');
    expect(pack.accounting).toMatchObject({ settled: 0, cancelled: 0, interrupted: 1 });
    expect(pack.delegations[0].state).toBe('interrupted');
  });
