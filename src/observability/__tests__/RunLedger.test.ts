...
describe('RunLedger', () => {
  ...
  it('closes a correlated run as partial only after every delegation is terminal and projects that fact', () => {
    ...
    const portable = buildPortableRunEvidence(run);
    expect(portable).toMatchObject({ version: 'portable-run-evidence/4', closeoutCompletionState: 'partial' });
  });
...
describe('v0.9.70 review and policy receipts', () => {
  it('round-trips a v8 partial closeout through the field whitelist', () => {
    ...
    expect(restored).toMatchObject({ schemaVersion: 8, status: 'closed', closeoutCompletionState: 'partial' });
  });
  ...
  it('migrates a v6 receipt snapshot and old outcome without losing the delegation', () => {
    ...
    expect(restored).toMatchObject({ schemaVersion: 8, closeoutCompletionState: 'complete' });
  ...
  it('normalizes both v1 and v8 stored rows to the same total in-memory arrays', () => {
    ...
    const restored = new RunLedger([v1, v8]).snapshot();
    expect(RUN_SCHEMA_VERSIONS).toEqual([1, 2, 3, 4, 5, 6, 7, 8]);
    ...
    const unsupported = { ...v8, schemaVersion: 9 } as unknown as StoredRunRecord;
