...
export const RUN_OWNER_LEASE_MS = 30_000;
...
export const RUN_SCHEMA_VERSIONS = [1, 2, 3, 4, 5, 6, 7, 8] as const;
...
/** Identifies one editor window and one extension-host lifetime inside that window. */
export interface RunHostIdentity {
  hostInstanceId: string;
  epoch: string;
}

/** A renewable lease prevents one editor window from adopting another window's live worker. */
export interface RunDelegationOwner extends RunHostIdentity {
  heartbeatAt: string;
}
...
export type RunOwnershipReconciliationBranch =
  | 'owner-missing'
  | 'same-host-current-epoch'
  | 'same-host-older-epoch'
  | 'foreign-owner-lease-active'
  | 'foreign-owner-lease-expired';

export interface RunOwnershipReconciliationDecision {
  handle: string;
  branch: RunOwnershipReconciliationBranch;
  action: 'keep-active' | 'interrupt';
  owner?: RunDelegationOwner;
...
export interface RunDelegation {
  ...
  state: 'active' | 'settled' | 'cancelled' | 'interrupted';
  settledAt?: string;
  cancelledAt?: string;
  cancellationReason?: string;
  /** Detection is not presented as the unknown moment work stopped. */
  interruption?: {
    reason: 'host-restarted' | 'worker-lost';
    lastObservedAt: string;
    detectedAt: string;
  };
  /** Process ownership is internal coordination data and is omitted from portable evidence. */
  owner?: RunDelegationOwner;
...
export interface StoredRunRecord {
  /** v2 correlation; v3 route/diff; v4 verdicts; v5 delivery; v6 reviews; v7 completion/read receipts; v8 interruption/ownership. */
...
export class RunLedger {
  ...
  private readonly host?: RunHostIdentity;
  ...
  constructor(records: readonly unknown[] = [], options: RunLedgerOptions = {}) {
    this.records = records.map(normalizeRun).filter((record): record is RunRecord => !!record).slice(-RUN_HISTORY_LIMIT);
    this.host = sanitizeHostIdentity(options.host);
    this.ownerLeaseMs = Number.isFinite(options.ownerLeaseMs) && Number(options.ownerLeaseMs) > 0
      ? Number(options.ownerLeaseMs)
      : RUN_OWNER_LEASE_MS;
  }

  /** A new extension host cannot resume process-private worker promises from persisted active rows. */
  reconcileRestoredActiveDelegations(
    detectedAt = new Date().toISOString(),
    observeDecision?: (decision: RunOwnershipReconciliationDecision) => void,
  ): DelegationInterruptionEvent[] {
    const events: DelegationInterruptionEvent[] = [];
    for (const run of this.records) {
      for (const delegation of run.delegations) {
        if (delegation.state !== 'active') continue;
        const decision = this.reconciliationDecision(delegation, detectedAt);
        ...
        const event: DelegationInterruptionEvent = {
          ...
          ...(delegation.attemptId ? { attemptId: delegation.attemptId } : {}),
          reason: 'host-restarted',
          lastObservedAt: delegation.progress?.lastMaterialProgressAt ?? delegation.dispatchedAt,
          detectedAt,
        };
        this.applyInterruption(delegation, event);
        events.push(event);
      }
    }
    if (events.length > 0) this.emit();
    return events;
  }

  /** Renew only this exact extension-host epoch; another window's lease is never touched. */
  recordHostHeartbeat(heartbeatAt = new Date().toISOString()): number {
    if (!this.host || !Number.isFinite(Date.parse(heartbeatAt))) return 0;
    let renewed = 0;
    for (const run of this.records) {
      for (const delegation of run.delegations) {
        if (delegation.state !== 'active' || !sameHost(delegation.owner, this.host)) continue;
        delegation.owner = { ...this.host, heartbeatAt };
        renewed++;
      }
    }
    if (renewed > 0) this.emit();
    return renewed;
  }

  /** Settle every active delegation owned by a worker that stopped without a terminal result. */
  recordWorkerInterrupted(
    agentId: string,
    reason: 'worker-lost' = 'worker-lost',
    detectedAt = new Date().toISOString(),
  ): DelegationInterruptionEvent[] {
    const events: DelegationInterruptionEvent[] = [];
    for (const run of this.records) {
      for (const delegation of run.delegations) {
        if (delegation.agentId !== agentId || delegation.state !== 'active') continue;
        const event: DelegationInterruptionEvent = {
          coordinatorId: run.coordinatorId,
          handle: delegation.handle,
          agentId,
  ...
  /** Whether this in-memory view contains any lease that needs renewal or observation. */
  hasActiveDelegations(): boolean {
    return this.records.some((run) => run.delegations.some((delegation) => delegation.state === 'active'));
  }

  /**
   * Merge the freshest shared snapshot before a whole-array workspaceState write. Locally-owned rows
   * remain authoritative; a stale window can never replace another host's active row with its older copy.
   */
  snapshotForPersistence(latestPersisted: readonly unknown[]): RunRecord[] {
    const external = latestPersisted
      .map(normalizeRun)
      .filter((record): record is RunRecord => !!record);
    const localById = new Map(this.records.map((run) => [run.id, run]));
    for (const externalRun of external) {
      const localRun = localById.get(externalRun.id);
      if (!localRun) {
        const restored = structuredClone(externalRun);
        this.records.push(restored);
        localById.set(restored.id, restored);
        continue;
      }
      mergeRunFromSharedSnapshot(localRun, externalRun, this.host);
    }
    this.records.sort((left, right) => left.startedAt.localeCompare(right.startedAt));
    ...
    if ((message.to === 'user' || message.to === 'unode')
      ...
      for (const run of affected.filter((candidate) => candidate.coordinatorId === message.from)) {
        if (run.delegations.every((delegation) => delegation.state !== 'active')) {
          ...
          run.closeoutCompletionState = message.type === 'task.partial'
  ...
  recordDelegationDispatched(event: {
    ...
    const dispatchedAt = event.dispatchedAt ?? new Date().toISOString();
    ...
    if (!delegation) {
      delegation = {
        ...
        ...(routing ? { routing } : {}),
        state: 'active',
        ...(this.host ? { owner: { ...this.host, heartbeatAt: dispatchedAt } } : {}),
        dispositions: [],
      };
      run.delegations.push(delegation);
    } else if (delegation.state === 'active' && this.host) {
      delegation.owner = { ...this.host, heartbeatAt: dispatchedAt };
    }
    ...
    if (event.replacesHandle && event.replacementReason && event.replacesHandle !== delegation.handle) {
      const original = this.findCoordinatorDelegation(event.coordinatorId, event.replacesHandle)?.delegation;
      if (original?.state === 'interrupted' && !original.dispositions.some((entry) =>
        entry.disposition === 'superseded' && entry.replacementHandle === delegation!.handle)) {
        original.dispositions.push({
          handle: original.handle,
          agentId: original.agentId,
          disposition: 'superseded',
          reason: redactEvidenceText(event.replacementReason),
          replacementHandle: delegation.handle,
  ...
  recordDelegationEvidence(event: {
    ...
    if (located.delegation.state === 'cancelled' || located.delegation.state === 'interrupted') {
      return;
    }
  ...
  recordDelegationProgress(event: { handle: string; agentId: string; progress: WorkerTaskProgressRecord }): void {
    ...
    if (located.delegation.state !== 'active') {
      return;
    }
  ...
  recordDelegationInterrupted(event: DelegationInterruptionEvent): boolean {
    const located = this.findDelegation(event.handle);
    if (!located || located.run.coordinatorId !== event.coordinatorId || located.delegation.agentId !== event.agentId ||
        located.delegation.state !== 'active') {
      return false;
    }
    this.applyInterruption(located.delegation, event);
    this.emit();
    return true;
  }
  ...
  recordDelegationCancelled(event: DelegationCancellationEvent): void {
    const located = this.findDelegation(event.handle);
    if (!located || located.delegation.state === 'settled' || located.delegation.state === 'interrupted') {
      return;
    }
  ...
  recordDisposition(event: DelegationDispositionEvent): void {
    ...
    if (located.delegation.state === 'interrupted' && event.disposition !== 'superseded' && event.disposition !== 'abandoned') {
      return;
    }
    const terminal = located.delegation.dispositions.find((entry) =>
      entry.disposition === 'superseded' || entry.disposition === 'abandoned');
    if (terminal) return;
    if (located.delegation.state === 'interrupted') {
      const duplicate = located.delegation.dispositions.some((entry) =>
        entry.disposition === event.disposition && entry.replacementHandle === event.replacementHandle);
      if (duplicate) return;
    }
  ...
  private applyInterruption(delegation: RunDelegation, event: DelegationInterruptionEvent): void {
  ...
  private reconciliationDecision(
    ...
    detectedAt: string,
  ): RunOwnershipReconciliationDecision {
    const owner = delegation.owner;
    if (!this.host || !owner) {
      return { handle: delegation.handle, branch: 'owner-missing', action: 'interrupt' };
    }
    if (owner.hostInstanceId === this.host.hostInstanceId) {
      return {
        handle: delegation.handle,
        branch: owner.epoch === this.host.epoch ? 'same-host-current-epoch' : 'same-host-older-epoch',
        action: owner.epoch === this.host.epoch ? 'keep-active' : 'interrupt',
        owner: structuredClone(owner),
      };
    }
    const heartbeat = Date.parse(owner.heartbeatAt);
    const detected = Date.parse(detectedAt);
    const heartbeatAgeMs = Number.isFinite(heartbeat) && Number.isFinite(detected)
      ? Math.max(0, detected - heartbeat)
      : this.ownerLeaseMs;
    const expired = heartbeatAgeMs >= this.ownerLeaseMs;
    return {
      handle: delegation.handle,
      branch: expired ? 'foreign-owner-lease-expired' : 'foreign-owner-lease-active',
      action: expired ? 'interrupt' : 'keep-active',
      owner: structuredClone(owner),
      heartbeatAgeMs,
    };
  }
...
function normalizeDelegation(value: RunDelegation): RunDelegation | undefined {
  if (!value || typeof value.handle !== 'string' || typeof value.agentId !== 'string' ||
      value.state !== 'active' && value.state !== 'settled' && value.state !== 'cancelled' && value.state !== 'interrupted') {
    return undefined;
  }
  const {
    ...
    interruption: untrustedInterruption,
    owner: untrustedOwner,
    ...rest
  } = value;
  ...
  const interruption = sanitizeInterruption(untrustedInterruption);
  const owner = sanitizeDelegationOwner(untrustedOwner);
  if (value.state === 'interrupted' && !interruption) return undefined;
  return {
    ...rest,
    ...
    ...(value.state === 'interrupted' && interruption ? { interruption } : {}),
    ...(owner ? { owner } : {}),
...
function sanitizeHostIdentity(value: RunHostIdentity | undefined): RunHostIdentity | undefined {
  if (!value || typeof value.hostInstanceId !== 'string' || !value.hostInstanceId.trim()
      || typeof value.epoch !== 'string' || !value.epoch.trim()) {
    return undefined;
  }
  return {
    hostInstanceId: value.hostInstanceId.trim().slice(0, 200),
    epoch: value.epoch.trim().slice(0, 200),
  };
}

function sanitizeDelegationOwner(value: RunDelegationOwner | unknown): RunDelegationOwner | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const candidate = value as Record<string, unknown>;
  const identity = sanitizeHostIdentity({
    hostInstanceId: typeof candidate.hostInstanceId === 'string' ? candidate.hostInstanceId : '',
    epoch: typeof candidate.epoch === 'string' ? candidate.epoch : '',
  });
  if (!identity || typeof candidate.heartbeatAt !== 'string' || !Number.isFinite(Date.parse(candidate.heartbeatAt))) {
    return undefined;
  }
  return { ...identity, heartbeatAt: candidate.heartbeatAt };
}

function sameHost(owner: RunDelegationOwner | undefined, host: RunHostIdentity): boolean {
  return owner?.hostInstanceId === host.hostInstanceId && owner.epoch === host.epoch;
}

function mergeRunFromSharedSnapshot(
  local: RunRecord,
  external: RunRecord,
  host: RunHostIdentity | undefined,
): void {
  const localByHandle = new Map(local.delegations.map((delegation, index) => [delegation.handle, index]));
  for (const externalDelegation of external.delegations) {
    const localIndex = localByHandle.get(externalDelegation.handle);
    if (localIndex === undefined) {
      local.delegations.push(structuredClone(externalDelegation));
      continue;
    }
    const localDelegation = local.delegations[localIndex];
    const mergedDispositions = mergeAppendOnly(
      localDelegation.dispositions,
      externalDelegation.dispositions,
      (entry) => JSON.stringify(entry),
    );
    const selected = preferExternalDelegation(localDelegation, externalDelegation, host)
      ? structuredClone(externalDelegation)
      : localDelegation;
    selected.dispositions = mergedDispositions;
    local.delegations[localIndex] = selected;
  }

  local.correlationIds = [...new Set([...local.correlationIds, ...external.correlationIds])];
  local.refusedDispatches = mergeAppendOnly(local.refusedDispatches, external.refusedDispatches, (entry) => JSON.stringify(entry));
  local.permissions = mergeAppendOnly(local.permissions, external.permissions, (entry) => JSON.stringify(entry));
  local.outcomeRepairs = mergeAppendOnly(local.outcomeRepairs, external.outcomeRepairs, (entry) => JSON.stringify(entry));
  local.verdicts = mergeAppendOnly(local.verdicts, external.verdicts, (entry) => JSON.stringify(entry));
  local.contextReceipts = mergeAppendOnly(local.contextReceipts, external.contextReceipts, (entry) => JSON.stringify(entry));
  local.contentReceipts = mergeAppendOnly(local.contentReceipts, external.contentReceipts, (entry) => JSON.stringify(entry));
  local.reviewObservations = mergeAppendOnly(local.reviewObservations, external.reviewObservations, (entry) => JSON.stringify(entry));
  local.activity = mergeAppendOnly(local.activity, external.activity, (entry) => JSON.stringify(entry))
    .sort((left, right) => left.timestamp.localeCompare(right.timestamp))
    .slice(-RUN_ACTIVITY_RETAINED_LIMIT);
  local.droppedActivityItems = Math.max(local.droppedActivityItems, external.droppedActivityItems);

  const localOwnsRun = !!host && local.delegations.some((delegation) => sameHost(delegation.owner, host));
  if (!localOwnsRun && runLifecycleTimestamp(external) > runLifecycleTimestamp(local)) {
    local.status = external.status;
    local.endedAt = external.endedAt;
    local.closeoutCompletionState = external.closeoutCompletionState;
  }
}

function preferExternalDelegation(
  local: RunDelegation,
  external: RunDelegation,
  host: RunHostIdentity | undefined,
): boolean {
  // An observer can mark a sleeping owner's row interrupted just before that owner resumes. A later
  // heartbeat from the same exact owner is proof that the observer's host-restarted conclusion is stale.
  if (activeOwnerOutlivedHostInterruption(external, local)) return true;
  if (activeOwnerOutlivedHostInterruption(local, external)) return false;
  if (host) {
    const localIsOwned = sameHost(local.owner, host);
    const externalIsOwned = sameHost(external.owner, host);
    if (localIsOwned !== externalIsOwned) return externalIsOwned;
    if (localIsOwned) {
      // Ordinarily this process's own shard is authoritative. A dispositioned interruption is the
      // exception: the coordinator decision is causally newer than the active snapshot it superseded.
      if (local.state === 'active'
          && external.state === 'interrupted'
          && external.dispositions.length !== 0) {
        return true;
      }
      return false;
    }
  }
  if (local.state !== 'active' && external.state === 'active') return false;
  if (local.state === 'active' && external.state !== 'active') return true;
  return delegationLifecycleTimestamp(external) > delegationLifecycleTimestamp(local);
}

function activeOwnerOutlivedHostInterruption(
  active: RunDelegation,
  interrupted: RunDelegation,
): boolean {
  if (active.state !== 'active'
      || interrupted.state !== 'interrupted'
      || interrupted.interruption?.reason !== 'host-restarted'
      || interrupted.dispositions.length !== 0
      || !active.owner
      || !interrupted.owner
      || active.owner.hostInstanceId !== interrupted.owner.hostInstanceId
      || active.owner.epoch !== interrupted.owner.epoch) {
    return false;
  }
  return Date.parse(active.owner.heartbeatAt) > Date.parse(interrupted.interruption.detectedAt);
}

function delegationLifecycleTimestamp(delegation: RunDelegation): string {
  return delegation.interruption?.detectedAt
    ?? delegation.cancelledAt
    ?? delegation.settledAt
    ?? delegation.owner?.heartbeatAt
    ?? delegation.dispatchedAt;
}

function runLifecycleTimestamp(run: RunRecord): string {
  return run.endedAt
    ?? run.delegations.map(delegationLifecycleTimestamp).sort().at(-1)
    ?? run.startedAt;
}

function mergeAppendOnly<T>(local: readonly T[], external: readonly T[], key: (entry: T) => string): T[] {
  const result = local.map((entry) => structuredClone(entry));
...
function sanitizeInterruption(value: RunDelegation['interruption'] | unknown): RunDelegation['interruption'] | undefined {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return undefined;
  const candidate = value as Record<string, unknown>;
  if ((candidate.reason !== 'host-restarted' && candidate.reason !== 'worker-lost') ||
      typeof candidate.lastObservedAt !== 'string' || !Number.isFinite(Date.parse(candidate.lastObservedAt)) ||
      typeof candidate.detectedAt !== 'string' || !Number.isFinite(Date.parse(candidate.detectedAt))) {
    return undefined;
  }
  return {
...
function projectCoordinatorTaskStatus(run: RunRecord, delegation: RunDelegation): CoordinatorTaskStatus {
  ...
  // Terminal coordinator decisions are immutable. Prefer the first one so a legacy ledger that already
  // contains the F7-16 contradiction still reports the original truth instead of the later invalid append.
  const latestDisposition = delegation.dispositions.find((entry) =>
    entry.disposition === 'superseded' || entry.disposition === 'abandoned')
    ?? delegation.dispositions[delegation.dispositions.length - 1];
  ...
  return {
    ...
    ...(delegation.state === 'settled'
      ...
      : delegation.state === 'cancelled' ? { terminalAt: delegation.cancelledAt }
        : delegation.state === 'interrupted' ? { terminalAt: delegation.interruption?.detectedAt } : {}),
    ...(delegation.state === 'interrupted' && delegation.interruption
