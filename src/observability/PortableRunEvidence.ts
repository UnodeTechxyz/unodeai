...
export const PORTABLE_RUN_EVIDENCE_VERSION = 'portable-run-evidence/4' as const;
...
export const PORTABLE_RETAINED: readonly PortableOmission[] = Object.freeze([
  ...
  },
  {
    ...
    reason: 'Bounded content facts only: rich-content class/extraction coverage or an opaque local-scope ordinal and count. No source path, query, provider, or content is retained.',
  },
]);
...
export interface PortableContentReceipt {
  /** Opaque `content-N`, `own-conversation`, or `local-scope-N`; never a source name or path. */
  ordinal: string;
  contentClass: 'pdf' | 'image' | 'conversation' | 'local-scope';
  action: 'stored' | 'read' | 'searched' | 'sent' | 'refused' | 'omitted' | 'file-read' | 'directory-list' | 'tree-scan' | 'git-tag-inspection';
  ...
  /** Local-scope-only bounded fact; this is an opaque ordinal, never a path or path fragment. */
  count?: number;
}
...
export const PORTABLE_SCHEMA_FIELDS: Readonly<Record<string, readonly string[]>> = Object.freeze({
  ...
  accounting: Object.freeze([
    'dispatched', 'settled', 'cancelled', 'interrupted', 'refusedBeforeDispatch', 'noExecutor', 'dispositionsByKind',
  ]),
...
export function buildPortableRunEvidence(
  ...
  const content = run.contentReceipts.flatMap((receipt): PortableContentReceipt[] => {
    ...
    if (!safe) { return []; }
    return safe.contentClass === 'local-scope'
      ? [{
        ordinal: safe.rootId,
      ...
      : safe.contentClass === 'conversation'
      ? [{
  ...
  return {
    ...
    accounting: {
      ...
      interrupted: run.delegations.filter((d) => d.state === 'interrupted').length,
