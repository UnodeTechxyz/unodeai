...
export function deriveRunMechanicalAccounting(run: RunRecord): RunMechanicalAccounting {
  return {
    ...
    interrupted: run.delegations.filter((delegation) => delegation.state === 'interrupted').length,
...
export function renderRunEvidencePack(run: RunRecord, exportedAt = new Date().toISOString()): string {
  ...
  const lines = [
    ...
    run.status === 'open'
      ...
      : run.closeoutCompletionState === 'partial'
        ? 'This run closed when its coordinator ended a correlated turn with unfinished or interrupted work and no live delegation left. Closure records ownership finalization, not completion or acceptance.'
    ...
    ...(accounting.interrupted > 0
      ? [`- Interrupted delegation handles: ${run.delegations
        .filter((delegation) => delegation.state === 'interrupted')
        .map((delegation) => `\`${escapeCode(delegation.handle)}\``)
        .join(', ')}.`]
      : []),
  ...
  for (const delegation of run.delegations) {
    ...
    if (delegation.state === 'interrupted' && delegation.interruption) {
      lines.push(
        `- Interruption: **${escapeText(delegation.interruption.reason)}**; last observed ${escapeText(delegation.interruption.lastObservedAt)}; detected ${escapeText(delegation.interruption.detectedAt)}.`,
      );
    }
    if (evidence) {
  ...
  if (run.contextReceipts.length === 0) {
    ...
    for (const receipt of run.contextReceipts) {
      ...
      for (const entry of receipt.entries) {
        ...
        const memoryTrust = entry.memoryTrustCounts
          ? `; selected rows: ${entry.memoryTrustCounts.untrusted} untrusted, ${entry.memoryTrustCounts.humanAttested} human-attested`
          : '';
        lines.push(`- ${escapeText(entry.kind)}: ${escapeText(entry.label)} (${entry.bytes} bytes; ${escapeText(entry.staleness)}${sensitivity}${memoryTrust}).`);
      }
      lines.push('');
    }
  }
  ...
  if (contentReceipts.length === 0) {
    ...
    for (const receipt of contentReceipts) {
      if (receipt.contentClass === 'local-scope') {
        lines.push(
          `- ${escapeCode(receipt.rootId)} | local scope | ${receipt.action} | ${receipt.count} ${receipt.count === 1 ? 'item' : 'items'}; no path, query, or content retained.`,
        );
        continue;
      }
