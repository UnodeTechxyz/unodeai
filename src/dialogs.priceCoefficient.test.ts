...
vi.mock('vscode', () => ({
  ...
  workspace: {
    getConfiguration: () => ({
      get: () => stored,
      inspect: () => ({ globalValue: stored, defaultValue: {} }),
...
describe('price coefficient input', () => {
  ...
  /**
   * The behaviour above was correct and the manual said the opposite for 21 releases.
   *
   * 2999a45 (2026-08-21) fixed "blank means free" by REFUSING blank, and documented that accurately.
   * f9a6f72 (v0.9.58, the next day) changed blank to mean list price — so the empty initial box could stay,
   * which is what stops an Enter bleeding through from the key prompt — and left USAGE.md and both wiki
   * pages describing the refusal that no longer existed. Nothing caught it: `check:docs` compares version
   * stamps, not claims, and a behavioural sentence in the manual has no gate of its own.
   *
   * This ties the sentence to the behaviour in the suite that owns the rule. It is deliberately narrow —
   * it guards this claim, not the manual in general.
   */
  it('is described accurately by every shipped document', () => {
    expect(parsePriceCoefficient('')).toEqual({ ok: true, value: 1 });
    const docs = [
      'USAGE.md',
      join('docs', 'wiki', 'index.html'),
      join('docs', 'wiki', 'README.md'),
    ];
    for (const relative of docs) {
      const text = readFileSync(resolve(process.cwd(), relative), 'utf8');
      expect(text, `${relative} must not claim an empty coefficient box is refused`)
        .not.toMatch(/empty box is refused/i);
    }
  });
  ...
  it('refuses what is not a coefficient', async () => {
    const { validate } = await runPrompt(undefined);
    // Assert the refusal states the accepted range, not one particular wording: the previous phrasing
    // ("0 or a positive number, e.g. 1, 0.33, or 0") named 0 twice and read as a contradiction to a
    // reader who had just typed 0.
    expect(validate('half')).toMatch(/between 0 and 1/);
    expect(validate('-1')).toMatch(/between 0 and 1/);
    expect(validate('NaN')).toMatch(/between 0 and 1/);
    expect(validate('1.5')).toMatch(/cannot be above 1/);
  });
