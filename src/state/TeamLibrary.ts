...
/** Explain the scope-specific effect before deleting a manual saved team. */
export function savedTeamDeleteConfirmation(ref: TeamLibraryRef, label: string): string {
  if (ref.scope === 'workspace') {
    return `Delete "${label}" from This project? This deletes .unode/teams/${ref.slug}.json from this project. `
      + 'Anyone who gets that file through Git will lose this saved team too. Your running team is unaffected.';
  }
  return `Delete "${label}" from All projects? This removes it only from your personal team library. `
    + 'Your running team is unaffected.';
}
