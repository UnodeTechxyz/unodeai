...
describe('deleting a saved team', () => {
  it('names the scope, sharing consequence, and unchanged running team before deletion', () => {
    const project = savedTeamDeleteConfirmation({ scope: 'workspace', slug: 'review-crew' }, 'Review Crew');
    expect(project).toContain('This project');
    expect(project).toContain('.unode/teams/review-crew.json');
    expect(project).toContain('through Git');
    expect(project).toContain('running team is unaffected');

    const personal = savedTeamDeleteConfirmation({ scope: 'global', slug: 'review-crew' }, 'Review Crew');
    expect(personal).toContain('All projects');
    expect(personal).toContain('personal team library');
    expect(personal).toContain('running team is unaffected');
    expect(personal).not.toContain('.unode/teams/');
  });
});
