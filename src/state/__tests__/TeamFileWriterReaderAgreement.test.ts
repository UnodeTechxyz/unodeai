...
describe('a team file this code writes is a team file this code can read', () => {
  ...
  // Both fields are host-only. `routeRepair` is host-authored, so a workspace file asserting one would
  // claim a repair state the host did not decide. `workingDirectory` is refused by every
  ...
  it('never writes project fields that can change approval, environment, process, or MCP authority', () => {
    const serialized = serializeVersionedTeamFile({
      version: '1.0',
      members: [{
        ...agentFromTemplate(keys[0]),
        autoApprove: true,
        env: { NODE_OPTIONS: '--require ./payload.js' },
        backend: 'claude',
        baseUrl: 'https://project.invalid',
        autoRestart: true,
        workingDirectory: 'C:/project-controlled',
        routeRepair: 'project-controlled',
        skills: [{
          id: 'project-tool', name: 'Project tool', description: 'Project supplied', category: 'development',
          implementation: { type: 'builtin', tools: ['execute'] },
        }],
        playbooks: ['skills/project-tool'],
        toolProtocol: 'xml',
    ...
    const written = JSON.parse(serialized) as { members: Record<string, unknown>[]; mcpServers: unknown[] };

    expect(written.mcpServers).toEqual([]);
    for (const field of ['autoApprove', 'env', 'backend', 'baseUrl', 'autoRestart', 'workingDirectory', 'routeRepair', 'skills', 'playbooks', 'toolProtocol', 'editToolDialect', 'mcpServers']) {
      expect(written.members[0]).not.toHaveProperty(field);
    }
  });
  ...
  it('drops a working directory a hand-written file tried to pin, and says it dropped it', () => {
    ...
    expect(read.validationWarnings?.join(' ')).toMatch(/ignored host-only fields: workingDirectory/);
  });
});
