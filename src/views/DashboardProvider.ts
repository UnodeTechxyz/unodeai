...
import { DELEGATION_STALLED_AFTER_MS, DelegationAgentState } from './orchestrationProgress';
...
export class DashboardProvider {
  ...
  async getDashboardHtml(webview: vscode.Webview): Promise<string> {
    ...
    return renderDashboardHtml(webview, {
      fontFallback: '-apple-system, sans-serif',
      styles: /* css */`
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .stat-running { color: var(--vscode-charts-green, var(--vscode-foreground)); }
    ...
    .lane-working .status-dot { background: var(--vscode-charts-green, var(--vscode-foreground)); }
    .lane-stalled .status-dot { background: var(--vscode-charts-yellow, var(--vscode-foreground)); }
    .lane-idle .status-dot { background: var(--vscode-charts-yellow, var(--vscode-foreground)); }
    /* Blue, not green. testing.iconPassed shares one value across dark, light and hc-dark, so the default
       light theme painted a dark-theme green on white at 2.00:1 — under the 3.00 floor. charts-blue is
       5.64-8.10 in all four bundled themes and also separates done from working. See FIELD_v0977_S1. */
    .lane-done .status-dot { background: var(--vscode-charts-blue, #3b82f6); }
    .lane-blocked .status-dot, .lane-error .status-dot { background: var(--vscode-testing-iconFailed, #dc3545); }
    .lane-stopped .status-dot { background: var(--vscode-descriptionForeground, var(--vscode-foreground)); }
    ...
    /* These badges sit inside .lane-board, whose ground is input-background - not the page ground. A 13%
       fill of the text's own colour lightened that ground enough to put the text under 4.50 in Dark Modern,
       so the fill is 6% and the shape comes from a border, which costs the text nothing. That is the
       construction .savings-banner already uses. The two neutral variants take no fill at all: they mean
       "no information", and their previous editor-background fill was the same colour as the board in
       three of the four bundled themes, so they had no shape there. Measured in FIELD_v0977_S1. */
    .verify-badge { display: inline-flex; max-width: 100%; padding: 3px 8px; border: 1px solid transparent; border-radius: 999px; font-size: 11px; font-weight: 800; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    /* Blue, not green: 11px bold text needs 4.50, which no green reaches in the light themes. Blue-vs-red
       is also a better pass/fail pair than green-vs-red. */
    .verify-passed { color: var(--vscode-charts-blue, #3b82f6); background: color-mix(in srgb, var(--vscode-charts-blue, #3b82f6) 6%, transparent); border-color: color-mix(in srgb, var(--vscode-charts-blue, #3b82f6) 40%, transparent); }
    .verify-failed { color: var(--vscode-testing-iconFailed, #dc3545); background: color-mix(in srgb, var(--vscode-testing-iconFailed, #dc3545) 6%, transparent); border-color: color-mix(in srgb, var(--vscode-testing-iconFailed, #dc3545) 40%, transparent); }
    .verify-skipped, .verify-missing { color: var(--vscode-descriptionForeground); border-color: color-mix(in srgb, var(--vscode-descriptionForeground) 40%, transparent); }
...
/** The Dashboard renderer is pure over the precomputed panel body and style; the shared shell owns its document security. */
export interface DashboardDocumentContent {
  fontFallback: string;
  styles: string;
  body: string;
}

export function renderDashboardHtml(webview: vscode.Webview, content: DashboardDocumentContent): string {
  return renderWebviewDocument(webview, {
    title: 'UnodeAi Dashboard',
    fontFallback: content.fontFallback,
...
function laneStatus(session: SessionInfo, progress?: DelegationAgentState): string {
  ...
  if (progress?.status) {
    if (progress.status === 'working' && Date.now() - Date.parse(progress.updatedAt) >= DELEGATION_STALLED_AFTER_MS) {
      return 'stalled';
    }
...
function taskText(session: SessionInfo, progress?: DelegationAgentState): { text: string; muted: boolean } {
  if (progress?.task) {
    const quietMs = Math.max(0, Date.now() - Date.parse(progress.updatedAt));
    const elapsedMs = Math.max(0, Date.now() - Date.parse(progress.startedAt));
    const timeline = progress.stopped
      ? `Stopped${progress.stopReason ? ` · ${progress.stopReason}` : ''}`
      : progress.status === 'working'
      ? progress.waitingOn
        ? `Waiting on ${progress.waitingOn} · step ${progress.stepCount} · ${durationMinutes(elapsedMs)}`
        : quietMs >= DELEGATION_STALLED_AFTER_MS
        ? `Stalled · no activity for ${durationMinutes(quietMs)}`
        : progress.liveState === 'assigned'
          ? `Assigned · ${durationMinutes(elapsedMs)}`
          : `Working · step ${progress.stepCount} · ${durationMinutes(elapsedMs)}`
      : undefined;
    return { text: timeline ? `${timeline} · ${progress.task}` : progress.task, muted: false };
  }
...
function durationMinutes(ms: number): string {
  return ms < 60_000 ? '<1 min' : `${Math.floor(ms / 60_000)} min`;
}
