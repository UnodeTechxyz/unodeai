...
describe('the builder opens on the decisions that are the user\'s', () => {
  ...
  it('keeps selected MCP grants visible outside the folded full server list', () => {
    const granted = renderAgentBuilderHtml({ cspSource: 'vscode-resource:' } as never, {
      ...view,
      mode: 'edit',
      agent: {
        id: 'a1', name: 'Granted Agent', role: 'senior-dev', roleKey: 'senior-dev', providerId: 'roam',
        model: 'deepseek-v4-pro', systemPrompt: 'Use the approved server.', skillIds: [], playbooks: [],
        mcpServers: ['github'],
      },
    });
    const fold = granted.indexOf('id="mcpAdvanced"');
    const selectedRow = granted.indexOf('data-mcp-id="github" checked');
    expect(granted).toContain('id="selectedMcpChecks"');
    expect(granted).toContain('Selected MCP servers (1)');
    expect(selectedRow).toBeGreaterThan(0);
    expect(selectedRow).toBeLessThan(fold);
    const mcpSection = granted.slice(granted.indexOf('id="mcpGrantsSection"'), granted.indexOf('<aside class="side">'));
    expect(mcpSection).not.toContain('usually left at connection default');

    const empty = html();
    expect(empty).toContain('Selected MCP servers (none)');
    expect(empty).toContain('No MCP servers granted yet. Open All MCP servers to add one.');
    expect(empty).toContain('syncMcpCards()');
  });
});

describe('renderAgentBuilderHtml', () => {
  it('sorts both role lists A-Z while keeping their fixed choices at the edges', () => {
    const roles = [
      { ...view.roles[0], id: 'zebra', name: 'Zebra' },
      { ...view.roles[0], id: 'agent-10', name: 'Agent 10' },
      { ...view.roles[0], id: 'alpha', name: 'alpha' },
      { ...view.roles[0], id: 'agent-9', name: 'Agent 9' },
    ];
    const html = renderAgentBuilderHtml({ cspSource: 'vscode-resource:' } as never, { ...view, roles });
    const optionLabels = (id: string) => {
      const body = html.match(new RegExp(`<select id="${id}"[^>]*>([\\s\\S]*?)</select>`))?.[1] ?? '';
      return [...body.matchAll(/<option[^>]*>([^<]*)<\/option>/g)].map((match) => match[1]);
    };
    const expected = roles.map((role) => role.name).sort(byDisplayName);
    const roleLabels = optionLabels('role');
    const filterLabels = optionLabels('roleFilter');

    expect(roleLabels[0]).toContain('Select a role');
    expect(roleLabels.slice(1, -1)).toEqual(expected);
    expect(roleLabels.at(-1)).toBe('Custom role');
    expect(filterLabels).toEqual(['All roles', ...expected]);
    expect(roles.map((role) => role.name)).toEqual(['Zebra', 'Agent 10', 'alpha', 'Agent 9']);
  });
  ...
  it('uses an explicit model listbox that opens unfiltered and filters only after typing', () => {
    const html = renderAgentBuilderHtml({ cspSource: 'vscode-resource:' } as never, view);
    expect(html).toContain('id="modelOptions" class="model-options" role="listbox"');
    expect(html).toContain('data-open-model-picker="model"');
    expect(html).not.toContain('list="modelOptions"');
    expect(html).not.toContain('<datalist');
    expect(html).toContain("input.addEventListener('focus', () => renderModelPicker(inputId, true))");
    expect(html).toContain("renderModelPicker('model', false)");
    expect(html).toContain("models.length + ' models · ' + modelSourceSummary(models)");
  });

  it('offers explicit Codex account-model loading when only the bundled default exists', () => {
    const html = renderAgentBuilderHtml({ cspSource: 'vscode-resource:' } as never, {
      ...view,
      defaultProviderId: 'codex',
      providers: [{
        ...view.providers[0],
        id: 'codex', connectionId: 'codex-cli', name: 'OpenAI Codex CLI',
        models: [{ id: 'codex-cli-default', name: 'Codex default', source: 'Bundled catalog' }],
        modelLoadAction: 'codex-cli',
      }],
    });
    expect(html).toContain('id="loadCodexModels"');
    expect(html).toContain('Load models from Codex');
    expect(html).toContain('Account models load from Codex.');
    expect(html).toContain("command: 'loadModels', providerId");
    expect(html).toContain("msg.command === 'modelsLoaded'");
    expect(html).toContain("msg.command === 'modelLoadFailed'");
  });
  ...
  it('keeps model catalogs and connection facts scoped to the selected connection', () => {
    ...
    expect(html).toContain('saving on this connection removes them');
    // Field feedback: the Load models button went unnoticed below Context window. It belongs directly under
    // the model dropdown it explains, before the Backup model field.
    const modelDropdown = html.indexOf('id="modelOptions"');
    const loadButton = html.indexOf('id="loadCodexModels"');
    const backupModel = html.indexOf('for="fallbackModel"');
    expect(modelDropdown).toBeGreaterThan(-1);
    expect(loadButton).toBeGreaterThan(modelDropdown);
    expect(loadButton).toBeLessThan(backupModel);
...
describe('catalogue lists render alphabetically', () => {
  ...
  it('orders every tool row by name', () => {
    const html = renderAgentBuilderHtml({ cspSource: 'vscode-resource:' } as never, {
      ...view,
      capabilities: [
        { id: 'testing', name: 'Testing', description: 'Write tests', category: 'development', requiredTools: ['read', 'write', 'execute'] },
        { id: 'code-generation', name: 'Code Generation', description: 'Write code', category: 'development', requiredTools: ['read', 'write'] },
      ],
    });
    const order = positionsIn(html, 'id="capabilityChecks"', ['Code Generation', 'Testing']);
    expect(order.every((at) => at > 0)).toBe(true);
    expect([...order].sort((a, b) => a - b)).toEqual(order);
    expect(html).toContain('Resulting permissions: read files, write files, run commands');
  });
});
