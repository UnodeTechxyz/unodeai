...
describe('SettingsPanel default provider', () => {
  ...
  it('does not load Smart models when a restored panel renders and requests only the focused provider', async () => {
    const listModels = vi.fn(async () => []);
    SettingsPanel.createOrShow({} as any, {
      bridge: { getSnapshot: async () => ({ providers: [], mcpServers: [] }) } as any,
      promptAndStoreSecret: async () => false,
      ...
      getSmartMode: () => ({
        ...
        roleTiers: {},
        taskTierHints: {},
        modelTiers: { premium: {}, standard: {}, economy: {} },
        providerIds: ['codex'],
      }),
    });
    await vi.waitFor(() => expect(state.panel.webview.html).toContain('requestedSmartModels'));
    expect(listModels).not.toHaveBeenCalled();
    expect(state.panel.webview.html).toContain("document.addEventListener('focusin'");
    expect(state.panel.webview.html).not.toContain('for (const p of smartProviders) vscode.postMessage');
  });

  it('offers a host-mediated Codex executable picker from Settings', async () => {
    const chooseCliExecutable = vi.fn(async () => {});
    SettingsPanel.createOrShow({} as any, {
      bridge: { getSnapshot: async () => ({
        providers: [{
          providerId: 'codex', connectionId: 'codex-cli', revision: 2, name: 'OpenAI Codex CLI',
          hasApiKey: false, canManageApiKey: false, authKind: 'codex-cli', catalogKind: 'codex-cli',
          billingKind: 'cli-account', availability: 'available',
          presentation: {
            runtimeLabel: 'OpenAI Codex CLI', billingLabel: 'Your OpenAI account',
            setup: { kind: 'cli', actionLabel: 'Set up Codex', loginCommand: 'codex login', requiredSetting: 'unode.codexCliPath' },
          },
        }],
        mcpServers: [],
      }) } as any,
    ...
    await vi.waitFor(() => expect(state.panel.webview.html).toContain('Choose Codex executable'));
    await state.onMessage?.({ command: 'chooseCliExecutable', providerId: 'codex' });
    expect(chooseCliExecutable).toHaveBeenCalledWith('codex');
  });
