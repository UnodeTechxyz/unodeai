...
describe('SessionPresentationModel', () => {
  ...
  it('drops selected-agent, transcript, and dispatch presentation state on workspace invalidation', () => {
    const model = new SessionPresentationModel();
    model.selectAgent('agent-1');
    model.replaceTranscript('agent-1', [{ role: 'user', text: 'root A', ts: '2026-09-19T00:00:00.000Z' }]);
    model.appendAgentMessage(coordination('root A dispatch', '2026-09-19T00:00:01.000Z'));

    model.reset();

    expect(model.selectedAgentId).toBe('');
    expect(model.transcript('agent-1')).toEqual([]);
    expect(model.agentMessages()).toEqual([]);
    expect(model.orderedEvents()).toEqual([]);
  });
