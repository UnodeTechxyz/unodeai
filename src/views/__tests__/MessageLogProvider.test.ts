...
describe('MessageLogProvider', () => {
  ...
  it('renders interruption facts before disposition and keeps Interrupted visible beside a replacement', () => {
    const bus = new MessageBus();
    const provider = new MessageLogProvider(bus);
    const view = makeView();
    provider.resolveWebviewView(view as never);
    const summary = {
      id: 'summary', coordinatorId: 'pm', coordinatorName: 'PM', startedAt: '2026-09-11T13:00:00.000Z',
      total: 1, done: 0, partial: 0, blocked: 1, working: 0,
      items: [{
        id: 'lost', coordinatorId: 'pm', coordinatorName: 'PM', agentId: 'dev', agentName: 'Developer',
        instruction: 'Build it.', scopeMode: 'fixed-session-permissions', status: 'interrupted',
    ...
    provider.setDelegationProgress([summary]);
    expect(view.webview.html).toContain('Interrupted — no active worker');
    expect(view.webview.html).toContain('Reason: host-restarted. Last observed: 2026-09-11T13:01:00.000Z. Detected: 2026-09-11T13:02:00.000Z.');
    expect(view.webview.html).not.toContain('Coordinator superseded task');

    provider.setDelegationProgress([{ ...summary, items: [{
      ...summary.items[0], coordinatorDisposition: 'superseded',
      dispositionReason: 'Replacement admitted.', replacementHandle: 'replacement-handle',
    }] }]);
    expect(view.webview.html).toContain('Interrupted — no active worker · Coordinator superseded task');
    expect(view.webview.html).toContain('Reason: host-restarted. Last observed: 2026-09-11T13:01:00.000Z. Detected: 2026-09-11T13:02:00.000Z.');
    expect(view.webview.html).toContain('Replacement: replacement-handle.');
    bus.dispose();
  });
