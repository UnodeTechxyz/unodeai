...
describe('run closeout presentation', () => {
  ...
  it('keeps every high-consequence extension surface wired to its explicit projection', () => {
    ...
    // The presentation style is user-selectable in v0.9.80. Pin the projection and the shared result policy.
    expect(source).toContain("showResultNotice('information', runEvidenceExportConfirmation(run))");
  });
});
