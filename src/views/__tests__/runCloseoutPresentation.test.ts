...
describe('run closeout presentation', () => {
  ...
  it('keeps every high-consequence extension surface wired to its explicit projection', () => {
    ...
    // v0.9.79 made this confirmation a centred modal, so an options argument now follows the message. The pin is
    // the wiring, not the presentation: the whole first argument must still be the partial/complete projection.
