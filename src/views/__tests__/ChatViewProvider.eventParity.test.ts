...
function emittedDelegationRenderer(html: string): (summary: unknown) => EmittedNode {
  const source = `const delegationStalledAfterMs = 120000;\n${emittedFunction(html, 'statusDot')}\n${emittedFunction(html, 'coordinatorDispositionLabel')}\n${emittedFunction(html, 'coordinatorDispositionTask')}\n${emittedFunction(html, 'interruptionStatusLabel')}\n${emittedFunction(html, 'interruptionTask')}\n${emittedFunction(html, 'delegationStatusLabel')}\n${emittedFunction(html, 'delegationMinutes')}\n${emittedFunction(html, 'delegationLiveStatus')}\n${emittedFunction(html, 'renderDelegation')}\nreturn renderDelegation;`;
  const document = {
    createElement: (): EmittedNode => {
      const node: EmittedNode = {
        className: '', title: '', textContent: '', dataset: {}, children: [],
...
describe('UX7 row 3 — each event kind reaches BOTH the sidebar and the Workbench', () => {
  ...
  it('labels quiet delegated work as stalled without adding retry or cancellation authority', () => {
    const h = harness();
    const render = emittedDelegationRenderer(h.workbenchHtml());
    const old = new Date(Date.now() - 3 * 60_000).toISOString();
    const text = visibleText(render({
      coordinatorName: 'PM', total: 1, done: 0, blocked: 0, working: 1,
      items: [{
        agentName: 'Reviewer', instruction: 'review the diff', status: 'working',
        startedAt: old, updatedAt: old, stepCount: 2,
      }],
    }));
    expect(text).toContain('Stalled');
    expect(text).toContain('no activity for 3 min');
    expect(text).not.toMatch(/Retry|Cancel/);
  });

  it('never labels a host-known approval wait as stalled', () => {
    const h = harness();
    const render = emittedDelegationRenderer(h.workbenchHtml());
    const old = new Date(Date.now() - 3 * 60_000).toISOString();
    const text = visibleText(render({
      coordinatorName: 'PM', total: 1, done: 0, blocked: 0, working: 1,
      items: [{
        agentName: 'Reviewer', instruction: 'apply the change', status: 'working',
        startedAt: old, updatedAt: old, stepCount: 2, waitingOn: 'your approval',
      }],
    }));
