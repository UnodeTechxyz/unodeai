...
export class WorktreePanel {
  ...
  private async render(): Promise<void> {
    try {
      ...
      this.review = EMPTY;
      void showResultNotice('warning', `UnodeAi: couldn't read worktree state — ${String(err)}`);
    }
  ...
  private onMessage(msg: { command?: unknown; agentId?: unknown; file?: unknown }): void {
    ...
    if (msg.command !== 'finalize') {
      ...
      void Promise.resolve(this.onLaneAction?.(action)).catch((err) => {
        void showResultNotice('error', `UnodeAi: lane action failed - ${String(err)}`);
      });
      return;
    }
    void this.onFinalize().then((result) => {
      ...
      void showResultNotice('error', `UnodeAi: finalize failed — ${String(err)}`);
    });
  }
}
