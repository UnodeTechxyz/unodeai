...
export class TeamViewProvider implements vscode.WebviewViewProvider {
  ...
  constructor(
    ...
    /** Effective Codex warning derived by the same host resolver as the backend/composer. */
    private readonly codexPermissionSummary: (config: SessionInfo['config']) => string | undefined = () => undefined,
  ) { }
  ...
  private scheduleStatusDecay(): void {
    ...
    const expiryTimes = [...this.delegationStates.values()]
      ...
      .map((state) => Date.parse(state.updatedAt) + DELEGATION_STALLED_AFTER_MS)
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .status-marker.status-coordinator-rejected, .status-marker.status-human-intervention-required, .status-marker.status-interrupted { color: var(--vscode-errorForeground, #dc3545); }
    ...
    .status-blocked, .status-interrupted { background: #dc354522; color: #dc3545; }
    ...
    .status-blocked .status-dot, .status-interrupted .status-dot { background: #dc3545; }
    ...
    .unsafe-badge { color: var(--vscode-errorForeground); font-size: 10px; font-weight: 700; }
    ...
    .compact-card.status-stalled .status-dot { background: var(--vscode-editorWarning-foreground, #ffc107); }
    ...
    .compact-card.status-blocked .status-dot, .compact-card.status-interrupted .status-dot { background: #dc3545; }
...
<body class="${compact ? 'compact' : ''}">
  ...
  <script nonce="${scriptNonce}">
    ...
    if (emptyState) {
      ...
      [
        ['createTeamPreset', 'Create/Save/Load a Team…', 'Pick a system default or manage one of your saved teams'],
  ...
  private _statusView(session: SessionInfo): { key: string; label: string; detail?: string } {
    ...
    if (delegated?.stopped && isRecent(delegated.updatedAt, 120000)) {
      return {
        key: 'stopped',
        label: 'Stopped',
        detail: delegated.stopReason
          ? `${delegated.task || 'delegated task'} stopped at ${delegated.stopReason}`
          : `${delegated.task || 'delegated task'} stopped at a host safety boundary`,
      };
    }
    if (delegated?.status === 'working') {
      const quietMs = Math.max(0, Date.now() - Date.parse(delegated.updatedAt));
      const elapsedMs = Math.max(0, Date.now() - Date.parse(delegated.startedAt));
      if (delegated.waitingOn) {
        return {
          key: 'working',
          label: `Waiting on ${delegated.waitingOn} · step ${delegated.stepCount} · ${durationMinutes(elapsedMs)}`,
          detail: `on ${delegated.task || 'delegated task'} for ${delegated.coordinatorName}`,
        };
      }
      if (quietMs >= DELEGATION_STALLED_AFTER_MS) {
        return {
          key: 'stalled',
          label: `Stalled · no activity for ${durationMinutes(quietMs)}`,
          detail: `${delegated.task || 'delegated task'} for ${delegated.coordinatorName}; no cancellation or retry was triggered`,
        };
      }
      return {
        key: 'working',
        label: delegated.busyCount && delegated.busyCount > 1
          ? `Working · ${delegated.busyCount} tasks`
          : delegated.liveState === 'assigned'
            ? `Assigned · ${durationMinutes(elapsedMs)}`
            : `Working · step ${delegated.stepCount} · ${durationMinutes(elapsedMs)}`,
        detail: `on ${delegated.task || 'delegated task'} for ${delegated.coordinatorName}`,
      };
    }
    ...
    if (delegated?.status === 'interrupted') {
      return {
        key: 'interrupted',
        label: 'Interrupted',
        detail: delegated.task || `no active worker remains for ${delegated.coordinatorName}`,
      };
    }
  ...
  private _renderSidebarRow(session: SessionInfo, changedFiles: ChangedFileSummary[] = []): string {
    ...
    const codexPermission = this.codexPermissionSummary(config);
      ...
      <div class="session-item status-${status.key} ${statusClass}" data-agent-id="${id}">
        <div class="session-row">
          <button class="session-open" type="button" data-command="${command}" data-agent-id="${id}"
            ...
            <span class="session-main">
              <span class="session-name">${esc(config.name)}${codexPermission ? ` <span class="unsafe-badge">${esc(codexPermission)}</span>` : ''}</span>
        ...
        <div class="session-detail">
          <div class="agent-details">
            ...
            ${codexPermission ? `<span class="unsafe-badge">${esc(codexPermission)}: no sandbox, network on, no approval prompts or checkpoints</span>` : ''}
    ...
    return /* html */`
      <div class="empty-state">
        ...
        <div class="empty-grid">
          <button class="empty-card" data-command="createTeamPreset">
            <span class="empty-title">Create/Save/Load a Team…</span>
            <span class="empty-copy">Pick a system default or manage one of your saved teams</span>
          </button>
