...
import type { LocalReadScopeConsentDecision } from '../backend/localReadScope';
import { ConsentGrant, ConsentGrantKind, MediaConsentKind } from '../security/ConsentGrants';
import type { RepositoryCliKind, RepositoryCliTrustRecord } from '../security/RepositoryCliConfig';
...
export interface SecuritySnapshot {
  ...
  /** Separate consent to launch the independently-networked OpenAI Codex CLI. */
  codexGrants?: ConsentGrant[];
  /**
  ...
  /** Session-only local project-discovery decisions. Absolute paths never leave this local webview. */
  localReadScopes?: LocalReadScopeConsentDecision[];
  repositoryCliGrants?: RepositoryCliTrustRecord[];
...
export interface SecurityPanelDeps {
  ...
  /** Host revalidates that the root is already a known declined decision before opening confirmation. */
  grantLocalReadScope: (absoluteRoot: string) => Promise<void>;
  /** Host revalidates that the root is already granted before revoking it. */
  revokeLocalReadScope: (absoluteRoot: string) => void;
  revokeRepositoryCliGrant: (kind: RepositoryCliKind, cwd: string, digest: string) => void;
  openSettings: () => void;
}
...
export class SecurityPanel {
  ...
  private onMessage(msg: {
    command?: string; host?: unknown; kind?: unknown; mediaKind?: unknown; root?: unknown;
    cwd?: unknown; digest?: unknown; profileId?: unknown; destination?: unknown; profileRevision?: unknown;
  }): void {
    if (msg?.command === 'revokeEgress' && typeof msg.host === 'string') {
      ...
      const kind: ConsentGrantKind = msg.kind === 'metadata'
        ? 'metadata'
        : msg.kind === 'media'
          ? 'media'
          : msg.kind === 'codex'
      ...
      const mediaKind: MediaConsentKind | undefined = msg.mediaKind === 'vision' || msg.mediaKind === 'transcription'
        ? msg.mediaKind
        : undefined;
      const modelScope = kind === 'model'
        && typeof msg.profileId === 'string'
        && typeof msg.destination === 'string'
        && typeof msg.profileRevision === 'number'
        ? { profileId: msg.profileId, destination: msg.destination, profileRevision: msg.profileRevision }
        : undefined;
      // A malformed media revoke must not broaden into a host-wide revocation.
      if (kind !== 'media' || mediaKind) {
        this.deps.revokeEgressHost(msg.host, kind, mediaKind, modelScope);
      }
      void this.render();
    } else if (msg?.command === 'grantLocalReadScope' && typeof msg.root === 'string') {
      void this.deps.grantLocalReadScope(msg.root).then(
        () => this.render(),
        () => this.render(),
      );
    } else if (msg?.command === 'revokeLocalReadScope' && typeof msg.root === 'string') {
      this.deps.revokeLocalReadScope(msg.root);
      void this.render();
    } else if (msg?.command === 'revokeRepositoryCliGrant'
      && (msg.kind === 'claude' || msg.kind === 'codex')
      && typeof msg.cwd === 'string' && typeof msg.digest === 'string') {
      this.deps.revokeRepositoryCliGrant(msg.kind, msg.cwd, msg.digest);
      void this.render();
...
export function renderSecurityHtml(
  ...
  displayNameForProviderId: (providerId: string) => string = (providerId) => providerId,
    ...
    const localReadList = s.localReadScopes?.length
      ? s.localReadScopes.map(({ absoluteRoot, status }) => {
        const action = status === 'granted' ? 'revokeLocalReadScope' : 'grantLocalReadScope';
        const label = status === 'granted' ? 'revoke' : 'grant';
        return `<li><code>${esc(absoluteRoot)}</code> ${status === 'granted' ? good('granted this session') : warn('declined')}`
          + ` <button class="link" data-local-root="${esc(absoluteRoot)}" data-local-action="${action}">${label}</button></li>`;
      }).join('')
      : '<li class="muted">No local project-discovery decision yet — the first attempted access asks you.</li>';

    const repositoryCliList = s.repositoryCliGrants?.length
      ? s.repositoryCliGrants.map((grant) => {
        const mode = grant.mode === 'native' ? good('Trust and load') : good('Project automation off');
        return `<li><code>${esc(grant.kind)}</code> ${mode}`
          + `<div class="provenance muted">${esc(grant.cwd)} · Digest ${esc(grant.digest.slice(0, 12))}… · Granted ${esc(grant.grantedAt)}</div>`
          + `<button class="link" data-cli-kind="${grant.kind}" data-cli-cwd="${esc(grant.cwd)}" data-cli-digest="${esc(grant.digest)}">revoke</button></li>`;
      }).join('')
      : '<li class="muted">No repository CLI configuration decision stored.</li>';
    ...
    const grants: Array<ConsentGrant & { kind: ConsentGrantKind; what: string }> = [
      ...
      ...(s.codexGrants ?? []).map((grant) => ({
        ...grant,
        kind: 'codex' as const,
        what: 'Codex CLI process → configured provider (OpenAI by default); does not grant ordinary OpenAI API access',
      })),
    ...
    const egressList = grants.length
      ? grants.map(({ host, kind, what, grantedAt, requester, mediaKind, profileId, destination, profileRevision, executablePath }) => {
        const displayedDestination = kind === 'codex' ? 'Codex CLI (configured provider)' : destination ?? host;
        return `<li><code>${esc(displayedDestination)}</code> <span class="muted">${what}</span> `
          + `<div class="provenance muted">${grantProvenance(grantedAt, requester)}`
          + `${kind === 'codex' ? ` · ${executablePath ? `Executable: ${esc(executablePath)}` : 'Legacy grant: no executable bound; next start asks again'}` : ''}</div>`
          + `<button class="link" data-revoke="${esc(host)}" data-kind="${kind}"`
          + `${mediaKind ? ` data-media-kind="${mediaKind}"` : ''}`
          + `${profileId ? ` data-profile-id="${esc(profileId)}"` : ''}`
          + `${destination ? ` data-destination="${esc(destination)}"` : ''}`
          + `${profileRevision !== undefined ? ` data-profile-revision="${profileRevision}"` : ''}>revoke</button></li>`;
      }).join('')
    ...
    const agentAccessList = s.agents?.length
      ? s.agents.map((agent) => {
        const folders = agent.backend === 'codex'
          ? '<span class="muted">Codex permissions are per agent: Ask for approval (default), Approve for me (Codex auto-review with a visible chat trail), or Full access (unsafe). Full access removes the sandbox, enables network, shows no approval prompts, and records no checkpoints. Plan and host ceilings use read-only. Folder grants do not confine Codex reads.</span>'
...
<body>
  ...
  <div class="card">
    <h2>Local folder discovery <span class="badge ok">session only</span></h2>
    <p>Parent or volume discovery never grants write or shell access. A decline blocks the current request; a later direct user request may ask again.</p>
    <ul id="localReadScopes">${localReadList}</ul>
  </div>

  <div class="card">
    <h2>Repository CLI configuration <span class="badge ok">content-bound</span></h2>
    <p>Claude or Codex project hooks, MCP servers, plugins and helpers load only under the exact stored workspace, working directory and content digest.</p>
    <ul id="repositoryCliGrants">${repositoryCliList}</ul>
  </div>
  ...
  <div class="card">
    ...
    <p class="row"><span>UnodeAi / Claude shell commands (<code>unode.commandApproval</code>)</span> ${cmdBadge}</p>
    <p class="row"><span>UnodeAi / Claude file writes (<code>unode.writeApproval</code>)</span> ${writeBadge}</p>
    <p class="muted">Codex CLI uses the native permission profile shown for that agent, not these two settings.</p>
  ...
  <script nonce="${scriptNonce}">
    ...
    document.querySelectorAll('button[data-revoke]').forEach((b) =>
      b.addEventListener('click', () => vscode.postMessage({
        ...
        profileId: b.getAttribute('data-profile-id'),
        destination: b.getAttribute('data-destination'),
        profileRevision: b.hasAttribute('data-profile-revision') ? Number(b.getAttribute('data-profile-revision')) : undefined,
      })));
    document.querySelectorAll('button[data-local-action]').forEach((b) =>
      b.addEventListener('click', () => vscode.postMessage({
        command: b.getAttribute('data-local-action'),
        root: b.getAttribute('data-local-root'),
      })));
    document.querySelectorAll('button[data-cli-kind]').forEach((b) =>
      b.addEventListener('click', () => vscode.postMessage({
        command: 'revokeRepositoryCliGrant',
        kind: b.getAttribute('data-cli-kind'),
        cwd: b.getAttribute('data-cli-cwd'),
        digest: b.getAttribute('data-cli-digest'),
      })));
