...
export interface TeamRulesPanelDeps {
  ...
  /** Re-check immediately before saving so a folder switch cannot write through a captured path. */
  canSave?: () => boolean;
}
...
/** Close a panel whose captured project binding is no longer valid. */
export function closeTeamRulesPanel(): void {
...
export async function openTeamRulesPanel(deps: TeamRulesPanelDeps): Promise<void> {
  ...
  panel.webview.onDidReceiveMessage(async (msg: { command?: string; text?: unknown; reviewPolicy?: unknown }) => {
    if (msg?.command === 'save' && typeof msg.text === 'string') {
      try {
        if (deps.canSave && !deps.canSave()) {
          throw new Error('the first workspace folder changed; reload the window before saving');
        }
        ...
        showResultNotice('information', 'Team policy and advisory guidance saved.');
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        showResultNotice('error', `Could not save team rules: ${message}`);
      }
    }
  });
}
