...
export interface AgentBuilderProviderOption {
  ...
  /** Explicit model discovery is offered only for the consent-gated Codex CLI route. */
  modelLoadAction?: 'codex-cli';
}
...
export interface AgentBuilderSavePayload {
  ...
  systemPrompt: string;
  skillIds: string[];
  /** Optional host-side narrowing used by reviewed saved-team restores; it can never add a tool. */
  allowedToolCeiling?: string[];
  /**
   * Host-only saved-team recovery. It changes confirmed capability grants while leaving every other
   * existing agent setting alone, including legacy model parameters a route no longer accepts.
   */
  permissionRestoreOnly?: boolean;
  playbooks: string[];
  mcpServers: string[];
...
export interface AgentBuilderPanelDeps {
  ...
  loadModels?: (providerId: string) => Promise<{ models: AgentBuilderModelOption[]; message: string }>;
...
export class AgentBuilderPanel {
  ...
  private async onMessage(msg: { command?: unknown; payload?: unknown; rowId?: unknown; action?: unknown }): Promise<void> {
    ...
    try {
      ...
      if (msg.command === 'loadModels' && this.viewModel && this.deps.loadModels) {
        const providerId = typeof (msg as { providerId?: unknown }).providerId === 'string'
          ? (msg as { providerId: string }).providerId
          : '';
        const provider = this.viewModel.providers.find((candidate) => candidate.id === providerId);
        if (!provider || provider.modelLoadAction !== 'codex-cli') return;
        try {
          const result = await this.deps.loadModels(providerId);
          void this.panel.webview.postMessage({
            command: 'modelsLoaded', providerId, models: result.models, message: result.message,
          });
        } catch (error) {
          void this.panel.webview.postMessage({
            command: 'modelLoadFailed', providerId,
      ...
      if (msg.command === 'promptTemplateAction' && this.agentId && this.deps.promptTemplateAction) {
        ...
        if (result.ok) {
          void showResultNotice('information', `UnodeAi Agent Builder: ${result.message}`);
        } else {
          void showResultNotice('warning', `UnodeAi Agent Builder: ${result.message}`);
        }
        if (result.ok) {
          await this.render();
        }
      ...
      if (msg.command === 'save' && this.viewModel) {
        ...
        if (!payload) {
          ...
          void showResultNotice('warning', `UnodeAi Agent Builder: ${reason}`);
          return;
        }
      ...
      void showResultNotice('error', `UnodeAi Agent Builder: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
...
export function renderAgentBuilderHtml(
  ...
  const orderedRoles = [...view.roles].sort((left, right) => byDisplayName(left.name, right.name));
  const selectedMcpServers = view.mcpServers.filter((server) => initial.mcpServers.includes(server.id));
  const unselectedMcpServers = view.mcpServers.filter((server) => !initial.mcpServers.includes(server.id));
  ...
  const roleOptions = orderedRoles.map((r) =>
  ...
  const initialRoleDefaultTier = orderedRoles.find((r) => r.id === initial.roleKey)?.tier ?? 'standard';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .model-combobox { position: relative; display: grid; grid-template-columns: minmax(0, 1fr) 30px; }
    .model-combobox > input { border-radius: 4px 0 0 4px; }
    .model-picker-button {
      border: 1px solid var(--vscode-input-border, var(--vscode-panel-border)); border-left: 0;
      border-radius: 0 4px 4px 0; color: var(--vscode-foreground);
      background: var(--vscode-button-secondaryBackground); cursor: pointer;
    }
    .model-picker-button:hover { background: var(--vscode-button-secondaryHoverBackground); }
    .model-options {
      ...
      box-shadow: 0 4px 12px var(--vscode-widget-shadow, rgba(0, 0, 0, .25));
    }
    .model-option { display: block; width: 100%; padding: 6px 8px; border: 0; text-align: left;
      color: var(--vscode-dropdown-foreground, var(--vscode-foreground)); background: transparent; cursor: pointer; }
    .model-option:hover, .model-option.active { background: var(--vscode-list-activeSelectionBackground); color: var(--vscode-list-activeSelectionForeground); }
    .model-option-source { display: block; margin-top: 2px; color: var(--vscode-descriptionForeground); font-size: 11px; }
...
<body>
  ...
  <div class="layout">
    <main class="main">
      ...
      <section class="panel">
        ...
        <div class="grid">
          ...
          <div class="field">
            ...
            <div class="model-combobox" data-model-combobox="model">
              <input id="model" autocomplete="off" spellcheck="false" placeholder="Type to filter models…" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="modelOptions">
              <button class="model-picker-button" type="button" data-open-model-picker="model" aria-label="Show all models">⌄</button>
              <div id="modelOptions" class="model-options" role="listbox" hidden></div>
            </div>
            <!-- Directly under the dropdown it explains (field feedback: below Context window it went unnoticed). -->
            <div class="status-line" id="modelStatus" role="status"></div>
            <button class="btn" id="loadCodexModels" type="button" hidden>Load models from Codex</button>
          </div>
          <div class="field">
            <label for="fallbackModel">Backup model</label>
            <div class="model-combobox" data-model-combobox="fallbackModel">
              <input id="fallbackModel" autocomplete="off" spellcheck="false" placeholder="Optional — type to filter…" role="combobox" aria-autocomplete="list" aria-expanded="false" aria-controls="fallbackModelOptions">
              <button class="model-picker-button" type="button" data-open-model-picker="fallbackModel" aria-label="Show all backup models">⌄</button>
              <div id="fallbackModelOptions" class="model-options" role="listbox" hidden></div>
            </div>
          </div>
      ...
      <section class="panel" id="skillPlaybooksSection">
        ...
        <p class="help">Claude CLI and Codex CLI both load assigned Playbooks through their native Skill systems. A Playbook never grants tools. An executable action can select only a handler compiled into the installed UnodeAi extension; Skill folders and remote catalogs cannot supply code. It can run only after Workspace Trust and a new exact-content approval for that one run. Executable actions are trusted local code with your user account's authority, not an OS sandbox.</p>
        ...
        <div class="toolbar">
          ...
          <select id="roleFilter" aria-label="Filter by role"><option value="">All roles</option>${orderedRoles.map((r) => `<option value="${escAttr(r.id)}">${esc(r.name)}</option>`).join('')}</select>
        ...
        <details class="section-advanced" id="skillFullList">
          ...
          <div class="skill-grid" id="skillGrid">${skillCards(view.catalog.skills, orderedRoles, initial.playbooks)}</div>
        </details>
      </section>
      ...
      <section class="panel" id="mcpGrantsSection">
        <h2>MCP Grants</h2>
        <p class="help" id="mcpCapabilityNotice" role="status"></p>
        <p class="selection-heading" id="selectedMcpHeading">${selectedMcpServers.length ? `Selected MCP servers (${selectedMcpServers.length})` : 'Selected MCP servers (none)'}</p>
        <div class="checks" id="selectedMcpChecks">${mcpChecks(selectedMcpServers, initial.mcpServers, '')}</div>
        <div class="empty" id="selectedMcpEmpty"${selectedMcpServers.length ? ' hidden' : ''}>No MCP servers granted yet. Open All MCP servers to add one.</div>
      </section>

      <details class="section-advanced" id="mcpAdvanced">
        <summary>
          <span>All MCP servers</span>
          <span class="advanced-hint" id="mcpAdvancedHint">${unselectedMcpServers.length} available; grants are explicit</span>
          <span class="advanced-notice" id="mcpAdvancedSummary" role="status" hidden></span>
        </summary>
        <section class="panel">
          <h2>All MCP servers</h2>
          <div class="checks" id="mcpChecks">${mcpChecks(unselectedMcpServers, initial.mcpServers, view.mcpServers.length ? '' : 'No MCP servers registered yet.')}</div>
  ...
  <script nonce="${scriptNonce}">
    ...
    const roles = ${jsonForScript(orderedRoles)};
    ...
    let openModelPickerId = null;
    let activeModelOptionIndex = -1;

    function modelSourceSummary(models) {
      const sources = [...new Set(models.map((model) => model.source).filter(Boolean))];
      return sources.length ? sources.join(', ') : 'connection catalog';
    }

    function closeModelPicker() {
      if (!openModelPickerId) return;
      const input = byId(openModelPickerId);
      const menu = byId(openModelPickerId === 'model' ? 'modelOptions' : 'fallbackModelOptions');
      menu.hidden = true;
      input.setAttribute('aria-expanded', 'false');
      openModelPickerId = null;
      activeModelOptionIndex = -1;
    }

    function chooseModel(inputId, id) {
      const input = byId(inputId);
      input.value = id;
      if (inputId === 'model') {
        selectedModel = id;
        syncToolProtocolAutoLabel();
        const provider = providers.find((candidate) => candidate.id === byId('provider').value);
    ...
    // A native datalist filters against the input's current saved value before it opens, so a saved
    // codex-cli-default hides every gpt-* row. This explicit listbox opens unfiltered, then filters only
    // after the user types. The input remains free-form for custom model ids.
    function renderModelPicker(inputId, showAll) {
      const provider = providers.find((candidate) => candidate.id === byId('provider').value) || providers[0];
      const models = modelCatalog.get(provider?.id) || [];
      const input = byId(inputId);
      const menu = byId(inputId === 'model' ? 'modelOptions' : 'fallbackModelOptions');
      ...
      for (const model of filtered) {
        ...
        if (model.source) {
          ...
          source.className = 'model-option-source';
          source.textContent = model.source;
          option.appendChild(source);
        }
        option.addEventListener('mousedown', (event) => event.preventDefault());
        option.addEventListener('click', () => chooseModel(inputId, model.id));
        menu.appendChild(option);
      }
      if (filtered.length === 0) {
        const empty = document.createElement('div');
        empty.className = 'empty';
        empty.textContent = models.length ? 'No matching models. You can still use the typed model id.' : 'No models are available yet.';
        menu.appendChild(empty);
      }
      openModelPickerId = inputId;
      activeModelOptionIndex = -1;
      menu.hidden = false;
    ...
    function syncModels() {
      ...
      const models = modelCatalog.get(provider?.id) || [];
      if (openModelPickerId) renderModelPicker(openModelPickerId, false);
      ...
      syncToolProtocolAutoLabel();
      const loadButton = byId('loadCodexModels');
      const hasCodexAccountModels = models.some((model) => model.source === 'Codex account');
      const needsCodexLoad = provider?.modelLoadAction === 'codex-cli' && !hasCodexAccountModels;
      loadButton.hidden = !needsCodexLoad;
      loadButton.disabled = loadingProviders.has(providerId);
      if (loadingProviders.has(providerId)) {
        byId('modelStatus').textContent = 'Loading live models and prices...';
      } else if (needsCodexLoad) {
        byId('modelStatus').textContent = models.length + ' bundled model' + (models.length === 1 ? '' : 's') + '. Account models load from Codex.';
      } else if (loadedProviders.has(providerId)) {
        byId('modelStatus').textContent = models.length
          ? models.length + ' models · ' + modelSourceSummary(models) + '.'
          : 'No live models returned.';
      } else {
        byId('modelStatus').textContent = models.length
          ? models.length + ' models · ' + modelSourceSummary(models) + '.'
          : 'No models are available for this connection.';
      }
    }
    ...
    function syncCapabilityControls(provider) {
      ...
      if (notice) {
        notice.textContent = unavailable
          ? unavailable === 1
            ? '1 skill capability is unavailable on this connection. Existing selections are kept only while you switch connections; saving on this connection removes them.'
            : unavailable + ' skill capabilities are unavailable on this connection. Existing selections are kept only while you switch connections; saving on this connection removes them.'
          : 'All listed skill capabilities are available on this connection.';
      }
      if (!skillsAvailable && !editingExistingAgent) {
        selectedSkills.clear();
    ...
    function syncSkillPlaybookControls(provider) {
      const available = provider?.playbooksAvailable !== false;
      ...
      if (notice) {
        notice.textContent = available
          ? ''
          : 'This connection does not load UnodeAi Playbooks. Existing selections are kept only while you switch connections; saving on this connection removes them.';
      }
    ...
    function syncMcpControls(provider) {
      ...
      if (notice) {
        notice.textContent = available
          ...
          : 'MCP grants are unavailable on this connection. Existing selections are kept only while you switch connections; saving on this connection removes them.';
      }
      setAdvancedIssue('mcp', !available);
      syncMcpCards();
    }

    function syncMcpCards() {
      const fullList = byId('mcpChecks');
      const selectedList = byId('selectedMcpChecks');
      const boxes = [...document.querySelectorAll('[data-mcp-id]')];
      let selectedCount = 0;
      boxes.forEach((box) => {
      ...
      byId('selectedMcpHeading').textContent = selectedCount
        ? 'Selected MCP servers (' + selectedCount + ')'
        : 'Selected MCP servers (none)';
      byId('selectedMcpEmpty').hidden = selectedCount > 0;
      byId('mcpAdvancedHint').textContent = boxes.length - selectedCount + ' available; grants are explicit';
    }
    ...
    function syncFolderAccessIssues() {
      ...
      if (byId('provider').value === 'anthropic' && readwrite.length > 1) {
        out.push('Claude CLI can use at most one Read+Write folder. Keep one writable folder or switch extra rows to Read.');
      }
    ...
    document.addEventListener('click', (event) => {
      if (event.target.closest('#loadCodexModels')) {
        const providerId = byId('provider').value;
        const provider = providers.find((candidate) => candidate.id === providerId);
        if (provider?.modelLoadAction !== 'codex-cli') return;
        loadingProviders.add(providerId);
        byId('modelStatus').textContent = 'Checking Codex and loading account models...';
        byId('loadCodexModels').disabled = true;
        vscode.postMessage({ command: 'loadModels', providerId });
        return;
      }
    ...
    byId('model').addEventListener('input', () => {
      selectedModel = byId('model').value;
      if (openModelPickerId === 'model') renderModelPicker('model', false);
      syncToolProtocolAutoLabel();
      syncToolProtocolControl(providers.find((provider) => provider.id === byId('provider').value));
      syncModelParamControls(providers.find((provider) => provider.id === byId('provider').value));
    });
    byId('fallbackModel').addEventListener('input', () => {
      selectedFallback = byId('fallbackModel').value;
      if (openModelPickerId === 'fallbackModel') renderModelPicker('fallbackModel', false);
    });
    for (const inputId of ['model', 'fallbackModel']) {
      const input = byId(inputId);
      input.addEventListener('focus', () => renderModelPicker(inputId, true));
      input.addEventListener('keydown', (event) => {
        if (event.key === 'Escape') { closeModelPicker(); return; }
        if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp' && event.key !== 'Enter') return;
        const menu = byId(inputId === 'model' ? 'modelOptions' : 'fallbackModelOptions');
        if (menu.hidden) renderModelPicker(inputId, true);
        const options = [...menu.querySelectorAll('.model-option')];
        if (options.length === 0) return;
        if (event.key === 'Enter' && activeModelOptionIndex >= 0) {
          event.preventDefault();
          chooseModel(inputId, options[activeModelOptionIndex].dataset.modelId || '');
          return;
        }
        if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
          event.preventDefault();
          activeModelOptionIndex = event.key === 'ArrowDown'
            ? (activeModelOptionIndex + 1) % options.length
            : (activeModelOptionIndex - 1 + options.length) % options.length;
          options.forEach((option, index) => option.classList.toggle('active', index === activeModelOptionIndex));
          options[activeModelOptionIndex].scrollIntoView({ block: 'nearest' });
        }
      });
    }
    document.querySelectorAll('[data-open-model-picker]').forEach((button) => button.addEventListener('click', () => {
      const inputId = button.dataset.openModelPicker;
      if (inputId) {
        byId(inputId).focus();
        renderModelPicker(inputId, true);
      }
    }));
    document.addEventListener('pointerdown', (event) => {
      if (openModelPickerId && !event.target.closest('[data-model-combobox]')) closeModelPicker();
    });
    window.addEventListener('message', (event) => {
      ...
      if (msg.command === 'mcpServers' && Array.isArray(msg.servers)) {
        ...
        const selectedContainer = document.getElementById('selectedMcpChecks');
        if (!container || !selectedContainer) { return; }
        const checked = new Set([...document.querySelectorAll('[data-mcp-id]:checked')].map((el) => el.dataset.mcpId));
      ...
      if ((msg.command === 'modelsLoaded' || msg.command === 'modelLoadFailed') && typeof msg.providerId === 'string') {
        loadingProviders.delete(msg.providerId);
        if (msg.command === 'modelsLoaded' && Array.isArray(msg.models)) {
          loadedProviders.add(msg.providerId);
          modelCatalog.set(msg.providerId, msg.models.filter((model) => model && typeof model.id === 'string'));
        }
        if (msg.providerId === byId('provider').value) {
          syncModels();
          byId('modelStatus').textContent = typeof msg.message === 'string' ? msg.message : byId('modelStatus').textContent;
          if (msg.command === 'modelsLoaded') renderModelPicker('model', true);
        }
        return;
      }
...
function capabilityChecks(capabilities: AgentBuilderCapabilityOption[], selected: string[]): string {
  ...
  return ordered.map((cap) => /* html */`
    <label class="check">
      ...
      <span><span class="skill-title">${esc(cap.name)}</span><span class="meta"> ${esc(cap.category)}</span><br><span class="meta">${esc(cap.description)}</span><br><span class="meta">Resulting permissions: ${esc((cap.requiredTools ?? []).map(permissionToolLabel).join(', ') || 'read-only')}</span><br><span class="meta" data-capability-limit hidden>Unavailable on this connection.</span></span>
    </label>`
  ).join('');
}
...
function mcpChecks(servers: AgentBuilderMcpOption[], selected: string[], emptyMessage = 'No MCP servers registered yet.'): string {
  if (servers.length === 0) {
    return emptyMessage ? `<div class="empty">${esc(emptyMessage)}</div>` : '';
  }
  const selectedSet = new Set(selected);
