...
the whole tree and that returned filenames and content may go to the configured model provider; a decline,
dismissed dialog, or unavailable dialog fails closed for the rest of the extension session. `volume` uses the
same gate with an extra warning that a filesystem volume can include other users' profiles. `workspace`,
`parent`, and `volume` are the only values; the setting is ignored when Workspace Trust is off and remains
restricted from workspace configuration.
