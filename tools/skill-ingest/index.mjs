...
const SAFETY_PATTERNS = [
  ...
  },
  {
    ...
    pattern: /\b(?:rm\s+-rf|rmdir\s+\/s|rd\s+\/s|Remove-Item\b.*-Recurse|DROP\s+TABLE|git\s+push\s+--force|reg\s+delete|apt-get\s+purge|yum\s+remove|brew\s+uninstall)\b/i,
  },
  {
    code: 'privilege_or_permission_change',
    message: 'Instruction references privilege, ownership, permission, or execution-policy changes',
    pattern: /\b(?:sudo|chown|chmod|icacls|TakeOwnership|Set-ExecutionPolicy|chattr|chflags)\b/i,
  },
  {
    code: 'hidden_or_bypass_execution',
    message: 'Instruction references a hidden process or policy-bypass execution shape',
    pattern: /(?:\b(?:powershell|pwsh)\b.*\bExecutionPolicy\s+Bypass\b|\bWindowStyle\s+Hidden\b|\b-w\s+hidden\b)/i,
  },
  {
    code: 'persistence_or_service_change',
    message: 'Instruction references persistence or service-control mechanisms',
    pattern: /\b(?:schtasks|crontab|launchctl|systemctl|Stop-Service|taskkill|pkill|sc\.exe\s+delete|reg\s+add)\b/i,
  },
  {
