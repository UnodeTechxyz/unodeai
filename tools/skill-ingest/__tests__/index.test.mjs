...
describe('skill-ingest instruction-safety gate', () => {
  ...
  it('flags cross-platform privilege, bypass and persistence shapes for review', () => {
    const findings = scanInstructionSafety([
      'sudo chmod 000 ./target',
      'powershell -ExecutionPolicy Bypass -WindowStyle Hidden -File helper.ps1',
    ...
    expect([...new Set(findings.map((finding) => finding.code))]).toEqual([
      ...
      'hidden_or_bypass_execution',
      'persistence_or_service_change',
      'destructive_command',
    ]);
    expect(findings.every((finding) => finding.severity === 'flag')).toBe(true);
  });
});
