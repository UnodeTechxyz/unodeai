...
/**
 * Poll a value until it satisfies `done`, or give up. For post-conditions that a command deliberately
 * does not wait for: the last observed value is returned either way, so the caller's own assertion
 * produces the diff rather than this helper reporting a bare timeout.
 */
async function waitFor<T>(read: () => T, done: (value: T) => boolean, timeoutMs = 15_000): Promise<T> {
...
describe('UnodeAi activation', () => {
  ...
  it('imports a local native-text PDF through the packaged attachment boundary without retaining its source metadata', async function () {
    if (!vscode.workspace.workspaceFolders?.length) {
      // The bundled smoke has a separate no-folder pass. This case belongs to its explicit refusal
      // assertion below; the attachment pipeline itself is exercised in the workspace-bound pass.
      this.skip();
      return;
    }
    const ext = vscode.extensions.getExtension(EXT_ID);
  ...
  it('refuses the workspace-bound PDF attachment action when no folder is open', async function () {
    if (vscode.workspace.workspaceFolders?.length) {
      this.skip();
      return;
    }
    ...
    const probe = ext!.exports as {
      __testLocalPdfAttachmentPipeline?: (attachment: {
        name: string; mime: string; kind: 'pdf'; dataBase64: string; size: number;
      }, fixture: { e2e: true }) => Promise<unknown>;
    };
    assert.strictEqual(typeof probe.__testLocalPdfAttachmentPipeline, 'function');
    const bytes = threePagePdfWithText(['ONE', 'TWO', 'THREE']);
    await assert.rejects(
      probe.__testLocalPdfAttachmentPipeline!({
        name: 'no-workspace.pdf',
        mime: 'application/pdf',
        kind: 'pdf',
        dataBase64: bytes.toString('base64'),
        size: bytes.byteLength,
      }, { e2e: true }),
      /Open a workspace folder before using this UnodeAi action/,
    );
  });
  ...
  it('repairs an old key coefficient only after the user opens Settings', async () => {
    ...
    try {
      ...
      // v0.9.78 detached this repair from the command on purpose: an editor that refuses a configuration
      // write while settings.json has an error must not be able to stop the panel from opening. So the
      // post-condition is eventual, not immediate. Reading once here would race the very decoupling under
      // test and pass or fail on scheduler luck. `WorkspaceConfiguration` is also a snapshot in this
      // extension-host version, so re-read the handle on every poll rather than holding one.
      const repaired = await waitFor(
        () => vscode.workspace.getConfiguration('unode').get<unknown>('priceMultiplier'),
        (value) => JSON.stringify(value) === JSON.stringify({ unode: 1 }),
      );
      assert.deepStrictEqual(repaired, { unode: 1 }, `key action was ${keyAction}`);
    } finally {
