...
import { assertIntegrationCatalog, proveIntegrationCatalogPlantedFailures } from './check-integration-catalog.mjs';
...
const smokeWorkspace = join(smokeDir, 'workspace');
...
function assertMarketplaceCatalogPresent(extensionPath) {
  for (const name of ['agents', 'mcp', 'skills']) {
    ...
    if (name === 'mcp') {
      assertIntegrationCatalog(parsed, 'bundled VSIX integration catalog');
      proveIntegrationCatalogPlantedFailures(parsed[0]);
    }
  }
}
...
try {
  ...
  rmSync(smokeDir, { recursive: true, force: true, maxRetries: 20, retryDelay: 250 });
  ...
  const noFolderTestOptions = {
    ...workspaceTestOptions,
    mochaOpts: {
      ...workspaceTestOptions.mochaOpts,
      grep: 'activates the extension|refuses the workspace-bound PDF attachment action',
      invert: false,
    },
  };
...
const withWorkspace = process.argv[2] === 'workspace';
process.env.VSCODE_TEST_OPTIONS = JSON.stringify(withWorkspace
  ? ${JSON.stringify(workspaceTestOptions)}
  : ${JSON.stringify(noFolderTestOptions)});
...
async function main() {
  ...
  if (withWorkspace) args.push(${JSON.stringify(smokeWorkspace)});
...
`);
  ...
  // First prove the packaged extension activates and refuses a workspace-bound action without a
  // folder. Then use a real, disposable workspace to exercise the full attachment and UI smoke.
  run(nodeBin, [launchPath, 'no-folder']);
  run(nodeBin, [launchPath, 'workspace']);
} finally {
  // A second Electron process can hold its disposable profile briefly after its extension host exits.
  // Let Node's Windows EPERM retry path finish that teardown without turning a successful smoke red.
  rmSync(smokeDir, { recursive: true, force: true, maxRetries: 20, retryDelay: 250 });
}
