...
try {
  ...
  const invalidAnchors = requirements.flatMap((requirement) => {
    const applied = replaceExactlyOnce(original, { file: SOURCE, from: requirement.from, to: requirement.to });
    return applied.kind === 'invalid' ? [`[${requirement.id}] ${applied.reason}`] : [];
  });
  if (invalidAnchors.length > 0) {
    throw new Error('Sensor mutation population contains invalid anchors:\n' + invalidAnchors.map((item) => '  - ' + item).join('\n'));
  }
  ...
  if (baseline.status !== 0) {
    ...
    for (const [index, requirement] of requirements.entries()) {
      const applied = replaceExactlyOnce(original, {
        file: SOURCE,
      ...
      if (applied.kind === 'invalid') {
        ...
        console.log('INVALID  ' + number + '  [' + requirement.id + '] ' + requirement.reason);
        invalidRuns.push({ requirement, reason: applied.reason });
    ...
    if (invalidRuns.length > 0) {
      console.error('\n' + invalidRuns.length + ' invalid mutation run(s):');
