...
export function bareThemeTokenLiterals(text) {
  ...
  return [...text.matchAll(HEX_LITERAL)]
    ...
    // `&#9888;` is a numeric HTML entity, not CSS `#9888`. The broad source scanner sees both unless
    // it accounts for the entity delimiter; accepting it as a colour literal would make this ratchet lie.
    .filter((match) => !isHtmlNumericEntity(text, match))
    .map((match) => ({ literal: match[0], line: lineForOffset(text, match.index ?? 0) }));
}

function isHtmlNumericEntity(text, match) {
  const start = match.index ?? -1;
  return start > 0 && text[start - 1] === '&' && text[start + match[0].length] === ';';
}
...
function selfTest() {
  ...
  const entity = evaluateThemeTokenRatchet(new Map([['Demo.ts', '<span>&#9888;</span>']]), new Map([['Demo.ts', 0]]));
  if (entity.violations.length !== 0) {
    throw new Error('webview theme-token gate self-test failed: an HTML numeric entity was counted as a colour literal');
  }
