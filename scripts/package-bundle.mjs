...
try {
  ...
  // Runtime dependencies are already inside the esbuild outputs and the staged manifest deliberately has
  // an empty dependency map. Asking VSCE to traverse npm here can yield an empty list on Windows.
  assertVsixBoundary(await listVsixFiles(staging, { noDependencies: true }), {
  ...
  run(vsce, ['package', '--no-dependencies', '--out', bundleOutPath], staging, {
    ...process.env,
    TZ: 'UTC',
    SOURCE_DATE_EPOCH,
  });
