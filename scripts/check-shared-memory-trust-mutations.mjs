...
try {
  ...
  const trustedMutation = replaceExactlyOnce(source, {
    file: SOURCE,
    from: trusted,
    to: ".filter((note) => note.kind === 'contract')",
  });
  if (trustedMutation.kind === 'invalid') {
    throw new Error(`INVALID trusted-note mutation: ${trustedMutation.reason}`);
  }
  const untrustedMutation = replaceExactlyOnce(trustedMutation.text, {
    file: SOURCE,
    from: untrusted,
    to: ".filter((note) => note.kind !== 'contract')",
  });
  if (untrustedMutation.kind === 'invalid') {
    throw new Error(`INVALID untrusted-note mutation: ${untrustedMutation.reason}`);
  }
  writeFileSync(sourcePath, untrustedMutation.text, 'utf8');
